...
import { isEmbeddedMode, setEmbeddedMode } from "../engine/infra/embedded-mode.js";
...
vi.mock("../engine/agents/defaults.js", () => ({
...
vi.mock("../engine/gateway/server-constants.js", () => ({
...
describe("EmbeddedTuiBackend", () => {
  ...
  it("uses the selected agent when running local global goal commands", async () => {
    loadSessionEntryMock.mockReturnValueOnce({
      cfg: {},
      canonicalKey: "global",
      storePath: "/tmp/crosswms-work-sessions.json",
  ...
  it("loads runtime plugins for the send-path workspace before returning embedded history", async () => {
    ...
    loadSessionEntryMock.mockReturnValue({
      ...
      canonicalKey: "agent:main:main",
      storePath: "/tmp/crosswms-sessions.json",
      entry: { spawnedWorkspaceDir: "/tmp/crosswms-custom-workspace" },
    });
    ...
    expect(ensureRuntimePluginsLoadedMock).toHaveBeenCalledWith({
      config: cfg,
      workspaceDir: "/tmp/crosswms-agent-main",
    });
  });
  ...
  it("aborts local post-turn maintenance when stop grace elapses", async () => {
    await withEnvAsync({ CDFKNOW_TUI_LOCAL_RUN_SHUTDOWN_GRACE_MS: "5" }, async () => {
      const { EmbeddedTuiBackend } = await import("./embedded-backend.js");
      const pending = deferred<{
        payloads: Array<{ text: string }>;
        meta: Record<string, unknown>;
  ...
  it("fails a queued local send immediately when shutdown grace is zero", async () => {
    await withEnvAsync({ CDFKNOW_TUI_LOCAL_RUN_SHUTDOWN_GRACE_MS: "0" }, async () => {
      const { EmbeddedTuiBackend } = await import("./embedded-backend.js");
      const first = deferred<{
        payloads: Array<{ text: string }>;
        meta: Record<string, unknown>;
