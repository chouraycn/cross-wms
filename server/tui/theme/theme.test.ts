...
import { importFreshModule } from "@cdf-know/plugin-sdk/test-fixtures";
...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      CDFKNOW_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when CDFKNOW_THEME=light", async () => {
    const mod = await importThemeWithEnv({ CDFKNOW_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when CDFKNOW_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ CDFKNOW_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats CDFKNOW_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ CDFKNOW_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("CDFKNOW_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      CDFKNOW_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ CDFKNOW_THEME: "light" });
    const darkMod = await importThemeWithEnv({ CDFKNOW_THEME: "dark" });
