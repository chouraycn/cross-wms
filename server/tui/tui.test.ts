...
import type { CrossWmsConfig } from "../config/config.js";
import { MAX_TIMER_TIMEOUT_MS } from "../engine/infra/parse-finite-number.js";
import { MALFORMED_STREAMING_FRAGMENT_ERROR_MESSAGE } from "../engine/assistant-error-format.js";
...
describe("resolveTuiFooterHostLabel", () => {
  ...
  it("renders only remote hosts when explicitly enabled", () => {
    const config = { tui: { footer: { showRemoteHost: true } } } satisfies CrossWmsConfig;
...
describe("resolveInitialTuiAgentId", () => {
  const cfg: CrossWmsConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/crosswms" },
        { id: "ops", workspace: "/tmp/crosswms/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/crosswms/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run cdf-know devices list");
    expect(state.pairingHint).toContain("cdf-know devices list");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("schedules a process-exit guard after standalone TUI return", () => {
    ...
    callback?.();
    expect(writeStderr).toHaveBeenCalledWith("cdf-know tui forcing process exit after return\n");
...
describe("resolveLocalAuthCliInvocation", () => {
  it("uses the source runner when dist is unavailable", () => {
    expect(
      resolveLocalAuthCliInvocation({
        ...
        wrapperPath: "/repo/cross-wms.mjs",
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/cross-wms.mjs", "models", "auth", "login"],
    });
  });
});
