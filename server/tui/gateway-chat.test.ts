...
} from "../engine/gateway/gateway-connection.test-mocks.js";
import { captureEnv, withEnvAsync } from "../test-utils/env.js";

vi.mock("../config/config.js", async () => {
  const mocks = await import("../engine/gateway/gateway-connection.test-mocks.js");
  return {
...
vi.mock("../engine/gateway/net.js", async () => {
  const mocks = await import("../engine/gateway/gateway-connection.test-mocks.js");
  return {
...
const { GatewayClientRequestError } = await import("../engine/gateway/client.js");
...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `crosswms-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      "CDFKNOW_GATEWAY_URL",
      "CDFKNOW_GATEWAY_TOKEN",
      "CDFKNOW_GATEWAY_PASSWORD",
      "CDFKNOW_TUI_SETUP_AUTH_SOURCE",
    ]);
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.CDFKNOW_STATE_DIR ?? "/tmp/crosswms",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.CDFKNOW_CONFIG_PATH ?? `${stateDir}/crosswms.json`,
    );
    delete process.env.CDFKNOW_GATEWAY_URL;
    delete process.env.CDFKNOW_GATEWAY_TOKEN;
    delete process.env.CDFKNOW_GATEWAY_PASSWORD;
    delete process.env.CDFKNOW_TUI_SETUP_AUTH_SOURCE;
  });
  ...
  it("uses config auth token for local mode when both config and env tokens are set", async () => {
    ...
    await withEnvAsync({ CDFKNOW_GATEWAY_TOKEN: "env-token" }, async () => {
      const result = await resolveGatewayConnection({});
      expect(result.token).toBe("config-token");
    });
  });

  it("falls back to CDFKNOW_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("keeps normal TUI local password mode env precedence by default", async () => {
    ...
    await withEnvAsync({ CDFKNOW_GATEWAY_PASSWORD: "env-password" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("uses configured local password for setup-launched TUI despite stale gateway password env", async () => {
    ...
    await withEnvAsync(
      {
        CDFKNOW_GATEWAY_PASSWORD: "stale-env-password", // pragma: allowlist secret
        CDFKNOW_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
        expect(result.password).toBe("config-password");
  ...
  it("still resolves env SecretRefs for setup-launched TUI config auth", async () => {
    loadConfig.mockReturnValue({
      ...
      gateway: {
        ...
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "CDFKNOW_GATEWAY_PASSWORD" },
        },
      },
    });
    ...
    await withEnvAsync(
      {
        CDFKNOW_GATEWAY_PASSWORD: "resolved-ref-password", // pragma: allowlist secret
        CDFKNOW_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
  ...
  it("prefers CDFKNOW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "CDFKNOW_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "crosswms-tui-file-secret-"));
...
describe("GatewayChatClient", () => {
  ...
  it("identifies the TUI as a tui client and skips device identity on insecure local ui paths", async () => {
    ...
    try {
      ...
      expect(constructedOptions[0]).toMatchObject({
        clientName: "cdf-know-tui",
        mode: "ui",
        preauthHandshakeTimeoutMs: 30_000,
        deviceIdentity: null,
      });
    } finally {
      vi.doUnmock("../engine/gateway/client.js");
      vi.resetModules();
    }
  });
  ...
  it("surfaces loopback block-mode start failures through disconnect handler", async () => {
    vi.useFakeTimers();
    const { startProxy, stopProxy } = await import("../engine/infra/net/proxy/proxy-lifecycle.js");
