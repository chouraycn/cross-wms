...
const LOCAL_MODEL_ENDPOINTS = (() => {
  try {
    const envEndpoints = process.env.CROSS_WMS_LOCAL_ENDPOINTS;
    if (envEndpoints) return JSON.parse(envEndpoints);
  } catch (e) {
    logger.warn('[Models] 本地端点环境变量解析失败:', (e as Error).message);
  }
  return DEFAULT_LOCAL_MODEL_ENDPOINTS;
})();
        ...
        try {
          ...
          if (endpoint.includes('anthropic.com')) {
            const resp = await fetch(`${endpoint}/v1/messages`, {
              ...
              body: JSON.stringify({
                model: model.id || process.env.CROSS_WMS_MODELS_DEFAULT || 'claude-3-5-sonnet-latest',
                max_tokens: 1,
    ...
    try {
      ...
      if (apiEndpoint.includes('anthropic.com')) {
        const testBody = JSON.stringify({
          model: modelId || process.env.CROSS_WMS_MODELS_DEFAULT || 'claude-3-5-sonnet-latest',
          max_tokens: 10,
          messages: [{ role: 'user', content: 'Hi' }],
        });
