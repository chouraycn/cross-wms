...
import { normalizeCsvOrLooseStringList } from "../infra/string-normalization.js";
...
export function normalizeStringList(input: any): string[] {
...
export function resolveOpenClawManifestBlock(params: {
  ...
  try {
    ...
    for (const key of manifestKeys) {
      const candidate = (parsed as Record<string, any>)[key];
...
export function resolveOpenClawManifestRequires(
  ...
  const requiresRaw =
    typeof metadataObj.requires === "object" && metadataObj.requires !== null
      ? (metadataObj.requires as Record<string, any>)
      : undefined;
  if (!requiresRaw) {
    return undefined;
  }
...
export function resolveOpenClawManifestInstall<T>(
  metadataObj: Record<string, any>,
  parseInstallSpec: (input: any) => T | undefined,
): T[] {
  const installRaw = Array.isArray(metadataObj.install) ? (metadataObj.install as any[]) : [];
  return installRaw
...
export function resolveOpenClawManifestOs(metadataObj: Record<string, any>): string[] {
