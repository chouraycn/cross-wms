...
function validateProviderOptionsAgainstDeclaration(params: {
  ...
  declaration: Readonly<Record<string, unknown>> | undefined;
}): string | undefined {
  ...
  for (const key of keys) {
    const expected = declaration[key] as string;
...
export async function generateVideo(
  ...
  for (const candidate of candidates) {
    ...
    const timeoutMs = resolveMediaProviderRequestTimeoutMs({
      timeoutMs: requestedTimeoutMs as number | undefined,
    ...
    try {
      ...
      const result: VideoResult = await provider.generateVideo(generationRequest as any);
