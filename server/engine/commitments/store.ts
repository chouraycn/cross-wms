...
export async function addCommitment(params: {
  ...
  const results = await upsertInferredCommitments({
    cfg: params.cfg,
    item: params.item,
    candidates: [
      {
...
export async function claimDueCommitments(params: {
  ...
  const commitments = await listDueCommitmentsForSession(params);
  if (commitments.length > 0) {
    await markCommitmentsAttempted({
      cfg: params.cfg,
      ids: commitments.map((c) => c.id),
      nowMs: params.nowMs,
    });
  }
  return commitments;
...
export async function updateCommitmentStatus(
  id: string,
  status: Extract<CommitmentStatus, "pending" | "sent" | "dismissed" | "snoozed" | "expired">,
  options?: { nowMs?: number; snoozedUntilMs?: number },
): Promise<boolean> {
  const nowMs = options?.nowMs ?? Date.now();
  let changed = false;
  await runExclusiveCommitmentsStoreWrite(resolveCommitmentStorePath(), async () => {
    const store = await loadCommitmentStore();
    store.commitments = store.commitments.map((c) => {
      if (c.id !== id) return c;
      changed = true;
      ...
      if (status === "sent") updated.sentAtMs = nowMs;
      if (status === "dismissed") updated.dismissedAtMs = nowMs;
      if (status === "expired") updated.expiredAtMs = nowMs;
      if (status === "snoozed" && options?.snoozedUntilMs) {
        updated.snoozedUntilMs = options.snoozedUntilMs;
      }
      return updated;
    });
    if (changed) {
...
export async function deleteCommitment(id: string): Promise<boolean> {
  ...
  await runExclusiveCommitmentsStoreWrite(resolveCommitmentStorePath(), async () => {
    ...
    changed = store.commitments.length !== before;
    if (changed) {
      await saveCommitmentStore(undefined, store);
    }
  });
...
export async function addHeartbeatRecord(record: {
  commitmentId: string;
...
export async function getCommitmentStats(params?: {
  agentId?: string;
}): Promise<{ total: number; pending: number; sent: number; dismissed: number; expired: number }> {
  const store = await loadCommitmentStore();
  const commitments = params?.agentId
    ? store.commitments.filter((c) => c.agentId === params.agentId)
    : store.commitments;
  return {
    total: commitments.length,
    pending: commitments.filter((c) => c.status === "pending").length,
    sent: commitments.filter((c) => c.status === "sent").length,
    dismissed: commitments.filter((c) => c.status === "dismissed").length,
