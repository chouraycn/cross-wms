/**
 * ACP Event Mapper
 * 事件映射器 - 将 ACP prompt 和工具事件转换为 Gateway 友好的文本、文件和元数据
 *
 * 参考 openclaw/src/acp/event-mapper.ts 设计
 */
...
type ToolKind = "read" | "edit" | "delete" | "move" | "search" | "execute" | "fetch" | "other";
...
export function extractTextFromPrompt(prompt: ContentBlock[], maxBytes?: number): string {
  ...
  for (const block of prompt) {
    ...
    if (block.type === "text") {
      ...
      const title = block.resource_link?.title ? ` (${escapeResourceTitle(block.resource_link.title)})` : "";
      const uri = block.resource_link?.uri ? escapeInlineControlChars(block.resource_link.uri) : "";
