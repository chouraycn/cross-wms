/**
 * ACP Commands
 * ACP 可用命令列表 - 暴露给兼容客户端的命令定义
 *
 * 参考 openclaw/src/acp/commands.ts 设计
 */
...
export function getCommandByName(name: string): AvailableCommand | undefined {
  return BASE_AVAILABLE_COMMANDS.find((cmd) => cmd.name === name);
}

export function hasCommand(name: string): boolean {
  return BASE_AVAILABLE_COMMANDS.some((cmd) => cmd.name === name);
}