...
import type { ResolvedAcpxPluginConfig } from "./configSchema.js";
...
function buildCodexAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Codex",
    packageSpec: `${CODEX_ACP_PACKAGE}@latest`,
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    packageSpec: `${CLAUDE_ACP_PACKAGE}@latest`,
...
async function prepareIsolatedCodexHome(params: {
  ...
  const trustedProjectPaths = [params.workspaceDir];
...
function renderIsolatedCodexConfig(params: {
  ...
  const config = params.sourceConfigToml || "";
  const pathsSection = params.projectPaths
    .map((p) => `  "${p}"`)
    .join("\n");
  const trustedPathsConfig = `\n[trusted_paths]\n${pathsSection}\n`;
  return config + trustedPathsConfig;
}
...
export async function writeClaudeAcpWrapper(
  baseDir: string,
  installedBinPath?: string,
): Promise<string> {
  await fs.mkdir(baseDir, { recursive: true });
...
function extractConfiguredAdapterArgs(params: {
  ...
  const packageIndex = parts.findIndex((part) =>
    isAcpPackageSpec(part, params.packageName),
  );
  if (packageIndex >= 0) {
...
export async function prepareAcpxCodexAuthConfig(params: {
  ...
  const claudeWrapperPath = await writeClaudeAcpWrapper(codexBaseDir);
