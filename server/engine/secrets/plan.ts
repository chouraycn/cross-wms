/**
 * Simplified port of openclaw/src/secrets/plan.ts
 *
 * Validates and normalizes serialized secrets apply plans before config mutation.
 *
 * Simplification: resolvePlanTargetAgainstRegistry from target-registry-query.js is
 * not ported yet. The stub returns null, so resolveValidatedPlanTarget always returns
 * null and isSecretsApplyPlan will reject plans with targets until target-registry-query
 * is ported. Type safety is preserved.
 */
import { isRecord as isObjectRecord } from "../infra/record-coerce.js";
...
import type { ResolvedPlanTarget } from "./target-registry-types.js";
...
/**
 * Stub for resolvePlanTargetAgainstRegistry from target-registry-query.js (unported).
 * Returns null so plan validation rejects targets until target-registry-query is ported.
 */
function resolvePlanTargetAgainstRegistry(_candidate: {
  type: SecretsPlanTargetType;
  pathSegments: string[];
  providerId?: string;
  accountId?: string;
