/**
 * Config-level secret reference and resolution types.
 *
 * Migrated from openclaw/src/config/types.secrets.ts (inlined) so the secrets
 * engine is self-contained under cross-wms without depending on openclaw's
 * config subsystem. These types describe config-level SecretRefs
 * ({ source, provider, id }) which are distinct from the storage-level
 * SecretRef ({ provider, key, type }) defined in types.ts.
 *
 * `isRecord` is sourced from the shared normalization-core package
 * (path-mapped in server/tsconfig.json).
 */
...
/**
 * Minimal config carrier shape used by the secrets engine. Adapted from
 * openclaw's OpenClawConfig: only the `secrets` block is consumed by the
 * resolver, while discovery treats the rest of the object as an opaque
 * traversable document. The index signature keeps it permissive so arbitrary
 * cross-wms config objects can be passed in.
 */
