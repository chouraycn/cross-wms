/**
 * Port of openclaw/src/secrets/runtime-state.ts
 *
 * Holds active secrets runtime snapshots, refresh context, and cleanup hooks.
 *
 * Import path adjustments:
 * - ../agents/auth-profiles/runtime-snapshots.js → ../agents/runtime-snapshots.js
 *   (cross-wms locates these stubs in agents/runtime-snapshots.ts)
 */
import {
  clearRuntimeAuthProfileStoreSnapshots,
  getRuntimeAuthProfileStoreSnapshot,
  replaceRuntimeAuthProfileStoreSnapshots,
} from "../agents/runtime-snapshots.js";
...
export function getLiveSecretsRuntimeAuthStores(): PreparedSecretsRuntimeSnapshot["authStores"] {
  ...
  return activeSnapshot.authStores.map((entry) => ({
    agentDir: entry.agentDir,
    store: (getRuntimeAuthProfileStoreSnapshot(entry.agentDir) as AuthProfileStore | null) ??
      structuredClone(entry.store),
  }));
}
