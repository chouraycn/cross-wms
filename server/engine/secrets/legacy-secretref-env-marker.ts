/**
 * Simplified port of openclaw/src/secrets/legacy-secretref-env-marker.ts
 *
 * Detects legacy SecretRef env markers in config values.
 *
 * Simplification: discoverConfigSecretTargets from target-registry-query.js is not
 * ported yet. The stub returns an empty array, so candidate collection is a no-op
 * until target-registry-query is ported. Type safety is preserved.
 */
...
/**
 * Stub for discoverConfigSecretTargets from target-registry-query.js (unported).
 * Returns an empty array until target-registry-query is ported.
 */
function discoverConfigSecretTargets(_config: OpenClawConfig): DiscoveredConfigSecretTarget[] {
  return [];
}
