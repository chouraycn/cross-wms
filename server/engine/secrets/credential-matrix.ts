/**
 * Simplified port of openclaw/src/secrets/credential-matrix.ts
 *
 * Generates the documented matrix of user-supplied credential fields that accept SecretRefs.
 * Uses cross-wms's existing target-registry (getSecretTargetRegistry) instead of the
 * unported getSourceSecretTargetRegistry from target-registry-data.js.
 */
import { getSecretTargetRegistry } from "./target-registry.js";
 ...
 *
 * Simplified: uses cross-wms getSecretTargetRegistry() instead of openclaw's
 * getSourceSecretTargetRegistry(). Entries are cast to SecretTargetRegistryEntry
 * to access optional fields (authProfileType) that cross-wms SecretTarget lacks.
 */
export function buildSecretRefCredentialMatrix(): SecretRefCredentialMatrixDocument {
  ...
  for (const entry of getSecretTargetRegistry() as SecretTargetRegistryEntry[]) {
