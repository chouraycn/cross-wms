...
import type { OpenClawConfig } from "./config-secret-types.js";
import type {
  ...
  SecretRef,
  SecretRefSource,
} from "./config-secret-types.js";
import { isValidEnvSecretRefId } from "./config-secret-types.js";
...
type ResolveSecretRefOptions = {
  ...
  /** Plugin manifest registry — accepted for backward compat but ignored (plugin integration not supported in cross-wms). */
...
/** Type guard for plugin-integration exec provider configs (skipped in cross-wms). */
function isPluginIntegrationSecretProviderConfig(
  value: unknown,
): value is PluginIntegrationSecretProviderConfig {
  return (
    typeof value === "object" &&
    value !== null &&
    (value as { source?: unknown }).source === "exec" &&
...
function resolveConfiguredProvider(params: {
  ...
  if (isPluginIntegrationSecretProviderConfig(providerConfig)) {
    // Plugin integration exec providers are not supported in cross-wms.
    throw providerResolutionError({
      source: ref.source,
      provider: ref.provider,
      message: `Secret provider "${ref.provider}" uses plugin integration which is not supported in cross-wms.`,
    });
  }
