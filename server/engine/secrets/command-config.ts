/**
 * Simplified port of openclaw/src/secrets/command-config.ts
 *
 * Collects and analyzes command-scoped secret assignments from OpenClaw config.
 *
 * Simplification: discoverConfigSecretTargetsByIds from target-registry-query.js is
 * not ported yet. The stub returns an empty array, so analysis returns empty results
...
/**
 * Stub for discoverConfigSecretTargetsByIds from target-registry-query.js (unported).
 * Returns an empty array until target-registry-query is ported.
 */
function discoverConfigSecretTargetsByIds(
  _config: OpenClawConfig,
  _targetIds: ReadonlySet<string>,
