...
function shouldHideHeartbeatChatOutput(runId: string, sourceRunId?: string): boolean {
  ...
  try {
    const cfg = getRuntimeConfig() as any;
  const visibility = (resolveHeartbeatVisibility as any)({ cfg, channel: "webchat" });
...
export function createAgentEventHandler({
  ...
  const resolveSpawnedBy = (sessionKey: string): string | null => {
    ...
    try {
      result = (loadGatewaySessionRow as any)(sessionKey)?.spawnedBy ?? null;
    } catch {
      // result stays null
    }
  ...
  const buildSessionEventSnapshot = (
    ...
    const row = (loadGatewaySessionRowForSnapshot as any)(sessionKey, agentId ? { agentId } : undefined);
  ...
  const resolveSessionDeliveryKey = (sessionKey: string, agentId?: string) => {
    ...
    const scopedAgentId = agentId ?? resolveDefaultAgentId(getRuntimeConfig() as any);
  ...
  const resolveNodeSessionDeliveryKeys = (sessionKey: string, agentId?: string) => {
    ...
    const defaultAgentId = resolveDefaultAgentId(getRuntimeConfig() as any);
  ...
  const finalizeLifecycleEvent = (evt: AgentEventPayload, opts?: TerminalLifecycleOptions) => {
    ...
    if (sessionKey) {
      ...
      if (!suppressRestartRecoveryProjection) {
        const persistence = (persistGatewaySessionLifecycleEvent as any)({
          sessionKey,
  ...
  const resolveToolVerboseLevel = (runId: string, sessionKey?: string) => {
    ...
    try {
      const { cfg, entry } = loadSessionEntry(sessionKey) as any;
  ...
  return (evt: AgentEventPayload) => {
    ...
    if (sessionKey && lifecyclePhase === "start") {
      void (persistGatewaySessionLifecycleEvent as any)({
        sessionKey,
        agentId: sessionAgentId,
        event: evt,
      }).catch(() => undefined);
