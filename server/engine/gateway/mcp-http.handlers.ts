    ...
    case "tools/call": {
      ...
      try {
        ...
        if (hookResult.blocked) {
          return jsonRpcResult(id, {
            content: [{ type: "text", text: (hookResult as { reason?: string }).reason ?? "blocked" }],
            isError: true,
          });
        }
        ...
        const result = await (tool as unknown as { execute: (id: string, params: unknown, signal?: AbortSignal) => Promise<unknown> }).execute(toolCallId, hookResult.params, params.signal);
