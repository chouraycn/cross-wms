...
import {
  resolveMemoryHostBackendConfig,
...
function shouldKeepQmdStartupManagerAlive(qmd: ResolvedQmdConfig): boolean {
  return (qmd.update.intervalMs ?? 0) > 0 || (qmd.update.embedIntervalMs ?? 0) > 0;
}
...
export async function startGatewayMemoryBackend(params: {
  ...
  for (const agentId of agentIds) {
    ...
    const resolved = resolveMemoryHostBackendConfig({ cfg: params.cfg, agentId } as any) as { backend?: string; qmd?: ResolvedQmdConfig } | undefined;
    if (!resolved) {
      continue;
    }
