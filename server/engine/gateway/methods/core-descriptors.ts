// Core gateway method descriptor 将 handler 名、auth scope、启动可用性与写策略集中在一个表中。
// 移植自 openclaw/src/gateway/methods/core-descriptors.ts。
// 依赖调整：../operator-scopes.js、./descriptor.js（均已移植）。
...
// 这是规范的 core method 策略表：每个 core handler 都必须出现在这里，
// 以使列表、鉴权、启动可用性与写节流保持同步。
...
/** 查找仅限 operator 的 core 方法 scope，排除 node 与 dynamic 方法。 */
...
/** 对保留给已认证 node 客户端的 core 方法返回 true。 */
...
/** 对所需 operator scope 由 handler 解析的 core 方法返回 true。 */
...
export function createCoreGatewayMethodDescriptors(
  ...
  for (const name of Object.keys(handlers)) {
    if (!specNames.has(name)) {
      // 未分类的 core handler 会绕过 scope/startup/write 元数据，因此在分发器能暴露
      // 一个缺少策略的方法之前失败。
