...
//    降级为始终返回 undefined（插件运行时未移植）。
//  - `./device-metadata-normalization.js` 的 `normalizeDeviceMetadataForPolicy`
//    内联降级实现（去除首尾空白、转小写）。
//  - `./node-registry.js` 的 `NodeSession` 类型降级为本地宽松占位。
import { normalizeOptionalLowercaseString } from "../infra/string-coerce.js";
import { normalizeUniqueStringEntries } from "../infra/string-normalization.js";
import {
 ...
 * Node 会话宽松占位类型（降级）。
 *
 * 降级原因：openclaw `./node-registry.js` 的 NodeSession 依赖完整的节点连接
 * 状态、能力握手与审批记录。这里仅描述命令策略所需字段。
 */
...
type RuntimePluginRegistry = {
  nodeHostCommands?: ReadonlyArray<{
    command: { command: string; dangerous?: boolean };
  }>;
  nodeInvokePolicies?: ReadonlyArray<{
    policy: {
      commands: readonly string[];
      dangerous?: boolean;
      defaultPlatforms?: readonly string[];
...
/**
 * 获取活跃运行时插件注册表（降级实现，始终返回 undefined）。
 *
 * 降级原因：openclaw `plugins/active-runtime-registry` 依赖完整的插件注册表
 * 运行时。返回 undefined 使所有插件相关查询安全降级为空列表。
 */
function getActiveRuntimePluginRegistry(): RuntimePluginRegistry | undefined {
  return undefined;
}
 ...
 * 为策略规范化设备元数据字段（降级实现）。
 *
 * 降级原因：openclaw `device-metadata-normalization` 还会处理平台特定别名
 * （如 "macos" → "mac"）。这里仅做基础小写化与空白清理。
 */
function normalizeDeviceMetadataForPolicy(value: string | undefined): string {
...
function resolveNodeCommandAllowlistInternal(
  ...
  const gatewayNodesConfig = (cfg.gateway as { nodes?: { allowCommands?: string[]; denyCommands?: string[] } | undefined } | undefined)?.nodes;
  const extra = gatewayNodesConfig?.allowCommands ?? [];
  const deny = new Set(gatewayNodesConfig?.denyCommands ?? []);
