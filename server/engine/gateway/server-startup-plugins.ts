...
export async function prepareGatewayPluginBootstrap(params: {
  ...
  if (shouldRunStartupMaintenance) {
    ...
    if (!params.minimalTestGateway) {
      const { runStartupSessionMigration } = (await import("./server-startup-session-migration.js")) as any;
      startupTasks.push(
  ...
  const gatewayPluginConfig = (params.minimalTestGateway
    ? params.cfgAtStart
    : (mergeActivationSectionsIntoRuntimeConfig as any)({
  ...
  const defaultAgentId = resolveDefaultAgentId(gatewayPluginConfig) as any;
  const defaultWorkspaceDir = resolveAgentWorkspaceDir(gatewayPluginConfig, defaultAgentId) as any;
