...
} from "@openclaw-src/agents/sessions/session-manager.js";
import { updateSessionStore } from "@openclaw-src/config/sessions.js";
import type {
  SessionCompactionCheckpoint,
  SessionCompactionCheckpointReason,
  SessionEntry,
} from "@openclaw-src/config/sessions.js";
import { isCompactionCheckpointTranscriptFileName } from "@openclaw-src/config/sessions/artifacts.js";
import { readFileRangeAsync } from "@openclaw-src/config/sessions/file-range.js";
import { streamSessionTranscriptLines } from "@openclaw-src/config/sessions/transcript-stream.js";
...
const checkpointsStore = new Map<string, SessionCompactionCheckpoint[]>();

export function getCompactionCheckpointCount(sessionKey: string): number {
  return checkpointsStore.get(sessionKey)?.length ?? 0;
}

export function getLatestCompactionCheckpoint(sessionKey: string): SessionCompactionCheckpoint | undefined {
  const sessionCheckpoints = checkpointsStore.get(sessionKey);
