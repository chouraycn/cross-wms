...
export async function invokeGatewayTool(params: {
  ...
  const resolveTools = (disablePluginTools: boolean) =>
    ...
    } as any) as { tools: typeof tools; total: number; filtered: number; agentId?: string };
  ...
  try {
    const gatewayTool: AnyAgentTool = tool as unknown as AnyAgentTool;
    ...
    if (hookResult.blocked) {
      const blockedResult = hookResult as { blocked: boolean; reason?: string; deniedReason?: string };
      return {
        ok: false,
        status: 403,
        toolName,
        error: {
          ...
          message: blockedResult.reason ?? "tool call blocked",
          requiresApproval: blockedResult.deniedReason === "plugin-approval",
        },
      };
    }
    return {
