...
import { readLocalFileSafely, FsSafeError, readSecureFile } from "../infra/fs-safe.js";
...
async function resolveAssistantMediaAvailability(
  ...
  try {
    ...
    const opened = await readLocalFileSafely({ filePath: localPath });
...
export async function handleControlUiAssistantMediaRequest(
  ...
  let opened: Awaited<ReturnType<typeof readLocalFileSafely>> | null = null;
  let localPath;
  ...
  try {
    ...
    await assertLocalMediaAllowed(localPath, localRoots);
    opened = await readLocalFileSafely({ filePath: localPath });
