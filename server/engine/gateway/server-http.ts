...
async function canRevealReadinessDetails(params: {
  ...
  const authResult = await authorizeHttpGatewayConnect({
    ...
    browserOriginPolicy: resolveHttpBrowserOriginPolicy(params.req) as any,
  });
...
export function createGatewayHttpServer(opts: {
  ...
  const loadGatewayConfig = (opts.getRuntimeConfig ?? getRuntimeConfig) as () => any;
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      if (requestPath === undefined) {
        sendGatewayAuthFailure(res, { rateLimited: false });
        return;
      }
...
export function attachGatewayUpgradeHandler(opts: {
  ...
  httpServer.on("upgrade", (req, socket, head) => {
    void runWithDiagnosticTraceContext(createDiagnosticTraceContext(), async () => {
      const configSnapshot = getRuntimeConfig() as any;
      const trustedProxies = configSnapshot.gateway?.trustedProxies ?? [];
      const allowRealIpFallback = configSnapshot.gateway?.allowRealIpFallback === true;
      const scopedNodeCapability = normalizePluginNodeCapabilityScopedUrl(req.url ?? "/");
      if (scopedNodeCapability.malformedScopedPath) {
