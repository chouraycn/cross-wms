...
function listReloadRules(): ReloadRule[] {
  ...
  const channelReloadRules: ReloadRule[] = listChannelPlugins().flatMap((plugin) =>
    ((plugin as any).reload?.configPrefixes ?? [])
      .map(
        (prefix): ReloadRule => ({
          prefix,
          kind: "hot",
      ...
      .concat(
        ((plugin as any).reload?.noopPrefixes ?? []).map(
          (prefix): ReloadRule => ({
            prefix,
            kind: "none",
          }),
  ...
  const pluginReloadRules: ReloadRule[] = (registry?.reloads ?? []).flatMap((entry) =>
    ((entry as any).registration?.restartPrefixes ?? [])
      .map(
        (prefix): ReloadRule => ({
          prefix,
          kind: "restart",
      ...
      .concat(
        ((entry as any).registration?.hotPrefixes ?? []).map(
          (prefix): ReloadRule => ({
            prefix,
            kind: "hot",
          }),
        ),
        ((entry as any).registration?.noopPrefixes ?? []).map(
          (prefix): ReloadRule => ({
            prefix,
            kind: "none",
          }),
