...
export function startGatewayConfigReloader(opts: {
  ...
  const handleInvalidSnapshot = (snapshot: ConfigFileSnapshot): boolean => {
    ...
    const issues = (formatConfigIssueLines as any)(snapshot.issues, "").join(", ");
  ...
  const applySnapshot = async (
    ...
    const previousPluginInstallConfig = asPluginInstallConfig(currentPluginInstallRecords as any) as any;
    const nextPluginInstallConfig = asPluginInstallConfig(nextPluginInstallRecords as any) as any;
    ...
    const followUp = resolveConfigWriteFollowUp(afterWrite) as any;
  ...
  const unsubscribeFromWrites =
    opts.subscribeToWrites?.((event: any) => {
  ...
  const createWatcher = () => {
    ...
    watcher = next;
    watcherUsesPolling = next.options.usePolling ?? false;
