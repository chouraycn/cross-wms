...
import type { ModelCatalogCost } from "@cdf-know/model-catalog-core/model-catalog-types";
...
function toCachedModelPricing(
  ...
  const input = parseNumberString((value as any).input) ?? 0;
  const output = parseNumberString((value as any).output) ?? 0;
  const cacheRead = parseNumberString((value as any).cacheRead) ?? 0;
  const cacheWrite = parseNumberString((value as any).cacheWrite) ?? 0;
  const tieredPricing = Array.isArray((value as any).tieredPricing)
    ? (value as any).tieredPricing
...
function loadManifestPricingContext(
  ...
  for (const row of planManifestModelCatalogRows({ registry: registry as any }).rows) {
    const pricing = toCachedModelPricing((row as any).cost);
    if (pricing) {
...
function addConfiguredWebSearchPluginModels(params: {
  ...
  for (const pluginId of params.manifestRegistry.plugins
    ...
    addResolvedModelRef({
      raw: (resolvePluginWebSearchConfig(params.config, pluginId) as any)?.model as string | undefined,
      aliasIndex: params.aliasIndex,
      refs: params.refs,
      allowManifestNormalization: params.allowManifestNormalization,
      allowPluginNormalization: params.allowPluginNormalization,
...
function hasPrivateOrLoopbackConfiguredEndpoint(
  ...
  const providerConfig = config.models?.providers?.[ref.provider];
  const model = findConfiguredProviderModel(config, ref, options) as any;
  return (
