 ...
 */
function authorizeTrustedProxyInternal(params: {
  req?: IncomingMessage;
  trustedProxies?: string[];
...
function authorizeTokenAuthInternal(params: {
  authToken?: string;
  connectToken?: string;
  limiter?: AuthRateLimiter;
  ip?: string;
...
function authorizePasswordAuthInternal(params: {
  authPassword?: string;
...
export function authorizeTokenAuth(
  connectToken?: string,
  authToken?: string,
): GatewayAuthResult {
  if (!authToken) {
  ...
  if (connectToken.length !== authToken.length) {
    return { ok: false, reason: "invalid token" };
  }
  if (!safeEqualSecret(connectToken, authToken)) {
    return { ok: false, reason: "invalid token" };
  }
  return { ok: true, method: "token" };
}

export function authorizePasswordAuth(
  connectPassword?: string,
  authPassword?: string,
): GatewayAuthResult {
  if (!authPassword) {
  ...
  if (!connectPassword) {
    return { ok: false, reason: "no password provided" };
  }
  if (!safeEqualSecret(connectPassword, authPassword)) {
...
async function authorizeGatewayConnectCore(
  ...
  if (auth.mode === "trusted-proxy") {
    ...
    const result = authorizeTrustedProxyInternal({
      req,
      trustedProxies,
    ...
    if (localDirect && auth.password && connectAuth?.password) {
      ...
      return authorizePasswordAuthInternal({
  ...
  if (auth.mode === "token") {
    return authorizeTokenAuthInternal({
...
export function authorizeSkillOperation(
  ...
  const result = checkSkillAccess(skillName, action, userRole);
  if (result.allowed) {
    return { ok: true, method: "token", user: userRole };
  }
  return { ok: false, reason: result.reason };
}
