...
import { parseFiniteNumber } from "../../../infra/parse-finite-number.js";
...
export function createOpenRouterSystemCacheWrapper(
  ...
  return (model, context, options) => {
    ...
    if (
      ...
      return (underlying as any)(model, context, options);
    }
    ...
    return streamWithPayloadPatch(
      ...
      model,
      context,
      stripCacheRetentionOption(options) as any,
      (payloadObj) => {
        (applyAnthropicEphemeralCacheControlMarkers as any)(
          payloadObj,
          (resolveAnthropicEphemeralCacheControl as any)(readStringValue(model.baseUrl), cacheRetention) ??
            null,
        );
      },
    );
