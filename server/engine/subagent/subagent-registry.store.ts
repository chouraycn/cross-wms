/**
 * Subagent Registry Store — 持久化存储层
 *
 * 基于 SQLite 存储子代理状态，提供增删改查功能。
 * 与现有的 db 层集成。
 */
...
export function initSubagentStore(db: Database.Database): void {
  ...
  ensureTable();
  initialized = true;
  logger.debug('[SubagentStore] Initialized');
}
...
export function cleanupOldSubagentInstances(olderThanMs: number): number {
  ...
  if (result.changes > 0) {
    logger.debug(`[SubagentStore] Cleaned up ${result.changes} old subagent instances`);
  }
...
export function clearSubagentStore(): void {
  ...
  logger.debug('[SubagentStore] Cleared all subagent instances');
}
