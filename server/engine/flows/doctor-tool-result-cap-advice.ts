...
} from "../agents/tool-result-truncation.js";
...
export function buildToolResultCapDoctorAdvice(params: ToolResultCapDoctorAdviceParams): string[] {
  ...
  const autoCap = resolveAutoLiveToolResultMaxChars(params.contextWindowTokens) as number;
  ...
  const autoEffectiveCap = calculateMaxToolResultCharsWithCap(
    params.contextWindowTokens,
    autoCap,
  ) as number;
