 ...
 *
 * 通过 skillToolExecutorRegistry 叶子模块调用 executeToolCall，避免 skillContextFactory
 * 与 toolRegistry 之间的循环依赖（skillRegistry 依赖本工厂的 createSkillContext，
 * 而 toolRegistry 依赖 skillRuntimeBridge → skillLifecycle → skillRegistry）。原动态 import()
 * 被 madge 8.0.0 跟踪仍会形成循环 (#11)，改用注册表彻底切断该边。
 */
import { getSkillToolExecutor } from './skillToolExecutorRegistry.js';
...
function createSkillToolRunner(): SkillToolRunner {
  return {
    async run(name: string, args: Record<string, unknown> = {}): Promise<string> {
      ...
      try {
        const executeToolCall = getSkillToolExecutor();
