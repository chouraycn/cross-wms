...
import { DEFAULT_CONTEXT_TOKENS, DEFAULT_MODEL, DEFAULT_PROVIDER } from "@openclaw-src/agents/defaults.js";
import { areRuntimeModelRefsEquivalent } from "@openclaw-src/agents/model-runtime-aliases.js";
import { getRuntimeConfig, projectConfigOntoRuntimeSourceSnapshot } from "@openclaw-src/config/config.js";
import { resolveMainSessionKey } from "@openclaw-src/config/sessions/main-session.js";
import { hasSessionAutoModelFallbackProvenance } from "@openclaw-src/config/sessions/model-override-provenance.js";
import { resolveStorePath } from "@openclaw-src/config/sessions/paths.js";
import { listSessionEntries } from "@openclaw-src/config/sessions/session-accessor.js";
import { resolveSessionTotalTokens, type SessionEntry } from "@openclaw-src/config/sessions/types.js";
import type { OpenClawConfig } from "@openclaw-src/config/types.js";
import { resolveCronJobsStorePath } from "@openclaw-src/cron/store.js";
import { listGatewayAgentsBasic } from "@openclaw-src/gateway/agent-list.js";
import { resolveHeartbeatSummaryForAgent } from "@openclaw-src/infra/heartbeat-summary.js";
import { peekSystemEvents } from "@openclaw-src/infra/system-events.js";
import { parseAgentSessionKey } from "@openclaw-src/routing/session-key.js";
import { createLazyImportLoader } from "@openclaw-src/shared/lazy-promise.js";
import { createLazyRuntimeSurface } from "@openclaw-src/shared/lazy-runtime.js";
import {
  summarizeActionableTaskAuditFindings,
  summarizeRetainedLostTaskAuditFindings,
} from "@openclaw-src/tasks/task-registry.audit.js";
import { resolveRuntimeServiceVersion } from "@openclaw-src/version.js";
import type { HeartbeatStatus, SessionStatus, StatusSummary } from "./status.types.js";

const RECENT_SESSION_LIMIT = 10;

const channelSummaryModuleLoader = createLazyImportLoader(
  () => import("@openclaw-src/infra/channel-summary.js"),
);
const channelPluginIdsModuleLoader = createLazyImportLoader(
  () => import("@openclaw-src/plugins/channel-plugin-ids.js"),
);
...
const taskRegistryMaintenanceModuleLoader = createLazyImportLoader(
  () => import("@openclaw-src/tasks/task-registry.maintenance.js"),
);
const staticModelCatalogResolverLoader = createLazyImportLoader(async () => {
  const modelCatalog = await import("@openclaw-src/agents/embedded-agent-runner/model.static-catalog.js");
  return {
