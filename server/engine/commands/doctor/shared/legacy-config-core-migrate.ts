...
import { migrateLegacySecretRefEnvMarkers } from "@openclaw-src/secrets/legacy-secretref-env-marker.js";
...
function repairNullAgentWorkspaces(cfg: OpenClawConfig, changes: string[]): OpenClawConfig {
  ...
  const nextAgents = agents.map((agent) => {
    if (agent && typeof agent === "object" && (agent as Record<string, any>).workspace === null) {
      repaired += 1;
      const { workspace: _workspace, ...rest } = agent as Record<string, any>;
      return rest;
    }
    return agent;
  });
