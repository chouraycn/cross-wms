...
} from "@openclaw-src/agents/provider-tool-policy.js";
import { pickSandboxToolPolicy } from "@openclaw-src/agents/sandbox-tool-policy.js";
import {
  isToolAllowedByPolicies,
  isToolAllowedByPolicyName,
} from "@openclaw-src/agents/tool-policy-match.js";
import { mergeAlsoAllowPolicy, resolveToolProfilePolicy } from "@openclaw-src/agents/tool-policy.js";
import type { OpenClawConfig } from "@openclaw-src/config/types.openclaw.js";
import type {
  AgentToolsConfig,
  ToolPolicyConfig,
  ToolsConfig,
} from "@openclaw-src/config/types.tools.js";
import { collectChannelRouteTargets } from "@openclaw-src/routing/channel-route-targets.js";
...
async function resolveDoctorChannelPreviewConfig(params: {
  ...
  const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] =
    await Promise.all([
      import("@openclaw-src/cli/command-secret-gateway.js"),
      import("@openclaw-src/cli/command-secret-targets.js"),
    ]);
...
export async function collectDoctorPreviewNotes(params: {
  ...
  if (hasChannelConfig) {
    ...
    if (emptyAllowlistWarnings.length > 0) {
      const { sanitizeForLog } = await import("@openclaw-src/../packages/terminal-core/src/ansi.js");
