...
import { getLoadedChannelPlugin, normalizeChannelId } from "@openclaw-src/channels/plugins/index.js";
import { moveSingleAccountChannelSectionToDefaultAccount } from "@openclaw-src/channels/plugins/setup-helpers.js";
import type { ChannelSetupPlugin } from "@openclaw-src/channels/plugins/setup-wizard-types.js";
import type { ChannelPlugin } from "@openclaw-src/channels/plugins/types.plugin.js";
import type { ChannelId, ChannelSetupInput } from "@openclaw-src/channels/plugins/types.public.js";
import { formatCliCommand } from "@openclaw-src/cli/command-format.js";
import {
  formatUnknownChannelMessage,
  formatUnsupportedChannelActionMessage,
} from "@openclaw-src/cli/error-format.js";
...
import { parseStrictNonNegativeInteger } from "@openclaw-src/infra/parse-finite-number.js";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "@openclaw-src/routing/session-key.js";
import { defaultRuntime, type RuntimeEnv } from "@openclaw-src/runtime.js";
...
import { WizardCancelledError } from "@openclaw-src/wizard/prompts.js";
import { applyAgentBindings, describeBinding } from "@openclaw-src/agents.bindings.js";
import type { ChannelChoice } from "@openclaw-src/onboard-types.js";
import { applyAccountName, applyChannelAccountConfig } from "./add-mutators.js";
import { channelLabel } from "./runtime-label.js";
import { requireValidConfigFileSnapshot, shouldUseWizard } from "./shared.js";

type ChannelSetupPluginInstallModule = typeof import("@openclaw-src/channel-setup/plugin-install.js");
type OnboardChannelsModule = typeof import("@openclaw-src/onboard-channels.js");

const channelSetupPluginInstallLoader = createLazyImportLoader<ChannelSetupPluginInstallModule>(
  () => import("@openclaw-src/channel-setup/plugin-install.js"),
);
const onboardChannelsLoader = createLazyImportLoader<OnboardChannelsModule>(
  () => import("@openclaw-src/onboard-channels.js"),
);
...
async function resolveCatalogChannelEntry(raw: string, cfg: OpenClawConfig | null) {
  ...
  const entries = cfg
    ? await import("@openclaw-src/channel-setup/trusted-catalog.js").then(
    ...
    : await import("@openclaw-src/channels/plugins/catalog.js").then(
...
async function channelsAddCommandImpl(
  ...
  if (useWizard) {
    const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([
      import("@openclaw-src/agents.config.js"),
      loadOnboardChannels(),
    ]);
  ...
  if (catalogEntry) {
    ...
    const { isCatalogChannelInstalled } = await import("@openclaw-src/channel-setup/discovery.js");
