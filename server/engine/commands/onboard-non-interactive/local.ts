...
import { resolveConfiguredSecretInputString } from "@openclaw-src/gateway/resolve-configured-secret-input-string.js";
import type { RuntimeEnv } from "@openclaw-src/runtime.js";
import { DEFAULT_GATEWAY_DAEMON_RUNTIME } from "@openclaw-src/daemon-runtime.js";
import { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } from "@openclaw-src/onboard-config.js";
import {
  applyWizardMetadata,
  DEFAULT_WORKSPACE,
...
} from "@openclaw-src/onboard-helpers.js";
...
> {
  ...
  try {
    ...
    const { resolveGatewayService } = await import("@openclaw-src/daemon/service.js");
  ...
  try {
    const { readLastGatewayErrorLine } = await import("@openclaw-src/daemon/diagnostics.js");
...
export async function runNonInteractiveLocalSetup(params: {
  ...
  if (!opts.skipHealth) {
    const { healthCommand } = await import("@openclaw-src/health.js");
