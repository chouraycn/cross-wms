...
import type { ApiKeyCredential } from "@openclaw-src/agents/auth-profiles/types.js";
...
import { enablePluginInConfig } from "@openclaw-src/plugins/enable.js";
import { resolvePreferredProviderForAuthChoice } from "@openclaw-src/plugins/provider-auth-choice-preference.js";
import { resolveManifestProviderAuthChoice } from "@openclaw-src/plugins/provider-auth-choices.js";
import {
  resolveDeprecatedProviderInstallCatalogEntry,
  resolveProviderInstallCatalogEntry,
} from "@openclaw-src/plugins/provider-install-catalog.js";
import type {
...
} from "@openclaw-src/codex-runtime-plugin-install.js";
import { ensureCopilotRuntimePluginForModelSelection } from "@openclaw-src/copilot-runtime-plugin-install.js";
import { createNonInteractiveLoggingPrompter } from "@openclaw-src/non-interactive-prompter.js";
import type { OnboardOptions } from "@openclaw-src/onboard-types.js";
...
export async function applyNonInteractivePluginProviderChoice(params: {
  ...
  if (!providerChoice) {
    ...
    const { ensureOnboardingPluginInstalled } = await import("@openclaw-src/onboarding-plugin-install.js");
  ...
  if (codexInstall.installed) {
    ...
    const { offerPostInstallMigrations } =
      await import("@openclaw-src/wizard/setup.post-install-migration.js");
