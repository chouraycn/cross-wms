...
vi.mock("@openclaw-src/skills/discovery/status.js", () => ({
  buildWorkspaceSkillStatus: () => ({ skills: [] }),
}));

vi.mock("@openclaw-src/plugins/loader.js", () => ({
...
vi.mock("@openclaw-src/config/config.js", async () => {
  const actual = await vi.importActual<typeof import("@openclaw-src/config/config.js")>("@openclaw-src/config/config.js");
  return {
    ...actual,
...
vi.mock("@openclaw-src/config/io.js", async () => {
  const actual = await vi.importActual<typeof import("@openclaw-src/config/io.js")>("@openclaw-src/config/io.js");
  return {
    ...actual,
    createConfigIO,
    readConfigFileSnapshot,
...
vi.mock("@openclaw-src/daemon/legacy.js", () => ({
  findLegacyGatewayServices,
  uninstallLegacyGatewayServices,
}));

vi.mock("@openclaw-src/daemon/inspect.js", () => ({
  findExtraGatewayServices,
  findSystemGatewayServices,
  renderGatewayServiceCleanupHints,
}));

vi.mock("@openclaw-src/daemon/service-audit.js", () => ({
...
vi.mock("@openclaw-src/daemon/program-args.js", () => ({
...
vi.mock("@openclaw-src/gateway/call.js", async () => {
  const actual = await vi.importActual<typeof import("@openclaw-src/gateway/call.js")>("@openclaw-src/gateway/call.js");
  return {
    ...actual,
    callGateway,
  };
...
vi.mock("@openclaw-src/process/exec.js", () => ({
  runExec,
...
vi.mock("@openclaw-src/infra/openclaw-root.js", () => ({
  resolveOpenClawPackageRoot,
  resolveOpenClawPackageRootSync: vi.fn(() => "/tmp/openclaw"),
}));

vi.mock("@openclaw-src/infra/update-runner.js", () => ({
  runGatewayUpdate,
}));

vi.mock("@openclaw-src/flows/doctor-health-contributions.js", () => ({
...
vi.mock("@openclaw-src/channels/plugins/doctor-contract-api.js", () => ({
  loadBundledChannelDoctorContractApi: vi.fn(() => undefined),
}));

vi.mock("@openclaw-src/channels/plugins/bootstrap-registry.js", () => ({
  getBootstrapChannelPlugin: vi.fn(() => undefined),
}));

vi.mock("@openclaw-src/agents/auth-profiles.js", async () => {
  const actual = await vi.importActual<typeof import("@openclaw-src/agents/auth-profiles.js")>(
    "@openclaw-src/agents/auth-profiles.js",
  );
  return {
    ...actual,
...
vi.mock("@openclaw-src/agents/auth-profiles/store.js", async () => {
  const actual = await vi.importActual<typeof import("@openclaw-src/agents/auth-profiles/store.js")>(
    "@openclaw-src/agents/auth-profiles/store.js",
  );
  return {
    ...actual,
    ensureAuthProfileStore,
...
vi.mock("@openclaw-src/daemon/service.js", () => ({
...
vi.mock("@openclaw-src/pairing/pairing-store.js", () => ({
  readChannelAllowFromStore: vi.fn().mockResolvedValue([]),
  upsertChannelPairingRequest: vi.fn().mockResolvedValue({ code: "000000", created: false }),
}));

vi.mock("@openclaw-src/runtime.js", () => ({
  defaultRuntime: {
    log: () => {},
    error: () => {},
    exit: () => {
...
vi.mock("@openclaw-src/utils.js", async () => {
  const actual = await vi.importActual<typeof import("@openclaw-src/utils.js")>("@openclaw-src/utils.js");
  return {
    ...actual,
...
vi.mock("@openclaw-src/channels/plugins/lifecycle-startup.js", () => ({
