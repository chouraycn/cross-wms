...
} from "@openclaw-src/agents/agent-scope.js";
import {
  buildAuthHealthSummary,
  DEFAULT_OAUTH_WARN_MS,
  formatRemainingShort,
} from "@openclaw-src/agents/auth-health.js";
import { evaluateStoredCredentialEligibility } from "@openclaw-src/agents/auth-profiles/credential-state.js";
import {
  resolveAuthProfileEligibility,
  resolveAuthProfileOrder,
} from "@openclaw-src/agents/auth-profiles/order.js";
import { resolveAuthStorePathForDisplay } from "@openclaw-src/agents/auth-profiles/paths.js";
import { ensureAuthProfileStoreWithoutExternalProfiles as ensureAuthProfileStore } from "@openclaw-src/agents/auth-profiles/store.js";
import type { AuthProfileCredential } from "@openclaw-src/agents/auth-profiles/types.js";
import { resolveProfileUnusableUntilForDisplay } from "@openclaw-src/agents/auth-profiles/usage.js";
import {
  listProviderEnvAuthLookupKeys,
  resolveProviderEnvAuthLookupMaps,
} from "@openclaw-src/agents/model-auth-env-vars.js";
import { resolveEnvApiKey, resolveUsableCustomProviderApiKey } from "@openclaw-src/agents/model-auth.js";
import { resolveCliRuntimeExecutionProvider } from "@openclaw-src/agents/model-runtime-aliases.js";
import {
...
} from "@openclaw-src/agents/model-selection.js";
import {
  OPENAI_CODEX_PROVIDER_ID,
  OPENAI_PROVIDER_ID,
  openAIProviderUsesCodexRuntimeByDefault,
} from "@openclaw-src/agents/openai-routing.js";
import { resolveProviderIdForAuth } from "@openclaw-src/agents/provider-auth-aliases.js";
import { resolveDefaultAgentWorkspaceDir } from "@openclaw-src/agents/workspace.js";
import { createConfigIO } from "@openclaw-src/config/config.js";
import {
  resolveAgentModelFallbackValues,
  resolveAgentModelPrimaryValue,
} from "@openclaw-src/config/model-input.js";
import {
  parseStrictFiniteNumber,
  parseStrictPositiveInteger,
} from "@openclaw-src/infra/parse-finite-number.js";
import { getShellEnvAppliedKeys, shouldEnableShellEnvFallback } from "@openclaw-src/infra/shell-env.js";
import {
  captureCurrentPluginMetadataSnapshotState,
  getCurrentPluginMetadataSnapshot,
  restoreCurrentPluginMetadataSnapshotState,
  setCurrentPluginMetadataSnapshot,
} from "@openclaw-src/plugins/current-plugin-metadata-snapshot.js";
import { loadManifestMetadataSnapshot } from "@openclaw-src/plugins/manifest-contract-eligibility.js";
import type { PluginMetadataSnapshot } from "@openclaw-src/plugins/plugin-metadata-snapshot.types.js";
import type { ProviderSyntheticAuthResult } from "@openclaw-src/plugins/provider-external-auth.types.js";
import { resolveProviderSyntheticAuthWithPlugin } from "@openclaw-src/plugins/provider-runtime.js";
import { resolveRuntimeSyntheticAuthProviderRefs } from "@openclaw-src/plugins/synthetic-auth.runtime.js";
...
import { resolveUserPath, shortenHomePath } from "@openclaw-src/utils.js";
...
type ProviderUsageRuntime = typeof import("@openclaw-src/infra/provider-usage.js");
type ProgressRuntime = typeof import("@openclaw-src/cli/progress.js");
...
const providerUsageRuntimeLoader = createLazyImportLoader<ProviderUsageRuntime>(
  () => import("@openclaw-src/infra/provider-usage.js"),
);
  ...
  try {
    ...
    if (missingProvidersInUse.length > 0) {
      const { buildProviderAuthRecoveryHint } =
        await import("@openclaw-src/agents/provider-auth-recovery-hint.js");
      runtime.log("");
