...
} from "@openclaw-src/agents/auth-profiles/profiles.js";
import { loadAuthProfileStoreForRuntime } from "@openclaw-src/agents/auth-profiles/store.js";
import type { AuthProfileCredential } from "@openclaw-src/agents/auth-profiles/types.js";
import { clearAuthProfileCooldown } from "@openclaw-src/agents/auth-profiles/usage.js";
import { normalizeProviderId } from "@openclaw-src/agents/model-selection-normalize.js";
...
import { parseDurationMs } from "@openclaw-src/cli/parse-duration.js";
import { logConfigUpdated } from "@openclaw-src/config/logging.js";
import { normalizeAgentModelRefForConfig } from "@openclaw-src/config/model-input.js";
...
} from "@openclaw-src/plugins/provider-auth-choice-helpers.js";
import { applyAuthProfileConfig } from "@openclaw-src/plugins/provider-auth-helpers.js";
import { createVpsAwareOAuthHandlers } from "@openclaw-src/plugins/provider-oauth-flow.js";
import { resolvePluginProviders } from "@openclaw-src/plugins/providers.runtime.js";
import {
  resolvePluginSetupProvider,
  resolvePluginSetupRegistry,
} from "@openclaw-src/plugins/setup-registry.js";
import type {
  ProviderAuthMethod,
  ProviderAuthResult,
  ProviderPlugin,
} from "@openclaw-src/plugins/types.js";
import type { RuntimeEnv } from "@openclaw-src/runtime.js";
import { normalizeSecretInput } from "@openclaw-src/utils/normalize-secret-input.js";
import { createClackPrompter } from "@openclaw-src/wizard/clack-prompter.js";
import { validateAnthropicSetupToken } from "@openclaw-src/auth-token.js";
...
import { isRemoteEnvironment } from "@openclaw-src/infra/remote-env.js";
...
async function runProviderAuthMethod(params: {
  ...
  const result = await params.method.run({
    ...
    openUrl: async (url) => {
      const { openUrl } = await import("@openclaw-src/onboard-helpers.js");
      await openUrl(url);
    },
    oauth: {
