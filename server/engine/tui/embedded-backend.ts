...
export class EmbeddedTuiBackend implements TuiBackend {
  ...
  async runGoalCommand(opts: Parameters<NonNullable<TuiBackend["runGoalCommand"]>>[0]) {
    ...
    switch (parsed.action) {
      case "status": {
        const snapshot = await SessionGoal({ sessionKey, storePath });
        return { text: SessionGoalStatus(snapshot.goal) };
      }
      case "start":
      case "set":
      case "create": {
        ...
        const goal = await createSessionHash({
          sessionKey,
          storePath,
          objective,
          fallbackEntry,
      ...
      case "pause": {
        const goal = await SessionGoalStatus({
          sessionKey,
          storePath,
          status: "paused",
      ...
      case "clear": {
        const removed = await SessionGoal({ sessionKey, storePath });
