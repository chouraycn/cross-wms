...
/**
 * Local type adapters for cross-wms stub signatures.
 *
 * The cross-wms versions of these infra helpers are simplified stubs with
 * different return types. We mirror the openclaw runtime shapes here so the
 * approval plan logic stays type-safe without modifying shared infra files.
 */
type DispatchUnwrap = { kind: "unwrapped"; argv: string[] } | { kind: "noop" };
type ShellMultiplexerUnwrap = {
  kind: "unwrapped";
  argv: string[];
  wrapper: string;
} | { kind: "noop" };
...
/** Synchronous file identity comparison using fs.Stats (matches openclaw semantics). */
function sameFileIdentitySync(left: fs.Stats, right: fs.Stats): boolean {
  return left.dev === right.dev && left.ino === right.ino;
}

function asDispatchUnwrap(value: string[] | null): DispatchUnwrap {
  if (value === null) {
    return { kind: "noop" };
  }
  return { kind: "unwrapped", argv: value };
}

function asShellMultiplexerUnwrap(
  value: string[] | null,
  wrapper: string,
  ...
  return { kind: "unwrapped", argv: value, wrapper };
}
...
function unwrapArgvForMutableOperand(argv: string[]): {
  ...
  while (true) {
    const dispatchUnwrap = asDispatchUnwrap(unwrapKnownDispatchWrapperInvocation(current));
...
function resolveCanonicalApprovalCwdSync(cwd: string):
  ...
  if (
    !sameFileIdentitySync(cwdStat, cwdLstat) ||
    !sameFileIdentitySync(cwdStat, cwdRealStat) ||
    !sameFileIdentitySync(cwdLstat, cwdRealStat)
  ) {
    return {
      ok: false,
...
export function revalidateApprovedCwdSnapshot(params: { snapshot: ApprovedCwdSnapshot }): boolean {
  ...
  return sameFileIdentitySync(params.snapshot.stat, current.snapshot.stat);
}
...
export function buildSystemRunApprovalPlan(params: {
  ...
  }) as SystemRunCommandRequest;
  if (!command.ok) {
    return { ok: false, message: command.message ?? "command resolution failed" };
  }
  ...
  const commandText = formatExecCommand(hardening.argv) as string;
