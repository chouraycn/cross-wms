...
/**
 * Local shape of a registered node-host plugin command.
 *
 * The cross-wms `OpenClawPluginNodeHostCommand` alias is currently typed as
 * `unknown`; we mirror the openclaw runtime shape here so dispatch logic stays
 * type-safe without modifying the shared plugin registry types.
 */
type NodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
let pluginRegistryLoaderModulePromise:
  | Promise<typeof import("../plugins/runtime-registry-loader.js")>
  | undefined;

async function loadPluginRegistryLoaderModule() {
  pluginRegistryLoaderModulePromise ??= import("../plugins/runtime-registry-loader.js");
...
export function listRegisteredNodeHostCapsAndCommands(): {
  ...
  for (const entry of (registry?.nodeHostCommands ?? []) as NodeHostCommandEntry[]) {
...
export async function invokeRegisteredNodeHostCommand(
  ...
  const registry = getActivePluginRegistry();
  const match = ((registry?.nodeHostCommands ?? []) as NodeHostCommandEntry[]).find(
