...
export async function evaluateSystemRunAllowlist(params: {
  ...
  safeBins: Set<string>;
  safeBinProfiles: Readonly<Record<string, SafeBinProfile>> | undefined;
  trustedSafeBinDirs: ReadonlySet<string> | undefined;
  ...
  const analysis = analyzeArgvCommand({ argv: params.argv, cwd: params.cwd, env: params.env }) as ExecCommandAnalysis;
...
export function resolvePlannedAllowlistArgv(params: {
  ...
  const plannedAllowlistArgv = resolvePlannedSegmentArgv(params.segments[0]) as string[] | undefined | null;
