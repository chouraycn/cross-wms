 ...
 *
 * 移植自 openclaw/src/plugin-sdk/facade-activation-check.runtime.ts
 * 降级策略：
 *  - ../config/types.js 未移植，改从 ../config/types.openclaw.js 导入 OpenClawConfig。
 *  - cross-wms 的 getRuntimeConfigSnapshot/getRuntimeConfigSourceSnapshot 返回 unknown
 *    占位，这里按 OpenClawConfig 断言。
 *  - cross-wms 的 configMayNeedPluginAutoEnable 返回 unknown，作为 truthiness 判别使用。
 *  - 其余依赖（config/paths、config/plugin-auto-enable.*、config/runtime-snapshot、
 *    plugins/bundled-dir、plugins/config-state、plugins/current-plugin-metadata-snapshot、
 *    plugins/default-enablement、plugins/manifest-registry、utils/parse-json-compat、
 *    ./facade-resolution-shared）均已在 cross-wms 中存在。
 */
...
function readFacadeBoundaryConfigSafely(): {
  ...
  try {
    ...
    if (sourceSnapshot) {
      ...
      return { rawConfig: sourceSnapshot as OpenClawConfig };
    }
