...
import type { PendingApprovalView, ApprovalKind, ApprovalRequest } from "./approval-types.js";
...
function listDecisionActions(actions: PendingApprovalView["actions"]): ExecApprovalReplyDecision[] {
  return normalizeDecisionList(
    actions.flatMap((action) =>
      "decision" in action && action.decision
        ? [action.decision as ExecApprovalReplyDecision]
        : [],
    ),
  );
}
function buildApprovalReactionPromptText(params: {
  ...
  if (view.approvalKind === "exec") {
    ...
    info.push(
      `Expires in: ${view.expiresAtMs ? formatExecApprovalExpiresIn(view.expiresAtMs, params.nowMs) : "never"}`,
    );
    ...
    sections.push(header.join("\n"));
    const details = [`Title: ${view.title ?? "Unknown"}`];
    if (view.description) {
      details.push(`Description: ${view.description}`);
    }
    details.push(`Severity: ${formatSeverity(view.severity as "info" | "warning" | "critical")}`);
    if (view.toolName) {
...
export function buildApprovalReactionPromptPayloadForRequest(params: {
  ...
  return buildApprovalPendingPromptPayload({
    request: params.request,
    view: buildPendingApprovalView(params.request) as PendingApprovalView,
    nowMs: params.nowMs,
  });
}
    ...
    params.view.approvalKind === "plugin"
      ? (() => {
          ...
          return withoutPresentation({
            ...
            text: replaceApprovalIdPlaceholder((payload as ReplyPayload).text, params.request.id),
          });
        })()
      : withoutPresentation(
          buildExecApprovalPendingReplyPayload({
            ...
            nowMs: params.nowMs,
          } satisfies ExecApprovalPendingReplyParams) as ReplyPayload,
        );
