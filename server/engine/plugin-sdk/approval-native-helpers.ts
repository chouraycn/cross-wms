...
type DeliverySuppressionInput = {
  cfg: OpenClawConfig;
  approvalKind: ApprovalKind;
  target: ChannelApprovalForwardTarget;
  request: ApprovalRequest;
};
type NativeApprovalForwardTarget = ChannelApprovalForwardTarget;
...
function isSessionApprovalEligibleViaForwarding(
  ...
  if (
    !(doesApprovalRequestMatchChannelAccount as unknown as (params: {
      cfg: OpenClawConfig;
      request: ApprovalRequest;
      channel: string;
      accountId?: string | null;
...
export function createNativeApprovalChannelRouteGates<TTarget extends NativeApprovalTarget>(
  ...
  const hasChannelOriginOrSessionTarget = (input: {
    ...
    }) as { channel?: string; accountId?: string | null } | null;
    return (
...
function createOriginTargetResolver<TTarget>(
  ...
  return (input: ApprovalResolverParams): TTarget | null => {
    ...
    return resolveApprovalRequestOriginTarget({
      ...
      channel: params.channel,
      accountId: input.accountId,
      resolveTurnSourceTarget: (request: ApprovalRequest) =>
      ...
      resolveFallbackTarget: params.resolveFallbackTarget
        ? (request: ApprovalRequest) => normalizeTarget(params.resolveFallbackTarget?.(request) ?? null)
        : undefined,
    }) as TTarget | null;
  };
}
