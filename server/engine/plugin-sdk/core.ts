...
export function buildThreadAwareOutboundSessionRoute(params: {
  ...
  normalizeThreadId?: (threadId: string | number | null | undefined) => string | undefined;
...
export function defineChannelPluginEntry<TPlugin>({
  ...
  const resolvedConfigSchema: ChannelEntryConfigSchema<TPlugin> =
    typeof configSchema === "function"
      ? (configSchema as () => ChannelEntryConfigSchema<TPlugin>)()
  ...
  const entry = {
    ...
    register(api: OpenClawPluginApi) {
      ...
      setRuntime?.(api.runtime as PluginRuntime);
  ...
  return {
    resolveDmPolicy: ({ cfg, accountId, account }: { cfg: unknown; accountId: string; account: TResolvedAccount }) =>
      buildAccountScopedDmSecurityPolicy({
        ...
        accountId,
        fallbackAccountId: dm.resolveFallbackAccountId?.(account) ?? account.accountId ?? undefined,
        policy: dm.resolvePolicy?.(account) ?? undefined,
        allowFrom: dm.resolveAllowFrom?.(account) ?? [],
        defaultPolicy: dm.defaultPolicy,
        allowFromPathSuffix: dm.allowFromPathSuffix,
        policyPathSuffix: dm.policyPathSuffix,
        approveChannelId: dm.approveChannelId,
        approveHint: dm.approveHint,
        normalizeEntry: dm.normalizeEntry
          ? (raw: string) => dm.normalizeEntry!(raw)
          : undefined,
        inheritSharedDefaultsFromDefaultAccount:
          dm.inheritSharedDefaultsFromDefaultAccount,
      }),
...
function resolveChatChannelPairing(
  ...
  return createInlineTextPairingAdapter(
    (pairing as ChatChannelPairingOptions).text,
  );
}
...
function resolveChatChannelThreading<TResolvedAccount>(
  ...
  const threadingOptions = threading as ChatChannelThreadingOptions<TResolvedAccount>;
  let resolveReplyToMode: ChannelThreadingAdapter["resolveReplyToMode"];
  if ("topLevelReplyToMode" in threading) {
    resolveReplyToMode = createTopLevelChannelReplyToModeResolver(
      threadingOptions.topLevelReplyToMode as string,
    );
  } else {
...
function resolveChatChannelOutbound(
  ...
  const outboundOptions = outbound as ChatChannelAttachedOutboundOptions;
  return {
    ...outboundOptions.base,
    ...createInlineAttachedChannelResultAdapter(outboundOptions.attachedResults),
  };
}
...
>(params: {
  ...
  } as unknown as ChannelPlugin<TResolvedAccount, Probe, Audit>;
}
