...
export type MigrationItem = {
  id: string;
  kind: "config" | "secret" | "file" | "manual" | string;
  action: "merge" | "set" | "delete" | "manual" | string;
  source?: string;
  target: string;
  status: "planned" | "migrated" | "skipped" | "conflict" | "error";
  reason?: string;
  message: string;
  sensitive?: boolean;
...
export type MigrationProviderPlugin = {
  id: string;
  detect: (ctx: MigrationProviderContext) => Promise<MigrationDetection>;
  apply: (
...
export function createMigrationItem(
  params: Partial<MigrationItem> & Pick<MigrationItem, "id" | "target" | "message">,
): MigrationItem {
  return {
    kind: "config",
    action: "merge",
