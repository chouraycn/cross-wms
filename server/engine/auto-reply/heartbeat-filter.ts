 ...
 *
 * Ported from openclaw/src/auto-reply/heartbeat-filter.ts. The OpenClaw
 * normalization-core helpers (`isRecord`, `normalizeOptionalString`,
 * `uniqueStrings`) and the `MESSAGE_TOOL_DELIVERY_HINTS` constant are inlined
 * so the module stays self-contained within cross-wms server/engine/auto-reply.
 */
...
/** Delivery hint lines injected before message-tool-only runs. */
...
function isEmbeddedToolResultOnlyContent(content: unknown): boolean {
  return (
    ...
    content.length > 0 &&
    content.every(
      (block) => isRecord(block) && TOOL_RESULT_BLOCK_TYPES.has(normalizeOptionalString(block.type) ?? ''),
    )
  );
}
...
function isFailedToolResultRecord(record: Record<string, unknown>): boolean {
  return (
    ...
    normalizeOptionalString(record.type) === 'tool_result_error'
  );
}
...
export function isHeartbeatUserMessage(
  ...
  if (trimmed === HEARTBEAT_TRANSCRIPT_PROMPT) return true;
  if (
...
function hasCompletedVisibleHeartbeatResponseToolCall(
  ...
  for (
    ...
    for (const resultId of collectSuccessfulToolResultCallIds(result)) {
      if (callIds.has(resultId)) return true;
    }
  }
  return false;
}
...
function resolveHeartbeatArtifactSpanEnd(
  ...
  while (index < messages.length) {
    ...
    if (hasVisibleHeartbeatResponseToolCall(message)) {
      if (hasCompletedVisibleHeartbeatResponseToolCall(messages, index)) return undefined;
      index++;
      continue;
    }
  ...
  if (sawNonTerminalAssistantOutput && !sawTerminalHeartbeatArtifact) return undefined;
  return index;
}
