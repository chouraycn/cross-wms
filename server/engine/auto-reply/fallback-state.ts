...
// cross-wms does not yet port reply/agent-runner-execution.ts. The runtime
// fallback attempt shape is narrow and only read here, so a local minimal stub
// keeps this module self-contained while preserving the public function surface.
/** Minimal fallback attempt record used by buildFallbackNotice helpers. */
...
export function buildFallbackNotice(params: {
  ...
  // cross-wms stub for areRuntimeModelRefsEquivalent does not read config; the
  // options shape is preserved for forward-compat but currently unused.
  void params.cfg;
