...
export function isCommandEnabled(cfg: OpenClawConfig, commandKey: string): boolean {
  if (commandKey === "config") {
    return isCommandFlagEnabled(cfg, "config") as boolean;
  }
  if (commandKey === "mcp") {
    return isCommandFlagEnabled(cfg, "mcp") as boolean;
  }
  if (commandKey === "plugins") {
    return isCommandFlagEnabled(cfg, "plugins") as boolean;
  }
  if (commandKey === "debug") {
    return isCommandFlagEnabled(cfg, "debug") as boolean;
  }
  if (commandKey === "bash") {
    return isCommandFlagEnabled(cfg, "bash") as boolean;
  }
  return true;
}
