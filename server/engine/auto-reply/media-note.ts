...
// self-contained; the surface stays compatible with the openclaw source so a
// later migration can swap the stubs back to the real modules.

/** Minimal subset of MsgContext fields accessed by buildInboundMediaNote. */
export type MsgContext = {
  ...
  MediaType?: string;
  Transcript?: string;
  MediaUnderstanding?: ReadonlyArray<{
    kind?: string;
    attachmentIndex?: number;
  }>;
  MediaUnderstandingDecisions?: ReadonlyArray<{
    capability?: string;
    outcome?: string;
    attachments?: ReadonlyArray<{
      chosen?: { outcome?: string };
      attachmentIndex?: number;
    }>;
  }>;
};

// cross-wms media/store.ts does not expose getMediaDir. Returning a stable
// placeholder keeps normalizeManagedInboundMediaRef a no-op for absolute paths
// until the real module is ported.
  ...
  if (Array.isArray(ctx.MediaUnderstanding)) {
    for (const output of ctx.MediaUnderstanding) {
      if (
        ...
        isValidAttachmentIndex(output.attachmentIndex ?? NaN, attachmentCount)
      ) {
        transcribedAudioIndices.add(output.attachmentIndex as number);
      }
    }
  }
    ...
    for (const decision of ctx.MediaUnderstandingDecisions) {
      ...
      for (const attachment of decision.attachments ?? []) {
        if (
          ...
          isValidAttachmentIndex(attachment.attachmentIndex ?? NaN, attachmentCount)
        ) {
          transcribedAudioIndices.add(attachment.attachmentIndex as number);
        }
      }
    }
  }
