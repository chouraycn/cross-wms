/**
 * Reply payload contracts and metadata helpers shared by dispatch and channel
 * renderers.
 *
 * Ported from openclaw/src/auto-reply/reply-payload.ts. Imports that pointed at
 * OpenClaw config/interactive packages are inlined as minimal local types so
 * the module stays self-contained within cross-wms server/engine/auto-reply.
 */

/** Reply threading mode resolved from config. */
export type ReplyToMode = 'off' | 'always' | 'auto' | null;

/** Channel-agnostic rich presentation block. Core degrades or asks the channel
 *  renderer to map it. Modeled loosely to avoid pulling the full interactive
 *  payload tree. */
...
export function isFastModeAutoProgressPayload(
...
function hasReplyPayloadMedia(
  payload: Pick<ReplyPayload, 'mediaUrl' | 'mediaUrls'>,
): boolean {
...
export type ReplyPayloadMetadata = {
  ...
  /**
   * Internal notices generated after a runtime/provider failure are not
   * assistant source replies. Dispatch may deliver them even when normal
