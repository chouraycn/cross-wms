...
} from '@openclaw-src/agents/embedded-agent-helpers/bootstrap.js';
import type { AgentMessage } from '@openclaw-src/agents/runtime/index.js';
import { buildSystemPromptReport } from '@openclaw-src/agents/system-prompt-report.js';
import {
  resolveFreshSessionTotalTokens,
  type SessionEntry,
  type SessionSystemPromptReport,
} from '@openclaw-src/config/sessions/types.js';
import { readSessionMessages } from '@openclaw-src/gateway/session-utils.fs.js';
import { estimateTokensFromChars } from '@openclaw-src/utils/cjk-chars.js';
...
export async function buildContextReply(params: HandleCommandsParams): Promise<ReplyPayload> {
  ...
  const totalsLine =
    cachedContextUsageTokens != null
      ...
      : `Session tokens (cached): any / ctx=${contextWindowLabel}`;
