...
import { loadSessionStore } from '@openclaw-src/config/sessions/store.js';
import type { SessionEntry, SessionScope } from '@openclaw-src/config/sessions/types.js';
...
function isCompleteReplyConfig(config: any): config is OpenClawConfig {
  return Boolean(
    config &&
...
function usesFullReplyRuntime(config: any): boolean {
  return Boolean(
    config &&
    typeof config === "object" &&
