...
import { isStoredCredentialCompatibleWithAuthProvider } from '@openclaw-src/agents/auth-profiles/order.js';
import { clearSessionAuthProfileOverride } from '@openclaw-src/agents/auth-profiles/session-override.js';
...
import type { ModelCatalogEntry } from '@openclaw-src/agents/model-catalog.js';
import { parseConfiguredModelVisibilityEntries } from '@openclaw-src/agents/model-selection-shared.js';
import {
...
import { applyModelOverrideToSessionEntry } from '@openclaw-src/sessions/model-overrides.js';
...
const modelCatalogRuntimeLoader = createLazyImportLoader(
  () => import('@openclaw-src/agents/model-catalog.runtime.js'),
);
const sessionAccessorRuntimeLoader = createLazyImportLoader(
  () => import('@openclaw-src/config/sessions/session-accessor.js'),
);
...
export async function createModelSelectionState(params: {
  ...
  if (
    ...
    const { ensureAuthProfileStore } = await import('@openclaw-src/agents/auth-profiles.runtime.js');
