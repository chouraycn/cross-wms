...
} from '@openclaw-src/agents/model-selection-shared.js';
...
async function loadResetModelCatalog(cfg: OpenClawConfig): Promise<ModelCatalogEntry[]> {
  const { loadModelCatalog } = await import('@openclaw-src/agents/model-catalog.js');
...
async function resolveResetFallbackModels(params: {
  ...
  if (params.agentId) {
    const { resolveAgentModelFallbacksOverride } = await import('@openclaw-src/agents/agent-scope.js');
...
function applySelectionToSession(params: {
  ...
  if (storePath) {
    void import('@openclaw-src/config/sessions/session-accessor.js')
