...
import { createTtsDirectiveTextStreamCleaner } from '@openclaw-src/tts/directives.js';
import { resolveStatusTtsSnapshot } from '@openclaw-src/tts/status-config.js';
import { resolveConfiguredTtsMode, shouldCleanTtsDirectiveText } from '@openclaw-src/tts/tts-config.js';
...
const channelPluginRuntimeLoader = createLazyImportLoader(
  () => import('@openclaw-src/channels/plugins/index.js'),
);
const messageActionRuntimeLoader = createLazyImportLoader(
  () => import('@openclaw-src/infra/outbound/message-action-runner.js'),
);
...
export function createAcpDispatchDeliveryCoordinator(params: {
  ...
  const startReplyLifecycleOnce = async () => {
    ...
    void Promise.resolve(params.onReplyStart?.()).catch((error: any) => {
      logVerbose(
