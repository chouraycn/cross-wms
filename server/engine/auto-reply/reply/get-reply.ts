...
import { resolveModelRefFromString } from '@openclaw-src/agents/model-selection.js';
import { resolveAgentTimeoutMs } from '@openclaw-src/agents/timeout.js';
import { DEFAULT_AGENT_WORKSPACE_DIR, ensureAgentWorkspace } from '@openclaw-src/agents/workspace.js';
import { resolveChannelModelOverride } from '@openclaw-src/channels/model-overrides.js';
import { type OpenClawConfig, getRuntimeConfig } from '@openclaw-src/config/config.js';
import { logVerbose } from '@openclaw-src/globals.js';
import { measureDiagnosticsTimelineSpan } from '@openclaw-src/infra/diagnostics-timeline.js';
...
} from '@openclaw-src/plugins/hook-agent-context.js';
...
const mediaUnderstandingApplyRuntimeLoader = createLazyImportLoader(
  () => import('@openclaw-src/media-understanding/apply.runtime.js'),
);
const linkUnderstandingApplyRuntimeLoader = createLazyImportLoader(
  () => import('@openclaw-src/link-understanding/apply.runtime.js'),
);
...
const hookRunnerGlobalLoader = createLazyImportLoader(
  () => import('@openclaw-src/plugins/hook-runner-global.js'),
);
    ...
    if (opts?.isHeartbeat) {
      ...
      if (heartbeatPending.shouldClear) {
        ...
        if (sessionKey && storePath) {
          const { updateSessionEntry } = await import('@openclaw-src/config/sessions/session-accessor.js');
