...
// 应用仅运行时生效的配置覆盖，不修改持久化配置。
//
// 降级说明：源文件依赖 ../utils.js 的 isPlainObject。cross-wms 该函数位于
// ../infra/plain-object.js，此处调整导入路径。其余依赖（./config-paths.js、
// ../infra/prototype-keys.js、./types.js）均已就绪或调整至 cross-wms 对应路径。
