...
// 配置 includes：$include 指令用于模块化配置。
//
// 降级策略：
// 1. 源文件依赖 ../infra/boundary-file-read.js 的 canUseRootFileOpen 和
//    openRootFileSync。cross-wms 的 boundary-file-read.ts 从 _fs-safe-stubs.ts
//    重新导出这些函数，但签名不同（cross-wms 接受 (filePath, rootDir, reader)
//    三参数形式，openclaw 接受选项对象）。此处重写 readConfigIncludeFileWithGuards
//    以适配 cross-wms 的降级签名。
// 2. 源文件依赖 ../infra/boundary-path.js 的 resolvePathViaExistingAncestorSync。
//    cross-wms 的 boundary-path.ts 未导出此函数，此处内联降级实现
//    （通过 realpathSync 尝试解析，失败时回退到原始路径）。
// 3. 源文件依赖 ../utils/parse-json-compat.js，cross-wms 中该模块位于
//    ../infra/parse-json-compat.js，此处调整导入路径。
// 4. 源文件依赖 ../utils.js 的 isPlainObject，cross-wms 中该函数位于
//    ../infra/plain-object.js，此处调整导入路径。
// 5. 源文件依赖 ../security/scan-paths.js 的 isPathInside 和
//    ../infra/prototype-keys.js 的 isBlockedObjectKey，这两个模块在 cross-wms
//    中均已就绪。
...
/**
 * 降级实现：openclaw 的 resolvePathViaExistingAncestorSync 通过遍历最近的
 * 已存在祖先目录来解析规范路径。此处使用 realpathSync 尝试解析，失败时
 * 回退到 path.normalize（保持与 openclaw 的安全检查兼容）。
 */
function resolvePathViaExistingAncestorSync(resolvedPath: string): string {
  try {
    return fs.realpathSync(resolvedPath);
  } catch {
    // 文件可能尚不存在 — 回退到 normalize，词法包含检查在调用方已通过
...
/**
 * 降级实现：openclaw 使用 openRootFileSync（选项对象签名）打开 root 范围文件。
 * cross-wms 的 openRootFileSync 签名为 (filePath, rootDir, reader)，且 canUseRootFileOpen
 * 始终返回 true。此处简化为直接 readFileSync + maxBytes 检查 + realpath 跟踪。
 */
export function readConfigIncludeFileWithGuards(params: IncludeFileReadParams): string {
  ...
  if (Buffer.byteLength(raw, "utf-8") > maxBytes) {
    throw new ConfigIncludeError(
      `Include file exceeds maximum size (${maxBytes} bytes): ${params.includePath}`,
      params.includePath,
    );
  }
  try {
