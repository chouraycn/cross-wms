...
// 为 schema 和文档表面规范化配置标签元数据。
//
// 降级说明：源文件依赖 @openclaw/normalization-core/string-coerce 的
// normalizeLowercaseStringOrEmpty，以及 ../shared/config-ui-hints-types.js 的
// ConfigUiHint/ConfigUiHints 类型。此处内联等价实现与类型定义。
...
/** 配置 UI 提示条目（降级内联类型，源自 shared/config-ui-hints-types）。 */
...
/** 用于过滤和分组 schema 提示的稳定配置 UI 标签词汇表。 */
...
/** 从 schema 路径和可选的提示元数据派生已知配置 UI 标签。 */
...
export function applyDerivedTags(hints: ConfigUiHints): ConfigUiHints {
  ...
  for (const [path, hint] of Object.entries(hints)) {
    ...
    // 在已知标签之后保留未知标签，这样外部/自定义 UI 标签能在规范化后存活。
    const tags = [
