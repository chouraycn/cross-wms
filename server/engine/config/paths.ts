...
/**
 * Resolve the directory that holds the active config file (and related
 * config artifacts such as backups, version metadata, etc.).
 *
 * This is the directory returned by {@link resolveStateDir} — the canonical
 * location for `openclaw.json` and associated mutable state.
 */
...
/**
 * Resolve the canonical runtime filesystem paths used by the config layer.
 *
 * Returns the active config path, state directory, and config directory in a
 * single object so callers can destructure all of them at once.
 */
