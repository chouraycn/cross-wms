...
//
// 移植自 openclaw/src/config/sessions/skill-prompt-blobs.ts
//
// 降级策略：
//  - 原文件依赖 ../../infra/json-files.js 的 writeTextAtomic，cross-wms 尚未移植该模块。
//    这里使用本地简化实现（基于 fs.promises 的临时文件 + rename）。
//  - 原文件依赖 ./types.js 的 SessionEntry / SessionSkillPromptRef / SessionSkillSnapshot 类型，
//    cross-wms 的 types.ts 使用不同的 schema，这里使用本地占位类型保持编译期类型安全。
import crypto from "node:crypto";
import fs from "node:fs";
import path from "node:path";

/** SessionSkillPromptRef 占位类型（与 openclaw 一致）。 */
...
/** SessionSkillSnapshot 占位类型（与 openclaw 一致）。 */
...
async function writeTextAtomic(
  filePath: string,
  data: string,
  options: { mode?: number; tempPrefix?: string; durable?: boolean } = {},
): Promise<void> {
  const dir = path.dirname(filePath);
  const prefix = options.tempPrefix ?? path.basename(filePath);
  const tempPath = path.join(dir, `.${prefix}.${process.pid}.${Date.now()}.tmp`);
  await fs.promises.mkdir(dir, { recursive: true });
  await fs.promises.writeFile(tempPath, data, { mode: options.mode ?? 0o644 });
