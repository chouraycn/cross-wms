...
//    * deliveryContextFromSession（原依赖 ../../utils/delivery-context.shared.js）
//    * getRuntimeConfig（原依赖 ../io.js）
//    * resolveStorePath（原依赖 ./paths.js）
//    * foldedSessionKeyAliasCandidates / hasMismatchedCaseSensitiveDeliveryProof /
//      isConfirmedLowercasedLegacyAlias / normalizeStoreSessionKey（原依赖 ./store-entry.js）
//    * readSessionStoreSnapshot（原依赖 ./store.js）
//    * resolveAllAgentSessionStoreTargetsSync（原依赖 ./targets.js）
//    * parseSessionThreadInfo（原依赖 ./thread-info.js）
//  - OpenClawConfig / SessionEntry 类型使用本地占位定义。
//  - 降级函数返回安全默认值（undefined / 空对象），主函数 extractDeliveryInfo 逻辑保持完整，
//    在降级模式下返回 { deliveryContext: undefined, threadId: undefined }。
...
/** 降级：解析会话存储 agent id。 */
function resolveSessionStoreAgentId(_cfg: OpenClawConfig, _sessionKey: string): string {
  return "";
}

/** 降级：判断是否需要折叠会话键别名证明。 */
function requiresFoldedSessionKeyAliasProof(_key: string): boolean {
  return false;
}

/** 降级：从会话条目提取投递上下文。 */
function deliveryContextFromSession(entry: SessionEntry | undefined): {
  channel?: string;
  to?: string;
  accountId?: string;
  threadId?: string | number;
  ...
  return {
    ...
    to: entry.to ?? entry.lastTo,
    accountId: entry.accountId ?? entry.lastAccountId,
    threadId: entry.threadId ?? entry.lastThreadId,
  };
}
...
/** 降级：规范化存储会话键。 */
function normalizeStoreSessionKey(key: string): string {
  return key.trim();
}

/** 降级：生成折叠会话键别名候选。 */
function foldedSessionKeyAliasCandidates(_normalized: string): string[] {
  return [];
}

/** 降级：判断是否大小写不匹配的投递证明。 */
function hasMismatchedCaseSensitiveDeliveryProof(
...
/** 降级：判断是否已确认的小写遗留别名。 */
function isConfirmedLowercasedLegacyAlias(
  _entry: SessionEntry | undefined,
  _normalized: string,
): boolean {
  return false;
...
/** 降级：解析所有 agent 会话存储目标。 */
function resolveAllAgentSessionStoreTargetsSync(_cfg: OpenClawConfig): Array<{
  agentId: string;
  storePath: string;
}> {
  return [];
