...
//  - updateSessionStore 使用本地降级实现（cross-wms 的 store.ts 使用 SessionStore 类，
//    API 不同）。降级版本直接操作传入的 store 对象并返回 0，不进行持久化。
...
async function updateSessionStore(
  storePath: string,
  mutator: (store: Record<string, SessionEntry>) => number | Promise<number>,
  _options?: {
    skipSaveWhenResult?: (result: number) => boolean;
    takeCacheOwnership?: boolean;
  },
): Promise<number> {
  // cross-wms 尚未移植文件支持的 updateSessionStore；这里仅执行 mutator 占位。
  // 调用方在降级模式下不会真正持久化清理结果。
