...
// formatSessionArchiveTimestamp 从 artifacts.js 内联（stub 实现），避免 store.ts ↔ artifacts.ts 循环依赖。
// 注：artifacts.ts 中为 stub（返回空字符串），session-artifact-names.ts 中有真实实现。
function formatSessionArchiveTimestamp(_date?: Date | number): string {
  return "";
}
import {
