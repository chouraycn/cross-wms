...
// 通过 diff 当前状态并保留元数据来准备配置写入。
//
// 降级说明：
// 1. 源文件依赖 @openclaw/model-catalog-core/provider-model-id-normalization 的
//    normalizeConfiguredProviderCatalogModelId。此处内联透传降级实现。
// 2. 源文件依赖 ../utils.js 的 isRecord。此处内联等价实现。
// 3. 源文件依赖 ./types.js 的 OpenClawConfig。cross-wms 该类型位于 ./types/openclaw.js。
...
/** 内联降级实现：normalizeConfiguredProviderCatalogModelId 透传。 */
function normalizeConfiguredProviderCatalogModelId(
  _provider: string,
  modelId: string,
  _policies?: ReadonlyMap<string, unknown>,
): string {
  return modelId;
}
...
// 在 patch 前克隆配置片段，使变更准备从不别名调用方。
function cloneUnknown<T>(value: T): T {
  return structuredClone(value);
}

/** 构建 RFC-7396 风格的 merge patch，介于 source 与 target 配置值之间。 */
...
function preserveUntouchedIncludes(params: {
  ...
  for (const includePath of collectIncludeOwnedPaths(params.rootAuthoredConfig)) {
    ...
    // 整条目数组 include 保留其位置所有权，同时允许无关的同级编辑。
    // 嵌套数组 include 要求数组本身未变更。
