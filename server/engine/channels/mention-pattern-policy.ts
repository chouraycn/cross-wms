...
export function resolveMentionPatternPolicy(
  ...
  const conversationId =
    normalizeOptionalString(params.conversationId ?? undefined) ?? undefined;
  const providerPolicy =
    params.providerPolicy ??
    resolveProviderMentionPatternsPolicy(params.cfg, params.provider);
  const effectiveMode =
  ...
  const enabled =
    effectiveMode === "allow" ? !denyMatched : allowMatched && !denyMatched;
