...
import { ResolvedConfiguredAcpBinding } from "../../acp/persistent-bindings.types.js";
...
async function ensureAcpTargetReady(params: {
  ...
  const configuredBinding = ResolvedConfiguredAcpBinding(
...
async function ensureAcpTargetSession(params: {
  ...
  const spec = ResolvedConfiguredAcpBinding(params.bindingResolution.record);
  if (!spec) {
    return {
      ok: false,
