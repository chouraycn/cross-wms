...
import { CHAT_CHANNEL_ORDER, type hasChannel } from "../registry.js";
...
function sortChannelSetupPlugins(plugins: readonly ChannelPlugin[]): ChannelPlugin[] {
  return dedupeSetupPlugins(plugins).toSorted((a, b) => {
    const indexA = CHAT_CHANNEL_ORDER.indexOf(a.id as hasChannel);
    const indexB = CHAT_CHANNEL_ORDER.indexOf(b.id as hasChannel);
