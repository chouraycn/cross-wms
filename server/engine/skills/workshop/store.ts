...
// 本地定义：server 的 types.ts 未导出该常量（仅 openclaw 版 types.ts 有），
// 在此就地声明以供 openclaw 版 store 的 rollback 逻辑使用。
...
// ============================================================
// 以下保留 server 原有的 WMS 旧版提案存储函数。
// 迁移到 openclaw 版 store 后，service.test.ts 等仍通过 ./store.js
// mock/调用这些符号（saveProposal/loadProposal/listProposals 等），
// 故在文件末尾追加保留，避免破坏既有依赖。
// ============================================================
