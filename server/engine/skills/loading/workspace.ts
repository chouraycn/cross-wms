...
// ============================================================================
// Workspace skills 目录与加载辅助（WMS 期望的对外 API）
// 目录约定：{workspaceDir}/.cross-wms/skills，与 source-install / refresh / session 保持一致
// ============================================================================

/**
 * 返回 workspace 技能目录的绝对路径：{workspaceDir}/.cross-wms/skills
 * 同步计算，调用方使用 await 时亦兼容（await 对非 Promise 值原样返回）。
 */
export function getWorkspaceSkillsDir(workspaceDir: string): string {
  return path.join(workspaceDir, ".cross-wms", "skills");
}

/**
 * 确保 workspace 技能目录存在（递归创建），返回该目录路径。
 * 供 install / source-install / archive-install 等安装流程使用。
 */
export async function ensureWorkspaceSkillsDir(workspaceDir: string): Promise<string> {
  const skillsDir = getWorkspaceSkillsDir(workspaceDir);
  fs.mkdirSync(skillsDir, { recursive: true });
  return skillsDir;
}

/**
 * 加载 workspace 下所有可见技能条目（同步）。
 * 复用 loadWorkspaceSkillEntries 的完整加载逻辑（含沙箱 / 符号链接校验）。
 * 返回 SkillEntry[]；调用方使用 await 时亦兼容。
 */
export function loadWorkspaceSkills(workspaceDir: string): SkillEntry[] {
  return loadWorkspaceSkillEntries(workspaceDir);
}

/**
 * 按技能名加载单个 workspace 技能条目。
 * 未找到时返回 null。
 */
export function loadWorkspaceSkill(workspaceDir: string, name: string): SkillEntry | null {
  const entries = loadWorkspaceSkillEntries(workspaceDir);
  return entries.find((entry) => entry.skill.name === name) ?? null;
}

/**
 * 检查指定名称的技能是否存在于 workspace 技能目录中。
 * 以 {skillsDir}/{name}/SKILL.md 是否存在为准。
 */
export async function workspaceSkillExists(
  workspaceDir: string,
  name: string,
): Promise<boolean> {
  const skillFile = path.join(getWorkspaceSkillsDir(workspaceDir), name, "SKILL.md");
  try {
...
/**
 * 列出 workspace 技能目录下所有技能的目录名。
 * 仅返回包含 SKILL.md 的子目录名。
 */
