...
// 移植自 openclaw/src/skills/loading/bundled-dir.ts。
// 降级：import.meta.url / fileURLToPath 替换为 __filename（CommonJS 模块）。
...
export function resolveBundledSkillsDir(
  ...
  try {
    // 降级：__filename 替换 import.meta.url（CommonJS 模块）
    const moduleUrl = opts.moduleUrl ?? `file://${__filename}`;
