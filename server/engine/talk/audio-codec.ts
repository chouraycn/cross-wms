 ...
 *
 * 自包含实现，参考 openclaw/src/talk/audio-codec.ts。
 */
...
// ============================================================================
// 静音检测：用于语音活动检测（VAD）回退与静默回合切分
// ============================================================================

/** 静音检测选项。 */
export type SilenceDetectionOptions = {
  /** 低于该 RMS 能量阈值（满刻度 32767）视为静音。 */
  threshold?: number;
  /** 可注入的时钟，便于测试。 */
  now?: () => number;
};
...
/**
 * 检测一段 16-bit PCM 是否静音。
 * 默认阈值 250（约 -42 dBFS），适用于静默回合切分回退。
 */
export function detectPcmSilence(
  pcm: Buffer,
  options: SilenceDetectionOptions = {},
): SilenceDetectionResult {
  const threshold = options.threshold ?? 250;
  const samples = readInt16Samples(pcm);
  if (samples.length === 0) {
    return { silent: true, rms: 0, peak: 0 };
  }
  let sumSquares = 0;
  let peak = 0;
  for (let i = 0; i < samples.length; i += 1) {
  ...
  return { silent: rms < threshold, rms, peak };
}
