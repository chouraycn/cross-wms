/**
 * Realtime voice tool definition and helpers for delegating work to the agent.
 *
...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Delegate the caller's request to the configured agent for normal tool-backed work, actions, context, memory, or reasoning before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final result before answering with the actual result.`,
  };
}
...
/** Resolve the tool allowlist paired with the consult exposure policy. */
...
export function buildRealtimeVoiceAgentConsultPolicyInstructions(config: {
  ...
  return [
    ...
    "- Call openclaw_agent_consult before answering requests that need facts, memory, current information, tools, workspace state, or the user's context.",
...
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    ...
    "Act as the configured agent on behalf of this user. Use available tools when the request asks you to do work.",
