...
export type ClassifiedCronError = {
  category?: CronRetryOn;
};

export function classifyCronError(error: unknown): ClassifiedCronError {
  const msg = error instanceof Error ? error.message : String(error);
  const hint = resolveCronExecutionRetryHint(msg);
  return { category: hint.category };
}

export function shouldRetryCronError(error: unknown): boolean {
  const msg = error instanceof Error ? error.message : String(error);
  const hint = resolveCronExecutionRetryHint(msg);
