...
export function calculateStaggerWindow(
  ...
  throw new Error("calculateStaggerWindow: 需要从 OpenClaw 移植此函数实现。路径: server/engine/cron/stagger.ts");
}
...
export function shouldStaggerJob(
  ...
  throw new Error("shouldStaggerJob: 需要从 OpenClaw 移植此函数实现。路径: server/engine/cron/stagger.ts");
}
