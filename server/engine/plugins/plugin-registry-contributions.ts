 ...
 * 移植自 openclaw/src/plugins/plugin-registry-contributions.ts。
 * 降级策略：
 ...
 */
import type { OpenClawConfig } from "./plugin-metadata-snapshot.types.js";
import {
...
function listManifestContractValues(
  ...
  return (plugin.contracts?.[contract] ?? []) as readonly string[];
}
...
function resolveContributionPluginIds(params: {
  ...
  const plugins = (params.index as unknown as { plugins?: Array<{ pluginId: string; enabled?: boolean }> })
    .plugins ?? [];
...
function listContributionManifestPlugins(
  ...
  if (plugins) {
    ...
    return plugins.filter((plugin) => enabledPluginIds.has(String(plugin.id))) as unknown as readonly PluginManifestRecord[];
  }
...
function loadCurrentManifestRegistryForPluginRegistry(
  _params: LoadPluginRegistryManifestParams,
): PluginManifestRegistry | undefined {
  // 降级：current-plugin-metadata-snapshot.ts 未移植，始终返回 undefined。
  return undefined;
}
...
export function listPluginContributionIds(
  ...
  const index = (params.lookUpTable?.index ?? loadPluginRegistrySnapshot(params)) as PluginRegistrySnapshot;
...
export function resolveProviderOwners(params: ResolveProviderOwnersParams): readonly string[] {
  ...
  if (params.lookUpTable) {
    const index = params.lookUpTable.index as PluginRegistrySnapshot;
    const owners: string[] = [];
  ...
  return resolvePluginContributionOwners({
    ...
    matches: (contributionId: string) => normalizeProviderId(contributionId) === providerId,
  });
}
  ...
  return loadManifestContractRegistry(params)
    .plugins.filter(
      (plugin) =>
        (!params.origin || plugin.origin === params.origin) &&
        listManifestContractValues(plugin, params.contract).length > 0 &&
        ((plugin.configContracts?.compatibilityRuntimePaths as readonly string[] | undefined) ?? []).includes(normalizedPath),
    )
    .map((plugin) => plugin.id)
    .toSorted((left, right) => left.localeCompare(right));
}
