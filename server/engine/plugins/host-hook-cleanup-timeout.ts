// Bounds plugin host cleanup hook execution with a shutdown timeout.
//
// 移植自 openclaw/src/plugins/host-hook-cleanup-timeout.ts。
//
// 降级策略：纯 TypeScript 实现，无外部依赖。行为与 openclaw 原版一致：
// 使用 Promise.race + setTimeout 限制清理钩子最长执行时间。
