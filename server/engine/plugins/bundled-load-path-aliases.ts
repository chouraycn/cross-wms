...
//
// 移植自 openclaw/src/plugins/bundled-load-path-aliases.ts。
//
// 降级策略：仅依赖 node:path 与本地 ./path-safety.js 的 isPathInside。
// cross-wms 已移植 path-safety.js，直接引用。行为与 openclaw 原版一致：
// 解析打包路径与旧版扩展路径之间的别名映射。
