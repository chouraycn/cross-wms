// 加载插件公共运行时表面，通过已文档化的入口点。
//
// 移植自 openclaw/src/plugins/public-surface-runtime.ts。
//
// 降级策略：
//  - 原文件依赖 ../utils.js 的 resolveUserPath。cross-wms 的 _fs-safe-stubs 提供的同名
//    导出仅接受单参数，无法传递 env。这里内联降级实现，与 bundled-dir.ts 中保持一致。
//  - 原文件依赖 ./bundled-dir.js 的 areBundledPluginsDisabled、resolveBundledPluginsDir。
//    cross-wms 已移植，直接引用。
//  - 行为与 openclaw 原版一致：解析源码树、dist、包布局下的公共表面路径。
...
/** 内联降级：../utils.js —— resolveUserPath，支持可选 env 参数。 */
...
/** 在源码、dist、包布局下解析 bundled 插件公共表面工件。 */
