...
//
// 移植自 openclaw/src/plugins/installed-plugin-index-install-records.ts。
//
// 降级策略：
//  - 原文件依赖 ../config/types.plugins.js 的 PluginInstallRecord。cross-wms 尚未
//    移植该模块。这里定义本地结构占位（与 installed-plugin-index-types.ts 中的
//    占位一致），仅含 normalizeInstallRecord/restoreInstallRecord 实际访问的字段。
//  - 原文件依赖 ./installed-plugin-index-types.js 的 InstalledPluginIndex 与
//    InstalledPluginInstallRecordInfo。cross-wms 已移植该模块，直接引用。
//  - 行为与 openclaw 原版一致：将持久化安装记录规范化为 index-safe 元数据并反向恢复。
 ...
 * 这里定义与 installed-plugin-index-types.ts 中 PluginInstallRecord 占位一致的字段集，
 * 仅含 normalizeInstallRecord/restoreInstallRecord 实际访问的字段。
 */
