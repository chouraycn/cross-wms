 ...
 * 移植自 openclaw/src/plugins/plugin-peer-link.ts。
 * 降级策略：
 *  - import.meta.url 改用 __filename。
 *  - hasErrnoCode 降级为内联实现。
 *  - resolveOpenClawPackageRootSync 降级为返回 undefined（infra/openclaw-root.ts 未移植）。
 *  - 所有 export async function 保持签名兼容，行为降级为 no-op 或返回空结果。
 */
...
/** 占位：解析 openclaw 包根目录（infra/openclaw-root.ts 未移植，降级为 undefined）。 */
