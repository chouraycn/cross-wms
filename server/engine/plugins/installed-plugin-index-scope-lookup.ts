...
//  - 原文件依赖 ./installed-plugin-index-config-path-scope.js 的
//    CONFIG_PATH_ACTIVATION_COMPAT_CODE。cross-wms 已移植，直接引用。
//  - 原文件依赖 ./installed-plugin-index.js 的 InstalledPluginIndex 与
//    InstalledPluginIndexRecord。改从 ./installed-plugin-index-types.js 引用
//    （cross-wms 已移植该模块，类型契约一致）。
//  - 行为与 openclaw 原版一致：基于 index 构建通道/provider/模型支持查找映射。
...
// ============================================================================
// 内联降级：./config-state.js —— normalizePluginId
// ============================================================================

/**
 * 规范化插件 ID。
 *
 * 降级说明：cross-wms 的 config-state.js 尚未移植。openclaw 原版对插件 ID
 * 执行大小写/分隔符规范化。这里降级为仅 trim 输入字符串。
 */
