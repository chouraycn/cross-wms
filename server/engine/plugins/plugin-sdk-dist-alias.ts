 ...
 * 移植自 openclaw/src/plugins/plugin-sdk-dist-alias.ts。
 * 降级策略：
 *  - infra/json-files.ts 未移植，tryReadJsonSync / writeJsonSync 降级为内联实现。
 *  - 所有 export 保持签名兼容；ensureOpenClawPluginSdkAlias 保留 fs 操作。
 */
...
/** 占位：安全读取 JSON（infra/json-files.ts 未移植）。 */
function tryReadJsonSync<T = unknown>(filePath: string): T | undefined {
  try {
