...
type EngineFactory = (ctx?: ContextEngineFactoryContext) => ContextEngine | Promise<ContextEngine>;
...
function createGlobalRegistry() {
  const engines = new Map<string, RegistryEntry>();
  ...
  return {
    ...
    register(
      ...
      engines.set(engineId, {
        ...
        displayName: (config.displayName as string) ?? engineId,
      });
    },
    unregister(engineId: string): boolean {
      return engines.delete(engineId);
    },
    getFactory(engineId: string): EngineFactory | undefined {
      return engines.get(engineId)?.factory;
    },
    getConfig(engineId: string): EngineConfig | undefined {
      return engines.get(engineId)?.config;
    },
    listEngines(): EngineRegistration[] {
      const result: EngineRegistration[] = [];
      for (const [engineId, entry] of engines.entries()) {
        result.push({
          engineId,
    ...
    getDefaultEngineId(): string | undefined {
      for (const [engineId, entry] of engines.entries()) {
        if (entry.isDefault) {
          return engineId;
        }
    ...
    async createEngine(
      _sessionId: string,
      options?: { engineId?: string; factoryContext?: ContextEngineFactoryContext },
    ): Promise<ContextEngine> {
      const engineId = options?.engineId ?? this.getDefaultEngineId();
      if (!engineId) {
        throw new Error("No default context engine registered");
      }
      const entry = engines.get(engineId);
      if (!entry) {
        throw new Error(`Context engine "${engineId}" not registered`);
      }
      const factoryCtx = options?.factoryContext ?? {};
      return await entry.factory(factoryCtx);
    },
    clear(): void {
      engines.clear();
    },
...
export const globalRegistry = createGlobalRegistry();
