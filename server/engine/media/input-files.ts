...
// Ported from openclaw/src/media/input-files.ts.
//
// Dependency adjustments:
//   - @openclaw/media-core/base64 (canonicalizeBase64, estimateBase64DecodedBytes)
//     → ./_openclaw-media-stubs.js
//   - @openclaw/media-core/content-length (parseMediaContentLength)
//     → ./_openclaw-media-stubs.js
//   - @openclaw/media-core/mime (detectMime) → ./_openclaw-media-stubs.js
...
//   - ../infra/net/ssrf.js (SsrFPolicy) → ./_porting-stubs.js
//   - ../logger.js (logWarn) → ./_porting-stubs.js
//   - ./media-services.js (convertHeicToJpeg) → ./_porting-stubs.js
//   - ./pdf-extract.js (extractPdfContent, PdfExtractedImage) → available
import {
  canonicalizeBase64,
  estimateBase64DecodedBytes,
  parseMediaContentLength,
  detectMime as detectMimeStub,
  readResponseWithLimit,
} from "./_openclaw-media-stubs.js";
import {
...
export async function fetchWithGuard(params: {
  ...
  try {
    ...
    try {
      contentLength = parseMediaContentLength(response.headers.get("content-length")) ?? null;
    } catch (err) {
      await discardIgnoredResponseBody(response);
      throw err;
    }
...
async function normalizeInputImage(params: {
  ...
  // cross-wms detectMime stub accepts {buffer} only; headerMime is not supported.
  const detectedMime = normalizeMimeType(
    await detectMimeStub({ buffer: params.buffer }),
  );
...
async function resolveInputFileMime(params: {
  ...
  // cross-wms detectMime stub accepts {buffer} only; headerMime is not supported.
  const sniffedMime = normalizeMimeType(await detectMimeStub({ buffer: params.buffer }));
  if (!sniffedMime) {
