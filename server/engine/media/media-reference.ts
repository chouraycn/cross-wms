...
// Ported from openclaw/src/media/media-reference.ts.
//
// Dependency adjustments:
//   - ../infra/local-file-access.js (safeFileURLToPath) → available in cross-wms
//   - ../utils.js (resolveUserPath) → ../infra/_fs-safe-stubs.js (process.env-aware
//     ~ expansion + path.resolve), matching the local-media-path.ts port pattern
//   - ./store.js (getMediaDir, resolveMediaBufferPath) → ./_porting-stubs.js
//     (openclaw store.ts not ported; stub provides media dir + buffer path resolution)
import fs from "node:fs/promises";
import path from "node:path";
import { safeFileURLToPath } from "../infra/local-file-access.js";
import { resolveUserPath } from "../infra/_fs-safe-stubs.js";
import { getMediaDir, resolveMediaBufferPath } from "./_porting-stubs.js";
