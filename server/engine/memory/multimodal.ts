...
export function isValidMultimodalFile(
  filePath: string,
  fileSize: number,
  settings: MemoryMultimodalSettings,
): { valid: boolean; reason?: string; modality?: MemoryMultimodalModality } {
  if (!isMemoryMultimodalEnabled(settings)) {
    return { valid: false, reason: "Multimodal memory is disabled" };
  }

  const modality = classifyMemoryMultimodalPath(filePath, settings);
  if (!modality) {
    return { valid: false, reason: `Unsupported file type: ${filePath}` };
  }

  if (fileSize > settings.maxFileBytes) {
    return { valid: false, reason: `File size exceeds limit: ${fileSize} > ${settings.maxFileBytes}` };
  }

  return { valid: true, modality };
}