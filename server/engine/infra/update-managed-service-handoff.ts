// 托管服务更新交接启动一个分离进程，在 gateway 退出后（launchd/systemd 类管理器下）
// 完成更新。
// 移植自 openclaw/src/infra/update-managed-service-handoff.ts
//
// 降级说明：
//  - resolveGatewayLaunchAgentLabel 来自 ./supervisor-markers.js（cross-wms 已有降级 stub）
//  - resolveGatewaySystemdServiceName / resolveGatewayWindowsTaskName 来自
//    ../daemon/constants.js，cross-wms 未移植 daemon 模块，本地降级 stub
//  - resolveOpenClawStateSqlitePath 来自 ../state/openclaw-state-db.paths.js，
//    cross-wms 未移植 state 数据库，降级为基于 resolveStateDir 的默认路径
import { spawn } from "node:child_process";
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";
import { resolveGatewayLaunchAgentLabel, SUPERVISOR_HINT_ENV_VARS, type RespawnSupervisor } from "./supervisor-markers.js";
...
/**
 * 解析 gateway systemd 服务名（降级 stub）。
...
function resolveGatewaySystemdServiceName(profile?: string): string {
  if (profile && profile.trim()) {
    return `openclaw-gateway-${profile.trim()}`;
  }
  return "openclaw-gateway";
}

/**
 * 解析 gateway Windows 计划任务名（降级 stub）。
 * openclaw 的 ../daemon/constants.js 导出此函数，cross-wms 未移植 daemon 模块。
 */
...
// ============================================================================
// 降级 stub —— 来自 ../state/openclaw-state-db.paths.js
// ============================================================================

/**
 * 解析 OpenClaw 状态数据库 SQLite 路径（降级实现）。
 * openclaw 的 ../state/openclaw-state-db.paths.js 导出此函数，
 * cross-wms 未移植 state 数据库模块，降级为基于 resolveStateDir 的默认路径。
 */
function resolveOpenClawStateSqlitePath(env: NodeJS.ProcessEnv): string {
  return path.join(resolveStateDir(env), "state.sqlite");
}
...
// HANDOFF_SCRIPT 是在分离子进程中运行的 CJS 脚本（字符串字面量）。
// 直接移植自 openclaw，原样保留，因为它在独立的 node 进程中执行。
