...
export function normalizeSystemRunApprovalPlan(value: unknown): SystemRunApprovalPlan | null {
  ...
  if (candidate.mutableFileOperand !== undefined && mutableFileOperand === null) return null;
  const commandText =
...
export function matchSystemRunApprovalEnvHash(params: {
  ...
  if (params.expectedEnvHash !== params.actualEnvHash) {
    return {
      ...
      details: { envKeys: params.actualEnvKeys, expectedEnvHash: params.expectedEnvHash, actualEnvHash: params.actualEnvHash },
    };
  }
  return { ok: true };
}
