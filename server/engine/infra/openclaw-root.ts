// 从运行时和 package 元数据解析 OpenClaw package 根目录。
// 降级实现：从 openclaw/src/infra/openclaw-root.ts 直接移植，无外部依赖。
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // 解析版本管理器（nvm、fnm、n、Homebrew/Linuxbrew）创建的符号链接，
  // 这些链接在 bin/ 中指向真实 package 位置。
  try {
...
function buildCandidates(opts: { cwd?: string; argv1?: string; moduleUrl?: string }): string[] {
  ...
  if (opts.moduleUrl) {
    try {
      ...
      // 忽略无效的 file:// URL，保留其他 package-root 提示
    }
  }
  if (opts.argv1) {
