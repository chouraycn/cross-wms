...
// 为 safe-bin allowlist 策略解析受信任目录。
//
// 降级策略：
// 1. 源文件依赖 @openclaw/normalization-core/string-normalization 的
//    normalizeSortedUniqueStringEntries、sortUniqueStrings、uniqueStrings，
//    cross-wms 中该模块位于 ./string-normalization.js，此处调整导入路径。
// 2. 其余依赖（node:fs、node:path）为 Node.js 内置模块。
