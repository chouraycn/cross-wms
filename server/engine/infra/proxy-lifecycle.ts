...
export async function startProxy(config: ProxyConfig | undefined): Promise<ProxyHandle | null> {
  ...
  if (activeUrl) {
    const registration = registerActiveManagedProxyUrl(new URL(proxyUrl), { loopbackMode: loopbackMode as ProxyLoopbackMode });
    return {
      proxyUrl,
  ...
  try {
    injectProxyEnv(proxyUrl, loopbackMode as ProxyLoopbackMode, proxyCaFile);
    registration = registerActiveManagedProxyUrl(new URL(proxyUrl), { loopbackMode: loopbackMode as ProxyLoopbackMode });
  } catch (err) {
