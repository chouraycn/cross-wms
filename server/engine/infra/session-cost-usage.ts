...
import { asFiniteNumber } from "./number-coercion.js";
...
} from "../config/sessions/session-artifact-names.js";
...
import { extractToolCallNames, countToolResults } from "./transcript-tools.js";
...
// ============================================================================
// 内联降级：agents/usage.js — NormalizedUsage 与 normalizeUsage
// ============================================================================
...
function normalizeUsage(raw: unknown): NormalizedUsage | null {
  ...
  return {
    ...
    cacheRead: asFiniteNumber(r.cacheRead ?? r.cacheReadTokens) ?? undefined,
    cacheWrite: asFiniteNumber(r.cacheWrite ?? r.cacheWriteTokens) ?? undefined,
    total: asFiniteNumber(r.total ?? r.totalTokens) ?? undefined,
  };
}

// ============================================================================
// 内联降级：utils/usage-format.js — 模型成本配置与估算
// ============================================================================
...
type ModelCostConfig = {
  ...
  cacheRead?: number;
  cacheWrite?: number;
  tieredPricing?: Array<{ maxTokens?: number; input: number; output: number }>;
};

function resolveModelCostConfig(_params: {
  provider?: string;
  model?: string;
...
function resolveModelCostConfigFingerprint(_config?: OpenClawConfig): string {
  return "default";
}
...
function estimateUsageCost(params: {
  ...
  const input = (usage.input ?? 0) * (cost.input ?? 0);
  const output = (usage.output ?? 0) * (cost.output ?? 0);
  const cacheRead = (usage.cacheRead ?? 0) * (cost.cacheRead ?? 0);
  const cacheWrite = (usage.cacheWrite ?? 0) * (cost.cacheWrite ?? 0);
  return input + output + cacheRead + cacheWrite;
}

// ============================================================================
// 内联降级：config/sessions/paths.js — 会话转录目录
// ============================================================================
...
function resolveSessionTranscriptsDirForAgent(agentId?: string): string {
  ...
  return agentId && agentId !== "default"
    ? path.join(base, "sessions", agentId)
...
function resolveSessionFilePath(
  ...
  const dir = resolveSessionTranscriptsDirForAgent(_options?.agentId);
  return path.join(dir, `${sessionId.trim()}.jsonl`);
}

// ============================================================================
// 内联降级：auto-reply/reply/strip-inbound-meta.js 与 shared/chat-envelope.js
// ============================================================================

function stripInboundMetadata(text: string): string {
  return text.replace(/^System:\s*/gim, "").trim();
}
...
function buildSessionCostSummaryFromCacheEntry(params: {
  ...
  for (const entry of params.entry.transcriptEntries) {
    ...
    if (ts !== undefined && ts < params.startMs) continue;
    if (ts !== undefined && ts > params.endMs) continue;
    if (ts !== undefined) {
    ...
    if (entry.role === "user") {
      messageCounts.user += 1; messageCounts.total += 1;
      if (ts !== undefined) lastUserTimestamp = ts;
    }
    if (entry.role === "assistant") {
      messageCounts.assistant += 1; messageCounts.total += 1;
      if (ts !== undefined) {
    ...
    if (ts !== undefined) {
      ...
      const daily = dailyMessageMap.get(dayKey) ?? {
        date: dayKey, total: 0, user: 0, assistant: 0, toolCalls: 0, toolResults: 0, errors: 0,
      };
      ...
      const utcQH = utcQuarterHourMessageMap.get(quarterBucket.key) ?? {
        date: quarterBucket.date, quarterIndex: quarterBucket.quarterIndex,
        total: 0, user: 0, assistant: 0, toolCalls: 0, toolResults: 0, errors: 0,
      };
      utcQH.total += entry.role === "user" || entry.role === "assistant" ? 1 : 0;
      if (entry.role === "user") utcQH.user += 1;
      else if (entry.role === "assistant") utcQH.assistant += 1;
      utcQH.toolCalls += entry.toolNames.length;
      utcQH.toolResults += entry.toolResultCounts.total;
      utcQH.errors += entry.toolResultCounts.errors;
      if (entry.stopReason && errorStopReasons.has(entry.stopReason)) utcQH.errors += 1;
      utcQuarterHourMessageMap.set(quarterBucket.key, utcQH);
    }
    ...
    if (ts !== undefined) {
      ...
      const quarterBucket = getUtcQuarterHourBucketKey(date);
      const utcQHToken = utcQuarterHourTokenMap.get(quarterBucket.key) ?? {
        date: quarterBucket.date, quarterIndex: quarterBucket.quarterIndex,
        input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0, totalCost: 0,
      };
      utcQHToken.input += usageTotals.input;
      utcQHToken.output += usageTotals.output;
      utcQHToken.cacheRead += usageTotals.cacheRead;
      utcQHToken.cacheWrite += usageTotals.cacheWrite;
      utcQHToken.totalTokens += usageTotals.totalTokens;
      utcQHToken.totalCost += usageTotals.totalCost;
      utcQuarterHourTokenMap.set(quarterBucket.key, utcQHToken);
      ...
      if (entry.provider || entry.model) {
        ...
        const dailyModel = dailyModelUsageMap.get(dailyModelKey) ?? {
          date: dayKey, provider: entry.provider, model: entry.model, tokens: 0, cost: 0, count: 0,
    ...
    if (entry.provider || entry.model) {
      ...
      const modelUsage = modelUsageMap.get(modelKey) ?? {
        provider: entry.provider, model: entry.model, count: 0, totals: emptyTotals(),
  ...
  const dailyBreakdown: SessionDailyUsage[] = Array.from(dailyMap.entries())
    ...
    .toSorted((a, b) => a.date.localeCompare(b.date));
  const dailyMessageCounts: SessionDailyMessageCounts[] = Array.from(dailyMessageMap.values())
    .toSorted((a, b) => a.date.localeCompare(b.date));
  const utcQuarterHourMessageCounts: SessionUtcQuarterHourMessageCounts[] = Array.from(utcQuarterHourMessageMap.values())
    .toSorted((a, b) => a.date.localeCompare(b.date) || a.quarterIndex - b.quarterIndex);
  const utcQuarterHourTokenUsage: SessionUtcQuarterHourTokenUsage[] = Array.from(utcQuarterHourTokenMap.values())
    .toSorted((a, b) => a.date.localeCompare(b.date) || a.quarterIndex - b.quarterIndex);
  const dailyLatency: SessionDailyLatency[] = Array.from(dailyLatencyMap.entries())
    ...
    .filter((e): e is SessionDailyLatency => Boolean(e))
    .toSorted((a, b) => a.date.localeCompare(b.date));
  ...
  const modelUsage = Array.from(modelUsageMap.values()).toSorted((a, b) => {
    ...
    return costDiff !== 0 ? costDiff : (b.totals?.totalTokens ?? 0) - (a.totals?.totalTokens ?? 0);
  });
  ...
  return {
    ...
    durationMs: firstActivity !== undefined && lastActivity !== undefined ? Math.max(0, lastActivity - firstActivity) : undefined,
    activityDates: Array.from(activityDatesSet).toSorted(),
    dailyBreakdown, dailyMessageCounts,
    utcQuarterHourMessageCounts: utcQuarterHourMessageCounts.length ? utcQuarterHourMessageCounts : undefined,
    utcQuarterHourTokenUsage: utcQuarterHourTokenUsage.length ? utcQuarterHourTokenUsage : undefined,
    dailyLatency: dailyLatency.length ? dailyLatency : undefined,
    dailyModelUsage: dailyModelUsage.length ? dailyModelUsage : undefined,
    messageCounts, toolUsage,
...
async function scanUsageFileForCache(params: {
  ...
  const appendOnlyPrevious =
    ...
    (!params.includeSessionSummary || params.previous.transcriptEntries)
      ? params.previous
      : undefined;
  const totals = emptyTotals();
  ...
  await scanTranscriptFile({
    ...
    onEntry: (entry) => {
      ...
      if (entry.usage) {
        ...
        if (ts !== undefined) {
          countedRecords += 1;
          usageEntries.push({ timestamp: ts, provider: entry.provider, model: entry.model, ...entryTotals });
        }
      }
      transcriptEntries?.push({
        timestamp: ts, role: entry.role, durationMs: entry.durationMs,
        provider: entry.provider, model: entry.model, stopReason: entry.stopReason,
        toolNames: entry.toolNames, toolResultCounts: entry.toolResultCounts,
  ...
  const sessionSummary =
    ...
    (params.includeSessionSummary || appendOnlyPrevious?.sessionSummary)
      ? (buildSessionCostSummaryFromCacheEntry({
          entry: {
            filePath: params.file.filePath, size: params.file.size, mtimeMs: params.file.mtimeMs,
            pricingFingerprint, scannedAt: Date.now(), parsedRecords, countedRecords,
            usageEntries, transcriptEntries: combinedTranscriptEntries, totals, sessionId,
          },
          sessionId, sessionFile: params.file.filePath,
  ...
  if (appendOnlyPrevious && startOffset !== undefined) {
    ...
    return {
      ...appendOnlyPrevious,
      size: params.file.size, mtimeMs: params.file.mtimeMs,
      pricingFingerprint, scannedAt: Date.now(),
      ...
      totals: previousTotals, sessionSummary,
    };
  }
  return {
    filePath: params.file.filePath, size: params.file.size, mtimeMs: params.file.mtimeMs,
    pricingFingerprint, scannedAt: Date.now(), parsedRecords, countedRecords,
    usageEntries, transcriptEntries: combinedTranscriptEntries, totals, sessionId, sessionSummary,
  };
}
...
async function refreshCostUsageCacheForPath(params?: {
  ...
  try {
    ...
    const maxFiles = params?.maxFiles !== undefined && Number.isFinite(params.maxFiles) && params.maxFiles > 0
      ? Math.floor(params.maxFiles) : undefined;
    const staleFiles = getUsageCostStaleFiles({
      cache, files: refreshFiles, pricingFingerprint, sessionSummaryFiles,
    })
      .toSorted((a, b) => {
    ...
    for (const file of staleFiles) {
      cache.files[file.filePath] = await scanUsageFileForCache({
        file, config: params?.config, resolveCost,
  ...
  try {
    ...
    const latestArchive = entries
      ...
      .toSorted((a, b) => {
        ...
        const tsB = parseSessionArchiveTimestamp(b, "deleted") ?? parseSessionArchiveTimestamp(b, "reset") ?? 0;
...
export async function loadCostUsageSummary(params?: {
  ...
  for (const file of files) {
    await scanUsageFile({
      filePath: file.filePath, config: params?.config, resolveCost,
      onEntry: (entry) => {
        const ts = entry.timestamp?.getTime();
        ...
        if (entry.costBreakdown?.total !== undefined) applyCostBreakdown(bucket, entry.costBreakdown);
        else applyCostTotal(bucket, entry.costTotal);
        dailyMap.set(dayKey, bucket);
        applyUsageTotals(totals, entry.usage);
        if (entry.costBreakdown?.total !== undefined) applyCostBreakdown(totals, entry.costBreakdown);
...
export async function loadCostUsageSummaryFromCache(params: {
  ...
  const staleFiles = getUsageCostStaleFiles({ cache, files, pricingFingerprint });
  if (params.requestRefresh !== false && staleFiles.length > 0) {
    const cachedFiles = countUsableUsageCostCacheFiles({ cache, files, pricingFingerprint });
    if (params.refreshMode === "sync-when-empty" && cachedFiles === 0) {
      const result = await refreshCostUsageCache({ config: params.config, agentId: params.agentId, startMs: params.startMs });
      [cache, files] = await Promise.all([
        readUsageCostCache(cachePath),
        listUsageCountedTranscriptFiles(params.agentId),
      ]);
      if (result === "refreshed") {
        const remainingStaleFiles = getUsageCostStaleFiles({ cache, files, pricingFingerprint });
  ...
  return buildCostUsageSummaryFromCache({
    cache, files, startMs: params.startMs, endMs: params.endMs,
    pricingFingerprint, refreshing: usageCostRefreshes.has(cachePath) || refreshRunning,
  });
}
...
export async function loadSessionCostSummaryFromCache(params: {
  ...
  let stale = !file || !isUsageCostCacheEntryFresh({ entry, file, pricingFingerprint, requireSessionSummary: true });
  let refreshRequested = false;
  if (params.requestRefresh !== false && stale) {
    if (params.refreshMode === "sync-when-empty") {
      const result = await refreshCostUsageCache({ config: params.config, agentId: params.agentId, sessionFiles: [params.sessionFile] });
      if (result === "refreshed") {
        [cache, stats] = await Promise.all([
          readUsageCostCache(cachePath),
          fs.promises.stat(params.sessionFile).catch(() => null),
        ]);
        file = stats ? { filePath: params.sessionFile, size: stats.size, mtimeMs: stats.mtimeMs } : undefined;
        entry = cache.files[params.sessionFile];
        stale = !file || !isUsageCostCacheEntryFresh({ entry, file, pricingFingerprint, requireSessionSummary: true });
      } else {
        requestCostUsageCacheRefresh({ config: params.config, agentId: params.agentId, sessionFiles: [params.sessionFile] });
  ...
  if (!summary && params.refreshMode === "sync-when-empty") {
    summary = await loadSessionCostSummary({
      sessionId: params.sessionId, sessionEntry: params.sessionEntry,
      sessionFile: params.sessionFile, config: params.config, agentId: params.agentId,
      startMs: params.startMs, endMs: params.endMs,
    });
  }
  if (summary && params.startMs !== undefined && params.endMs !== undefined && !isSessionSummaryContainedInRange(summary, params.startMs, params.endMs)) {
    summary = entry
      ? buildSessionCostSummaryFromCacheEntry({ entry, sessionId: params.sessionId, sessionFile: params.sessionFile, startMs: params.startMs, endMs: params.endMs })
      : params.refreshMode === "sync-when-empty"
        ? await loadSessionCostSummary({ sessionId: params.sessionId, sessionEntry: params.sessionEntry, sessionFile: params.sessionFile, config: params.config, agentId: params.agentId, startMs: params.startMs, endMs: params.endMs })
        : null;
  }
  return {
    summary,
    cacheStatus: {
      ...
      cachedFiles: stale ? 0 : 1, pendingFiles: stale ? 1 : 0, staleFiles: stale ? 1 : 0,
...
export async function loadSessionCostSummariesFromCache(params: {
  ...
  const cachePath = resolveUsageCostCachePath(params.agentId);
  const pricingFingerprint = resolveUsageCostPricingFingerprint(params.config);
  const statTasks = params.sessions.map((session) => async () => await fs.promises.stat(session.sessionFile).catch(() => null));
  const statsPromise = runTasksWithConcurrency({ tasks: statTasks, limit: USAGE_COST_TRANSCRIPT_STAT_CONCURRENCY }).then(({ results }) => results);
  const [cache, stats, refreshRunning] = await Promise.all([readUsageCostCache(cachePath), statsPromise, isUsageCostCacheRefreshRunning(cachePath)]);
  const staleFiles = new Set<string>();
  let cachedFiles = 0;
  const summaries = params.sessions.map((session, index) => {
    const stat = stats[index];
    const file = stat ? { filePath: session.sessionFile, size: stat.size, mtimeMs: stat.mtimeMs } : undefined;
    const entry = cache.files[session.sessionFile];
    const stale = !file || !isUsageCostCacheEntryFresh({ entry, file, pricingFingerprint, requireSessionSummary: true });
    if (stale) { staleFiles.add(session.sessionFile); return null; }
    cachedFiles += 1;
    const summary = entry?.sessionSummary ?? null;
    if (summary && params.startMs !== undefined && params.endMs !== undefined && !isSessionSummaryContainedInRange(summary, params.startMs, params.endMs)) {
      return entry ? buildSessionCostSummaryFromCacheEntry({ entry, sessionId: session.sessionId, sessionFile: session.sessionFile, startMs: params.startMs, endMs: params.endMs }) : null;
    }
    return summary;
  });
  const refreshRequested = params.requestRefresh !== false && staleFiles.size > 0;
  if (refreshRequested) {
    requestCostUsageCacheRefresh({ config: params.config, agentId: params.agentId, sessionFiles: [...staleFiles] });
  }
  ...
  return {
    ...
    cacheStatus: {
      ...
      cachedFiles, pendingFiles: staleFileCount, staleFiles: staleFileCount,
      refreshedAt: cache.updatedAt || undefined,
    },
  };
}
...
export function requestCostUsageCacheRefresh(params?: {
  ...
  const state: UsageCostRefreshState = {
    agentId: params?.agentId, cachePath, config: params?.config,
    fullRefreshRequested: false, pendingSessionFiles: new Set(),
    running: false, sessionsDir: path.dirname(cachePath),
  };
...
function mergeUsageCostRefreshRequest(
  ...
  params?: { config?: OpenClawConfig; agentId?: string; sessionFiles?: string[] },
): void {
  if (params?.config) state.config = params.config;
  if (params?.agentId) state.agentId = params.agentId;
  if (!params?.sessionFiles) { state.fullRefreshRequested = true; return; }
  for (const sessionFile of params.sessionFiles) state.pendingSessionFiles.add(sessionFile);
}

function scheduleUsageCostRefresh(refreshKey: string, state: UsageCostRefreshState, delayMs = 0): void {
...
async function runQueuedUsageCostRefresh(refreshKey: string, state: UsageCostRefreshState): Promise<void> {
  state.running = true;
  let retryDelayMs = 0;
  try {
    while (state.fullRefreshRequested || state.pendingSessionFiles.size > 0) {
      ...
      const result = await refreshCostUsageCacheForPath({
        cachePath: state.cachePath, config: state.config, agentId: state.agentId,
...
export async function discoverAllSessions(params?: {
  ...
  for (const file of files) {
    ...
    const shouldReplace = !existing || (isPrimaryTranscript && !existingIsPrimary) || (isPrimaryTranscript === existingIsPrimary && file.mtimeMs >= existing.mtime);
    if (shouldReplace) {
      discovered.set(sessionId, { sessionId, sessionFile: filePath, mtime: file.mtimeMs, firstUserMessage: firstUserMessage ?? existing?.firstUserMessage });
      continue;
    }
...
export async function loadSessionCostSummary(params: {
  ...
  const sessionFile = resolveExistingUsageSessionFile(params as Parameters<typeof resolveExistingUsageSessionFile>[0]);
  ...
  const dailyLatencyMap = new Map<string, number[]>();
  const dailyModelUsageMap = new Map<string, SessionDailyModelUsage>();
  const messageCounts: SessionMessageCounts = { total: 0, user: 0, assistant: 0, toolCalls: 0, toolResults: 0, errors: 0 };
  const toolUsageMap = new Map<string, number>();
  const modelUsageMap = new Map<string, SessionModelUsage>();
  const errorStopReasons = new Set(["error", "aborted", "timeout"]);
  const latencyValues: number[] = [];
  ...
  await scanTranscriptFile({
    filePath: sessionFile, config: params.config, resolveCost,
    onEntry: (entry) => {
      const ts = entry.timestamp?.getTime();
      if (params.startMs !== undefined && ts !== undefined && ts < params.startMs) return;
      if (params.endMs !== undefined && ts !== undefined && ts > params.endMs) return;
      if (ts !== undefined) {
        if (!firstActivity || ts < firstActivity) firstActivity = ts;
        if (!lastActivity || ts > lastActivity) lastActivity = ts;
      }
      if (entry.role === "user") { messageCounts.user += 1; messageCounts.total += 1; if (entry.timestamp) lastUserTimestamp = entry.timestamp.getTime(); }
      if (entry.role === "assistant") {
        messageCounts.assistant += 1; messageCounts.total += 1;
        const tsLocal = entry.timestamp?.getTime();
        if (tsLocal !== undefined) {
          const latencyMs = entry.durationMs ?? (lastUserTimestamp !== undefined ? Math.max(0, tsLocal - lastUserTimestamp) : undefined);
      ...
      if (entry.toolNames.length > 0) { messageCounts.toolCalls += entry.toolNames.length; for (const name of entry.toolNames) toolUsageMap.set(name, (toolUsageMap.get(name) ?? 0) + 1); }
      if (entry.toolResultCounts.total > 0) { messageCounts.toolResults += entry.toolResultCounts.total; messageCounts.errors += entry.toolResultCounts.errors; }
      ...
      if (entry.timestamp) {
        ...
        activityDatesSet.add(dayKey);
        const daily = dailyMessageMap.get(dayKey) ?? { date: dayKey, total: 0, user: 0, assistant: 0, toolCalls: 0, toolResults: 0, errors: 0 };
        daily.total += entry.role === "user" || entry.role === "assistant" ? 1 : 0;
        if (entry.role === "user") daily.user += 1; else if (entry.role === "assistant") daily.assistant += 1;
        daily.toolCalls += entry.toolNames.length; daily.toolResults += entry.toolResultCounts.total; daily.errors += entry.toolResultCounts.errors;
        if (entry.stopReason && errorStopReasons.has(entry.stopReason)) daily.errors += 1;
        dailyMessageMap.set(dayKey, daily);
      }
      ...
      if (entry.timestamp) {
        ...
        dailyMap.set(dayKey, { tokens: existing.tokens + entryTokenTotals.componentTotal, cost: existing.cost + entryCost });
        if (entry.provider || entry.model) {
          const modelKey = `${dayKey}::${entry.provider ?? "unknown"}::${entry.model ?? "unknown"}`;
          const dailyModel = dailyModelUsageMap.get(modelKey) ?? { date: dayKey, provider: entry.provider, model: entry.model, tokens: 0, cost: 0, count: 0 } as SessionDailyModelUsage;
          dailyModel.tokens += entryTokenTotals.componentTotal; dailyModel.cost += entryCost; dailyModel.count += 1;
          dailyModelUsageMap.set(modelKey, dailyModel);
        }
      }
      if (entry.provider || entry.model) {
        const key = `${entry.provider ?? "unknown"}::${entry.model ?? "unknown"}`;
        const existing = modelUsageMap.get(key) ?? { provider: entry.provider, model: entry.model, count: 0, totals: emptyTotals() } as SessionModelUsage;
        existing.count += 1;
        applyUsageTotals(existing.totals, entry.usage);
        if (entry.costBreakdown?.total !== undefined) applyCostBreakdown(existing.totals, entry.costBreakdown);
  ...
  const dailyBreakdown: SessionDailyUsage[] = Array.from(dailyMap.entries())
    .map(([date, data]) => ({ date, tokens: data.tokens, cost: data.cost }))
    .toSorted((a, b) => a.date.localeCompare(b.date));
  const dailyMessageCounts: SessionDailyMessageCounts[] = Array.from(dailyMessageMap.values()).toSorted((a, b) => a.date.localeCompare(b.date));
  const dailyLatency: SessionDailyLatency[] = Array.from(dailyLatencyMap.entries())
    .map(([date, values]) => { const stats = computeLatencyStats(values); return stats ? Object.assign({ date }, stats) : null; })
    .filter((e): e is SessionDailyLatency => Boolean(e)).toSorted((a, b) => a.date.localeCompare(b.date));
  const dailyModelUsage = Array.from(dailyModelUsageMap.values()).toSorted((a, b) => a.date.localeCompare(b.date) || b.cost - a.cost);
  const toolUsage: SessionToolUsage | undefined = toolUsageMap.size ? {
    totalCalls: Array.from(toolUsageMap.values()).reduce((sum, count) => sum + count, 0), uniqueTools: toolUsageMap.size,
    tools: Array.from(toolUsageMap.entries()).map(([name, count]) => ({ name, count })).toSorted((a, b) => b.count - a.count),
  } : undefined;
  ...
  return {
    sessionId: params.sessionId, sessionFile, firstActivity, lastActivity,
    durationMs: firstActivity !== undefined && lastActivity !== undefined ? Math.max(0, lastActivity - firstActivity) : undefined,
    activityDates: Array.from(activityDatesSet).toSorted(),
    dailyBreakdown, dailyMessageCounts,
    dailyLatency: dailyLatency.length ? dailyLatency : undefined,
    dailyModelUsage: dailyModelUsage.length ? dailyModelUsage : undefined,
    messageCounts, toolUsage, modelUsage,
    latency: computeLatencyStats(latencyValues), ...totals,
  };
}
...
export async function loadSessionUsageTimeSeries(params: {
  ...
  if (params.maxPoints !== undefined && params.maxPoints !== null) {
    if (!Number.isFinite(params.maxPoints) || params.maxPoints <= 0) return { sessionId: params.sessionId, points: [] };
  }
  ...
  await scanUsageFile({
    ...
    onEntry: (entry) => {
      ...
      cumulativeCost += cost;
      points.push({ timestamp: ts, input, output, cacheRead, cacheWrite, totalTokens, cost, cumulativeTokens, cumulativeCost });
    },
  });
  ...
  if (sortedPoints.length > maxPoints) {
    ...
    for (let i = 0; i < sortedPoints.length; i += step) {
      ...
      let bucketInput = 0, bucketOutput = 0, bucketCacheRead = 0, bucketCacheWrite = 0, bucketTotalTokens = 0, bucketCost = 0;
      for (const point of bucket) { bucketInput += point.input; bucketOutput += point.output; bucketCacheRead += point.cacheRead; bucketCacheWrite += point.cacheWrite; bucketTotalTokens += point.totalTokens; bucketCost += point.cost; }
      downsampledCumulativeTokens += bucketTotalTokens; downsampledCumulativeCost += bucketCost;
      downsampled.push({ timestamp: bucketLast.timestamp, input: bucketInput, output: bucketOutput, cacheRead: bucketCacheRead, cacheWrite: bucketCacheWrite, totalTokens: bucketTotalTokens, cost: bucketCost, cumulativeTokens: downsampledCumulativeTokens, cumulativeCost: downsampledCumulativeCost });
    }
...
export async function loadSessionLogs(params: {
  ...
  for await (const parsed of readJsonlRecords(sessionFile)) {
    try {
      ...
      const toolName = normalizeOptionalString(rawToolName as string);
      ...
      if (role === "assistant") {
        ...
        if (usage) {
          ...
          const breakdown = extractCostBreakdown(usageRaw as UsageLike);
          if (breakdown?.total !== undefined) cost = breakdown.total;
          else {
            const costConfig = resolveCost({ provider: message.provider as string | undefined, model: message.model as string | undefined });
            cost = estimateUsageCost({ usage, cost: costConfig });
          }
        }
      }
      logs.push({ timestamp, role: role as SessionLogEntry["role"], content, tokens, cost });
  ...
  return sortedLogs.length > limit ? sortedLogs.slice(-limit) : sortedLogs;
}
