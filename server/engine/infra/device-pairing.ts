...
import { normalizeUniqueSingleOrTrimmedStringList } from "./string-normalization.js";
import {
...
/** 设备可在不更改审批或 token 状态的情况下刷新的元数据字段 */
...
/** 配对列表 API 返回的组合待处理/配对视图 */
export type DevicePairingList = {
  pending: DevicePairingPendingRequest[];
  paired: PairedDevice[];
};

/** 所有者审批和 bootstrap 审批流程的授权失败类别 */
...
export function listApprovedPairedDeviceRoles(
  ...
  // 已批准角色来自配对记录本身。这是所有者批准的持久契约，
  // 独立于任何当前活跃的 tokens
  return mergeRoles(device.roles, device.role) ?? [];
}

/** 列出活跃 token 角色，受限于持久批准的配对角色 */
export function listEffectivePairedDeviceRoles(
  ...
  if (activeTokenRoles && activeTokenRoles.length > 0) {
    // 有效角色是活跃 token 角色，受限于批准的配对契约。
    // 迷失的 token 条目不得授予新访问权限
  ...
  // token 条目是权威的。无 token 的旧记录失败关闭，
  // 这样粘性历史角色字段就不能在 token 迁移后保留访问权限
  return [];
}

/** 返回配对设备是否当前拥有某个角色的活跃 token */
...
function refreshPendingDevicePairingRequest(
  ...
  return {
    ...
    // 如果任一请求是交互式的，保持待处理请求可见以供审批
    silent: Boolean(existing.silent && incoming.silent),
    isRepair: existing.isRepair || isRepair,
    // 保留原始创建时间戳，使重连不能提升此请求的队列位置。
    // 在此处使用 Date.now() 会让攻击者静默刷新最近性并赢得隐式 --latest 审批竞争
    ts: existing.ts,
  };
}
...
/** 通过可选的调用方 scope 检查批准待处理请求（用于 operator 授权） */
...
/** 通过有界的 bootstrap profile 交接批准待处理请求 */
...
/** 拒绝待处理请求并撤销该设备的匹配 bootstrap tokens */
...
/** 移除一个已批准的配对设备角色，同时保留不相关的角色 tokens */
...
/** 汇总 token 元数据而不暴露 bearer token 字符串 */
...
/** 验证设备角色 token，将其范围限定到批准基线，并标记最后使用 */
...
/** 返回某个角色的可重用 token，或在批准的 scope 基线内签发一个 */
...
function resolveDeviceTokenUpdateContext(params: {
  ...
  // token 签发和轮换必须保持在配对审批为该设备记录的角色集内
...
/** 在可选的调用方 scope 授权后撤销一个活跃角色 token */
