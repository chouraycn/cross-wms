// 包更新父进程与 post-install doctor 子进程之间的 IPC 契约。
// 直接移植自 openclaw/src/infra/update-doctor-result.ts，
// 依赖的 ./tmp-openclaw-dir.js 已在 cross-wms 中存在。
...
export async function writeUpdatePostInstallDoctorResult(params: {
  ...
  // Advisory details 可能包含 config 派生的 ID；已存在的路径必须 fail closed。
