...
function resolveAdapterForConversation(ref: ConversationRef): SessionBindingAdapter | null {
  return resolveAdapterForChannelAccount({ channel: ref.channel, accountId: ref.accountId });
}
    ...
    bind: async (input) => {
      ...
      if (!adapter) {
        throw new SessionBindingError(
          ...
          { channel: normalizedConversation.channel, accountId: normalizedConversation.accountId },
        );
      }
      if (!adapter.bind) {
        throw new SessionBindingError(
      ...
      if (!supportedPlacements.includes(placement)) {
        throw new SessionBindingError(
          ...
          { channel: normalizedConversation.channel, accountId: normalizedConversation.accountId, placement },
        );
      }
      const bound = await adapter.bind({ ...input, conversation: normalizedConversation, placement });
      if (!bound) {
        throw new SessionBindingError(
