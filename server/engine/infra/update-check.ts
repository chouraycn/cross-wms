// 计算 OpenClaw 安装的 git、依赖与 registry 更新状态。
// 移植自 openclaw/src/infra/update-check.ts。
// 降级实现：
//   - runCommandWithTimeout 来自 ../process/exec.js（未移植），使用本地 child_process.spawn 实现
//   - fetchWithTimeout 来自 ../utils/fetch-timeout.js，cross-wms 中位于 ./fetch-timeout.js
//   - detectPackageManager 来自 ./detect-package-manager.js（已在 cross-wms 中移植）
//   - compareOpenClawReleaseVersions 来自 ./npm-registry-spec.js（已存在）
//   - compareComparableSemver/parseComparableSemver 来自 ./semver-compare.js（已存在）
//   - channelToNpmTag/UpdateChannel 来自 ./update-channels.js（已在 cross-wms 中移植）
...
/**
 * 本地 runCommandWithTimeout 实现。
 * 降级实现：openclaw 中从 ../process/exec.js 导入，
 * cross-wms 未移植该模块，这里使用 child_process.spawn 提供等价功能。
 */
async function runCommandWithTimeout(
  ...
  return new Promise((resolve) => {
    ...
    let stdoutBytes = 0;
    let stderrBytes = 0;
    const truncated = (buf: string, currentBytes: number, chunk: Buffer): string => {
    ...
    child.stdout?.on("data", (chunk: Buffer) => {
      ...
      stdout = truncated(stdout, stdoutBytes - chunk.length, chunk);
      stdoutBytes = Math.min(stdoutBytes, maxOutputBytes);
    });
    child.stderr?.on("data", (chunk: Buffer) => {
      stderrBytes += chunk.length;
      stderr = truncated(stderr, stderrBytes - chunk.length, chunk);
      stderrBytes = Math.min(stderrBytes, maxOutputBytes);
    });
    ...
    child.on("close", (code) => {
      ...
      resolve({ stdout, stderr: stderr || "", code: code ?? null });
    });
  });
}
