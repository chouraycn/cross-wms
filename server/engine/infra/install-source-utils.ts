...
import { runCommandWithTimeout } from "../plugins/_parent__process__exec.js";
import { resolveUserPath } from "./_fs-safe-stubs.js";
...
export async function resolveNpmSpecMetadata(params: { spec: string; timeoutMs?: number }): Promise<
  | { ok: true; metadata: NpmSpecResolution }
  | { ok: false; error: string }
> {
  const res = await runCommandWithTimeout(
    ["npm", "view", params.spec, "name", "version", "dist.integrity", "dist.shasum", "openclaw", "--json"],
    { timeoutMs: Math.max(params.timeoutMs ?? 60_000, 60_000), env: createNpmMetadataEnv() },
  );
  if (res.code !== 0) {
    const raw = res.stderr.trim() || res.stdout.trim();
    if (/E404|is not in this registry/i.test(raw)) {
      return { ok: false, error: `Package not found on npm: ${params.spec}. See https://docs.openclaw.ai/tools/plugin for installable plugins.` };
    }
...
> {
  ...
  if (!(await pathExists(resolved))) return { ok: false, error: `archive not found: ${resolved}` };
  if (!resolveArchiveKind(resolved)) return { ok: false, error: `unsupported archive: ${resolved}` };
...
function normalizeNpmPackEntry(entry: unknown): { filename?: string; metadata: NpmSpecResolution } | null {
  ...
  const resolvedSpec = (name && version ? `${name}@${version}` : undefined) ?? (id ? parseResolvedSpecFromId(id) : undefined);
  return {
    filename: normalizeOptionalString(rec.filename),
    metadata: { name, version, resolvedSpec, integrity: normalizeOptionalString(rec.integrity), shasum: normalizeOptionalString(rec.shasum) },
  };
}
...
async function findPackedArchiveInDir(cwd: string): Promise<string | undefined> {
  ...
  const sortedByMtime = await Promise.all(
    archives.map(async (entry) => ({ name: entry.name, mtimeMs: (await fs.stat(path.join(cwd, entry.name))).mtimeMs })),
  );
...
export async function packNpmSpecToArchive(params: {
  ...
  | { ok: true; archivePath: string; metadata: NpmSpecResolution }
  | { ok: false; error: string }
> {
  const res = await runCommandWithTimeout(
    ["npm", "pack", params.spec, "--ignore-scripts", "--json"],
    { timeoutMs: Math.max(params.timeoutMs, 300_000), cwd: params.cwd, env: createNpmMetadataEnv({ npmConfigCwd: params.cwd }) },
  );
  if (res.code !== 0) {
    const raw = res.stderr.trim() || res.stdout.trim();
    if (/E404|is not in this registry/i.test(raw)) {
  ...
  if (!(await pathExists(archivePath))) {
    ...
    if (!fallbackPacked) return { ok: false, error: "npm pack produced no archive" };
    archivePath = path.join(params.cwd, fallbackPacked);
  }
  return { ok: true, archivePath, metadata: parsedJson?.metadata ?? {} };
}
...
export async function resolveNpmPackArchiveMetadata(params: {
  ...
  | { ok: true; archivePath: string; tarballName: string; metadata: NpmSpecResolution }
  | { ok: false; error: string }
> {
  ...
  const archiveMetadataTimeoutMs = archiveStat && archiveStat.size > 100 * 1024 * 1024 ? 300_000 : 60_000;
  const res = await runCommandWithTimeout(
    ["npm", "pack", archivePath, "--ignore-scripts", "--dry-run", "--json"],
    { timeoutMs: Math.max(params.timeoutMs ?? archiveMetadataTimeoutMs, archiveMetadataTimeoutMs), env: createNpmMetadataEnv() },
  );
  if (res.code !== 0) {
    return { ok: false, error: `npm pack metadata read failed: ${res.stderr.trim() || res.stdout.trim()}` };
  }
  const parsedJson = parseNpmPackJsonOutput(res.stdout || "");
  ...
  return { ok: true, archivePath, tarballName: parsedJson.filename ?? path.basename(archivePath), metadata: parsedJson.metadata };
}
