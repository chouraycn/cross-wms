// 为本地存储配置 SQLite WAL 及相关 pragma。
// 注意：openclaw 中 MAX_TIMER_TIMEOUT_MS 来自 ../shared/number-coercion.js，
// 但 cross-wms 的 shared/number-coercion.ts 未导出该常量，
// 仅 infra/number-coercion.ts 导出，因此此处改为从 ./number-coercion.js 导入。
import childProcess from "node:child_process";
import fs from "node:fs";
import path from "node:path";
import type { DatabaseSync } from "node:sqlite";
import { MAX_TIMER_TIMEOUT_MS } from "./number-coercion.js";

// WAL 维护配置 SQLite 预写式日志并调度有界检查点，
// 防止状态数据库累积无界 WAL 文件。
...
/**
 * @deprecated 请使用 DEFAULT_SQLITE_WAL_CHECKPOINT_INTERVAL_MS。
 * 周期性检查点默认使用 PASSIVE。
 */
...
function readMountEntries(): MountEntry[] {
  try {
    ...
    // macOS/BSD 在 mount 输出中暴露文件系统类型名，而非 Linux superblock 魔数，
    // 因此保留此回退以处理命名文件系统类型。
  }
  try {
  ...
  if (process.platform === "win32") {
    ...
    if (isWindowsDrivePath(normalizedTargetPath)) {
      try {
        ...
        // 当父组件不可读时 Windows 可能拒绝 SMB 路径规范化。
        // 将无法分类的已打开数据库视为网络存储。
        return "rollback";
      }
    }
...
/** 以安全的锁重试/WAL 顺序配置每连接 SQLite pragma */
