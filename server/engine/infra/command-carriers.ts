...
const ENV_OPTIONS_WITH_VALUE = new Set(["-C","-P","-S","-s","-u","--argv0","--block-signal","--chdir","--default-signal","--ignore-signal","--split-string","--unset"]);
const ENV_SPLIT_STRING_OPTIONS = new Set(["-S", "-s", "--split-string"]);
const ENV_STANDALONE_OPTIONS = new Set(["-0", "-i", "--ignore-environment", "--null"]);
const SUDO_OPTIONS_WITH_VALUE = new Set(["-C","-D","-g","-h","-p","-R","-T","-U","-u","--chdir","--chroot","--close-from","--command-timeout","--group","--host","--other-user","--prompt","--role","--type","--user"]);
const SUDO_STANDALONE_OPTIONS = new Set(["-A","-B","-b","-E","-H","-i","-k","-N","-n","-P","-S","-s","--askpass","--background","--bell","--login","--no-update","--non-interactive","--preserve-env","--preserve-groups","--reset-home","--reset-timestamp","--set-home","--shell","--stdin"]);
const SUDO_NON_EXEC_OPTIONS = new Set(["-K","-l","-V","-v","-e","--edit","--help","--list","--remove-timestamp","--validate","--version"]);
...
function parseCarrierOptionToken(
  ...
  if (token.startsWith("--")) {
    ...
    if (standaloneOptions.has(name) || optionsWithValue.has(name) || nonExecutingOptions.has(name)) {
      const parsedOption: ParsedCarrierOption = { name, hasInlineValue: option.hasInlineValue };
  ...
  for (let index = 1; index < token.length; index += 1) {
    ...
    if (optionsWithValue.has(name)) {
      options.push({ name, hasInlineValue: index < token.length - 1, inlineValue: index < token.length - 1 ? token.slice(index + 1) : undefined });
      return options;
    }
...
function stripSudoEnvAssignmentsFromCommandArgv(executable: string, argv: string[]): string[] | null {
...
function findParsedCarrierOption(options: readonly ParsedCarrierOption[], names: ReadonlySet<string>): ParsedCarrierOption | undefined {
  return options.find((option) => names.has(option.name));
}

function resolveEnvSplitPayload(payload: string, trailingArgv: string[], depth: number): string[] | null {
...
export function parseEnvInvocationPrelude(argv: string[], depth = 0): ParsedEnvInvocationPrelude | null {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (token.startsWith("-")) {
      ...
      if (splitStringOption) {
        ...
        return splitArgv ? { assignmentKeys, commandIndex: trailingIndex, splitArgv, usesModifiers } : null;
      }
...
function resolveSudoLikeCarriedArgv(argv: string[]): string[] | null {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    const option = parseCarrierOptionToken(token, standaloneOptions, optionsWithValue, executable === "sudo" ? SUDO_NON_EXEC_OPTIONS : undefined);
    if (!option) { return null; }
    const consumeNextValue = knownCarrierOptionConsumesNextValue(option, optionsWithValue, executable === "sudo" ? SUDO_NON_EXEC_OPTIONS : undefined);
...
export function resolveCarrierCommandArgv(argv: string[], depth = 0, options?: { includeExec?: boolean }): string[] | null {
  const executable = normalizeExecutableToken(argv[0] ?? "");
  switch (executable) {
    case "env": return resolveEnvCarriedArgv(argv, depth);
    case "command": case "builtin": return resolveCommandBuiltinCarriedArgv(argv);
    case "sudo": case "doas": return resolveSudoLikeCarriedArgv(argv);
