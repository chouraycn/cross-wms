...
// 识别可携带隐藏命令载荷的 wrapper 命令。
//
// 降级策略：源文件依赖 ../utils/shell-argv.js，cross-wms 中该模块位于
// ./shell-argv.js（infra 目录内），此处调整导入路径。其余依赖
//（./exec-wrapper-tokens.js、./inline-option-token.js）已在 cross-wms 就绪。
