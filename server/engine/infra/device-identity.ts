...
/** Gateway/设备 Ed25519 身份，用于 APNs 中继和 gateway 认证。 */
...
// Swift 存储原始 Ed25519 密钥字节；Node crypto 需要围绕它们的 DER/PEM 包装。
    ...
    if (normalized?.kind === "identity") {
      if (normalized.stored) {
        try {
          ...
          // 即使尽力规范化失败，也继续使用已识别的 OpenClaw 密钥材料。
        }
      }
      return normalized.identity;
    }
    if (normalized?.kind === "recognized-invalid") {
      // 避免覆盖可识别但无效的身份文件；调用方仍可使用新密钥。
...
/** 使用 PEM Ed25519 私钥签名 UTF-8 负载并返回 base64url 字节。 */
...
/** 将 PEM 或原始 base64/base64url 公钥规范化为标准原始 base64url 字节。 */
...
/** 从 PEM 或原始 base64/base64url 公钥材料派生稳定的设备 ID。 */
...
/** 将 PEM Ed25519 公钥导出为标准原始 base64url 字节。 */
export function publicKeyRawBase64UrlFromPem(publicKeyPem: string): string {
  return base64UrlEncode(derivePublicKeyRaw(publicKeyPem));
}

/** 验证 UTF-8 负载签名与 PEM 或原始 base64/base64url 公钥材料匹配。 */
