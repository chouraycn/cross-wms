...
export function stripTargetTopicSuffix(
  ...
  const trimmed = raw.trim();
  const numericTopicMatch = options.allowNumericShorthand
    ? /^(-?\d+):(\d+)$/.exec(trimmed)
    : null;
...
function resolveChannelTargetProviderPrefix(
  ...
  if (!prefix || TARGET_KIND_PREFIXES.has(prefix)) return undefined;
...
export function validateTargetProviderPrefix(params: {
  ...
  if (!selectedChannel || selectedChannel === "last") return undefined;
  const prefixed = resolveChannelTargetProviderPrefix(params.to);
  if (!prefixed || prefixed.channel === selectedChannel) return undefined;
  return new Error(
