/**
 * Directive tag 辅助 — 从用户文本中解析内联指令标签
 *
 * 支持的指令：
 *   [[audio_as_voice]] — 标记文本需以语音形式输出
 *   [[reply_to_current]] — 回复当前消息
 *   [[reply_to:<id>]] — 回复指定 id 的消息
 *
 * 参考 openclaw/src/utils/directive-tags.ts
 */
...
function normalizeDirectiveWhitespace(text: string): string {
  // 提取 → 规范化正文 → 还原：
  // 将每个代码块（fenced ```/~~~ 和缩进 4 空格/Tab 的 indent-code）
  // 用哨兵分隔的占位符暂存，使正文正则不触碰它们。
...
/**
 * 从文本内容中去除内联指令标签，同时保留消息形状。
 * 去除后为空的文本保持空字符串以保留调用方语义。
 * 当没有文本部分改变时返回输入消息引用（包括原始 content 数组），
 * 在混合内容中复用未改变的文本部分引用，使 identity 相等性消费者
 * 避免不必要的变动。
 */
