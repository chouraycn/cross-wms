// 为安全的 project-local 安装构建 npm 环境覆盖。
...
/** 用于限定 npm 配置和缓存路径的 project-local 安装选项 */
...
/**
 * 为 OpenClaw 管理的安装构建绕过宿主新鲜度策略的 npm 参数。
 * 既有 npmrc 策略决定 before 或 min-release-age 哪个更安全。
 */
...
/**
 * 为 project-local 安装创建 npm 环境，清除全局/工作区配置，
 * 并加入 fetch、新鲜度、缓存和 POSIX script-shell 默认值。
 */
...
/** 当调用方 env 已固定 npm 生命周期脚本 shell 时返回 true */
...
/** 当存在可用 POSIX shell 时解析其绝对路径，用于 npm 生命周期脚本 */
...
/** 仅当调用方未配置 script-shell 时设置 npm 的 script-shell env */
