...
/** Resolved proxy configuration sourced from environment variables. */
...
export function readProxyFromEnv(env: NodeJS.ProcessEnv = process.env): ProxyEnvConfig {
  ...
  if (noProxy) {
    config.noProxy = noProxy.split(/[,\s]/).map((s) => s.trim()).filter(Boolean);
  }
  return config;
}

/** Return whether a hostname should bypass the proxy given a NO_PROXY list. */
export function shouldBypassProxy(hostname: string, noProxy: string[] = []): boolean {
  ...
  for (const entry of noProxy) {
    const normalized = entry.trim().toLowerCase().replace(/^\*\./, "").replace(/^\./, "");
    if (!normalized || normalized === "*") return true;
    if (lower === normalized || lower.endsWith("." + normalized)) return true;
  }
  return false;
}

/** Return whether any proxy is enabled in the given environment. */
