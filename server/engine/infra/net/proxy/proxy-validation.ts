...
// 移植自 openclaw/src/infra/net/proxy/proxy-validation.ts
// 降级：
//  - ../../../config/zod-schema.proxy.js (ProxyConfig) → ../../_runtime-stubs.js（已有 stub）
//  - ../../push-apns-http2.js → ../../push-apns-http2.js（已有降级实现）
//  - ../runtime-fetch.js → ../../runtime-fetch.js（已有降级实现）
//  - ../undici-runtime.js → ../undici-runtime.js（已移植）
//  - ./proxy-tls.js → ../../proxy-tls.js（已有 stub，返回 unknown 需强转）
...
export function resolveProxyValidationConfig(
  ...
  if (overrideUrl) {
    // cross-wms 的 proxy-tls.ts 是 stub（返回 unknown），需强转为 string | undefined。
...
export async function runProxyValidation(
  ...
  // cross-wms 的 proxy-tls.ts 是 stub（loadManagedProxyTlsOptions 返回 unknown），
  // 需强转为 ManagedProxyTlsOptions | undefined。
  let proxyTls: ManagedProxyTlsOptions | undefined;
  try {
    proxyTls = loadManagedProxyTlsOptions(config.proxyCaFile) as
