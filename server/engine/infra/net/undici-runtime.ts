...
// HTTP/1, timeout, proxy TLS, and IP-safe proxy policies.
...
function createIpSafeProxyClientFactory(): UndiciProxyClientFactory {
  return (origin, options) => {
    ...
    const clientOptions = isRecord(options)
...
/** Creates a direct undici Agent with HTTP/1-only dispatcher policy. */
...
/**
 * Creates an EnvHttpProxyAgent with proxy TLS, IP-safe proxy pools,
...
export function createHttp1EnvHttpProxyAgent(
  ...
  const managedTlsOptions = addActiveManagedProxyTlsOptions(options) as
    | object
    | undefined;
  return new EnvHttpProxyAgent(
    withHttp1OnlyDispatcherOptions(
      addIpSafeProxyClientFactory(managedTlsOptions ?? {}),
      timeoutMs,
      {
        connect: true,
        proxyTls: true,
...
export function createHttp1ProxyAgent(
  ...
  const normalized =
    typeof options === "string" || options instanceof URL
      ...
      : { ...options };
  // cross-wms 的 managed-proxy-undici.ts 是 stub（返回 unknown），需强转为 object。
  const managedTlsOptions = addActiveManagedProxyTlsOptions(normalized as object) as
    | object
    | undefined;
  return new ProxyAgent(
    withHttp1OnlyDispatcherOptions(
      addIpSafeProxyClientFactory(managedTlsOptions ?? normalized),
      timeoutMs,
      {
        proxyTls: true,
      },
