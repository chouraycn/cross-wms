...
function resolveShellWrapperScriptCandidatePath(params: {
  ...
  while (idx < argv.length) {
    ...
    if (!isPowerShell && /^-[^-]*c[^-]*$/i.test(token)) { return undefined; }
    if (token === "-s" || (!isPowerShell && /^-[^-]*s[^-]*$/i.test(token))) { return undefined; }
    if (hasDisqualifyingShellWrapperScriptOption(token)) { return undefined; }
    if (SHELL_WRAPPER_OPTIONS_WITH_VALUE.has(token)) { idx += 2; continue; }
    if (isPowerShell && POWERSHELL_OPTIONS_WITH_VALUE_RE.test(token)) { idx += 2; continue; }
    if (token.startsWith("-") || token.startsWith("+")) { idx += 1; continue; }
    break;
  }
...
function resolveShellWrapperPositionalArgvCandidatePath(params: {
  ...
  if (!isShellWrapperSegment(params.segment)) { return undefined; }
  const argv = params.segment.argv;
  if (!Array.isArray(argv) || argv.length < 4) { return undefined; }
  const wrapper = normalizeExecutableToken(argv[0] ?? "");
  if (!["ash", "bash", "dash", "fish", "ksh", "sh", "zsh"].includes(wrapper)) { return undefined; }
  const inlineMatch = resolveInlineCommandMatch(argv, POSIX_INLINE_COMMAND_FLAGS, {
    allowCombinedC: true,
  });
  if (inlineMatch.valueTokenIndex === null || !inlineMatch.command) { return undefined; }
  if (!isDirectShellPositionalCarrierCommand(inlineMatch.command)) { return undefined; }
  const carriedExecutable = argv
    .slice(inlineMatch.valueTokenIndex + 1)
    .map((token) => token.trim())
    .find((token) => token.length > 0);
  if (!carriedExecutable) { return undefined; }
  const carriedName = normalizeExecutableToken(carriedExecutable);
  if (isDispatchWrapperExecutable(carriedName) || isShellWrapperExecutable(carriedName)) { return undefined; }
...
function resolveSegmentAllowlistMatch(params: {
  ...
  const isShellWrapperInv = isShellWrapperSegment(allowlistSegment);
  ...
  const executableMatch = matchExecutableAllowlistForSegment({
    ...
    inlineCommand,
    isShellWrapperInvocation: isShellWrapperInv,
...
function evaluateSegments(
  ...
  const satisfied = segments.every((segment) => {
    ...
    const by = resolveSegmentSatisfaction({
      match, segment, effectiveArgv, context: params, allowSkills, skillBinTrust,
    });
    const inlineResult = resolveInlineCommandFallback({
      by, inlineCommand, context: params, inlineDepth,
    });
    if (inlineResult) {
...
function evaluateAuthorizationCandidate(params: {
  ...
  if (match) { return { match, satisfiedBy: "allowlist" }; }
  const satisfiedBy = resolveSegmentSatisfaction({
    match, segment: params.candidate.sourceSegment, effectiveArgv, context: params.context,
    allowSkills: params.allowSkills, skillBinTrust: params.skillBinTrust,
  });
...
function evaluateAuthorizationPlanGroup(params: {
  ...
  for (const candidate of params.group.candidates) {
    const result = evaluateAuthorizationCandidate({
      candidate, context: params.context, allowSkills: params.allowSkills, skillBinTrust: params.skillBinTrust,
    });
  ...
  return {
    evaluation: { allowlistSatisfied, allowlistMatches: matches, segmentAllowlistEntries, segmentSatisfiedBy },
    segments,
  };
}
...
function evaluateAuthorizationPlan(params: {
  ...
  const groupEvaluations: PlanGroupEvaluation[] = params.plan.groups.map((group) => {
    const { evaluation, segments } = evaluateAuthorizationPlanGroup({
      group, context: params.context, allowSkills, skillBinTrust,
    });
    return { analysis: { ok: true, segments }, evaluation, opToNext: group.opToNext ?? null };
  });
  return finalizeShellAllowlistEvaluations({ evaluations: groupEvaluations, authorizationPlan: params.plan });
}
...
export function evaluateExecAllowlist(
  ...
  if (!params.analysis.ok || params.analysis.segments.length === 0) {
    return { allowlistSatisfied: false, allowlistMatches, segmentAllowlistEntries, segmentSatisfiedBy };
  }
  ...
  return { allowlistSatisfied: true, allowlistMatches, segmentAllowlistEntries, segmentSatisfiedBy };
}
...
function addAllowAlwaysPattern(
  ...
  if (!exists) { out.push({ pattern, argPattern }); }
}
...
function collectAllowAlwaysPatterns(params: {
  ...
  const powerShellFileScriptArgv = resolvePowerShellFileScriptArgv({ segment, cwd: params.cwd });
  const inlineCommand = powerShellFileScriptArgv ? null : trustPlan.shellInlineCommand;
  const positionalArgvPath =
    inlineCommand !== null
      ? resolveShellWrapperPositionalArgvCandidatePath({ segment, cwd: params.cwd, env: params.env, platform: params.platform })
      : undefined;
  ...
  if (!inlineCommand) {
    const scriptPath =
      powerShellFileScriptArgv?.[0] ??
      resolveShellWrapperScriptCandidatePath({ segment, cwd: params.cwd });
    if (scriptPath) {
  ...
  for (const nestedSegment of nested.segments) {
    collectAllowAlwaysPatterns({
      segment: nestedSegment, cwd: params.cwd, env: params.env, platform: params.platform,
      strictInlineEval: params.strictInlineEval, depth: params.depth + 1, out: params.out,
    });
  }
}
...
export function resolveAllowAlwaysPatternEntries(params: {
  ...
  for (const segment of params.segments) {
    collectAllowAlwaysPatterns({
      segment, cwd: params.cwd, env: params.env, platform: params.platform,
      strictInlineEval: params.strictInlineEval, depth: 0, out: patterns,
    });
  }
  return patterns;
}
