// 移植自 openclaw/src/infra/heartbeat-events-filter.ts
...
/** 心跳 OK token（降级常量，源文件来自 ../auto-reply/tokens.js） */
const HEARTBEAT_TOKEN = "HEARTBEAT_OK";

/** 心跳响应工具说明（降级常量，源文件来自 ../auto-reply/heartbeat.js） */
const HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS =
  "Use the heartbeat_respond tool to report the outcome. Set notify=false when nothing needs the user's attention.";
...
// 通过嵌入实际事件内容为 cron 事件构建动态 prompt。
// 这确保模型直接看到提醒文本，而不是依赖"显示在上方的系统消息中"，
// 后者在上下文中可能不可见。
...
// 检测心跳特定的噪声，使 cron 提醒不会在非提醒事件上触发。
