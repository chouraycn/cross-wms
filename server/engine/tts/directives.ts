...
function buildProviderOrder(left: SpeechProviderPlugin, right: SpeechProviderPlugin): number {
  const leftOrder = (left.autoSelectOrder as number | undefined) ?? Number.MAX_SAFE_INTEGER;
  const rightOrder = (right.autoSelectOrder as number | undefined) ?? Number.MAX_SAFE_INTEGER;
...
function resolveDirectiveProvider(
  ...
  return providers.find(
    (provider) =>
      ...
      (provider.aliases as readonly string[] | undefined)?.some((alias) => normalizeLowercaseStringOrEmpty(alias) === normalized),
  );
}
  ...
  cleanedText = replaceOutsideMarkdownCode(cleanedText, directiveRegex, (_match, [body = ""]) => {
    ...
    for (const token of tokens) {
      ...
      for (const provider of directiveProvidersLocal) {
        ...
        const parseDirectiveToken = provider.parseDirectiveToken as
          | ((ctx: SpeechDirectiveTokenParseContext) => SpeechDirectiveTokenParseResult | undefined)
          | undefined;
        const parsed = parseDirectiveToken?.({
          key,
          value: rawValue,
          policy,
