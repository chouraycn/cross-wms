...
// 移植自 openclaw/src/cli/program/help.ts
//
// 降级策略：
//  - 原模块依赖 ../../../packages/terminal-core/src/{links,theme}.js；
//    cross-wms 未移植 terminal-core 包；内联降级实现（无颜色输出）。
//  - 原模块依赖 ../../infra/git-commit.js 的 resolveCommitHash；降级为返回 null。
//  - 原模块依赖 ../../utils.js 的 escapeRegExp；内联简单实现。
//  - 原模块依赖 ../argv.js 的 isRootVersionInvocation（已移植）。
//  - 原模块依赖 ../banner.js 的 formatCliBannerLine, hasEmittedCliBanner（已移植）。
//  - 原模块依赖 ../cli-name.js 的 replaceCliName, resolveCliName（已移植）。
//  - 原模块依赖 ../log-level-option.js（已移植）。
//  - 原模块依赖 ./context.js, ./core-command-descriptors.js, ./subcli-descriptors.js,
//    ./error-output.js（均已移植）。
...
const theme = {
  heading(value: string): string { return value; },
  option(value: string): string { return value; },
  command(value: string): string { return value; },
  muted(value: string): string { return value; },
  info(value: string): string { return value; },
  error(value: string): string { return value; },
};
...
function resolveCommitHash(_params: { moduleUrl?: string }): string | null {
  return null;
}
...
export function configureProgramHelp(
  ...
  if (isRootVersionInvocation(process.argv)) {
    const commit = resolveCommitHash({ moduleUrl: __filename });
    console.log(
