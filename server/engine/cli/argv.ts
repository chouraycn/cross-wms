// 低级 CLI argv 辅助：根选项、help/version 检测、命令路径解析。
// 移植自 openclaw/src/cli/argv.ts。
//
// 降级策略：
//  - 原模块依赖 `../daemon/runtime-binary.js`（cross-wms 已存在 `isBunRuntime`/`isNodeRuntime`）。
//  - 原模块依赖 `../infra/cli-root-options.js`（已移植为 `./cli-root-options.js`）。
//  - 原模块依赖 `../infra/parse-finite-number.js`（cross-wms 已存在 `parseStrictPositiveInteger`）。
//  - 原模块依赖 `./program/core-command-descriptors.js` 与 `./program/subcli-descriptors.js`
//    （cross-wms 已有降级 stub，导出静态 descriptor 列表）。
...
export function isHelpOrVersionInvocation(argv: string[]): boolean {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    // Positional `help` 可能是已知叶子命令的命令参数。
    // 未知根仍保持 help 兼容，因为插件可能稍后注册命名空间。
...
function isRootInvocationForFlags(
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    // 未知 flag 与 subcommand-scoped help/version 应回退到 Commander。
    return false;
  }
  return hasTarget;
}
...
export function normalizeRootNoColorArgv(
  ...
  for (let index = 0; index < remainingArgs.length; index += 1) {
    ...
    if (arg === "--no-color") {
      // Commander 可将 dash-prefixed token 视为命令选项值。
      // 早期调用方保持保守；最终 Commander 解析可传递元数据。
...
export function getPositiveIntFlagValue(argv: string[], name: string): number | null | undefined {
  ...
  // 保留 absent 与 present-but-invalid 的区分，使 route-first 调用方可以将非法输入
  // 延迟到 Commander 而不是静默应用默认值。
