...
// 移植自 openclaw/src/cli/plugins-cli.ts。
//
// 降级策略：
//  - 原模块依赖 `../../packages/terminal-core/src/links.js` 的 `formatDocsLink`。
//    cross-wms 未移植；内联降级实现。
//  - 原模块依赖 `../../packages/terminal-core/src/theme.js` 的 `theme`。
//    cross-wms 未移植；内联降级实现。
//  - 原模块依赖 `./program/helpers.js` 的 `parseStrictPositiveIntOption`。
//    cross-wms 未移植；内联降级实现。
//  - 原模块依赖 `./program/parent-default-help.js` 的 `applyParentDefaultHelpAction`。
//    cross-wms 未移植；内联降级实现（no-op）。
//  - 原模块通过动态 import 引用各 plugins-*-command.js 模块。
//    cross-wms 已移植这些模块（降级版本）。
...
// ===== 内联 formatDocsLink stub（替代未移植的 terminal-core/links.js）=====
function formatDocsLink(_path: string, fallback: string): string {
  return `https://${fallback}`;
}
// ===== formatDocsLink stub 结束 =====

// ===== 内联 parseStrictPositiveIntOption stub =====
function parseStrictPositiveIntOption(value: string, _flagName: string): number {
...
// ===== parseStrictPositiveIntOption stub 结束 =====

// ===== 内联 applyParentDefaultHelpAction stub（no-op）=====
function applyParentDefaultHelpAction(_command: Command): void {
  // 降级 no-op：openclaw 的 program/parent-default-help.js 未移植。
}
// ===== applyParentDefaultHelpAction stub 结束 =====
