...
//    `../infra/_runtime-stubs.js`（`resolveStateDir`）。`pathExists` 内联降级实现。
...
import { resolveStateDir } from "../infra/_runtime-stubs.js";
...
/** Resolves the active shell from environment paths, defaulting to zsh for any shells. */
...
export async function installCompletion(shell: string, yes: boolean, binName = "openclaw") {
  ...
  try {
    try {
      ...
      if (!yes) {
        // eslint-disable-next-line no-console -- completion install reports profile creation.
    ...
    if (!update.changed) {
      if (!yes) {
        // eslint-disable-next-line no-console -- completion install reports no-op.
    ...
    if (!yes) {
      ...
      // eslint-disable-next-line no-console -- completion install reports progress.
      console.log(`${action} completion in ${profilePath}...`);
    }

    await fs.writeFile(profilePath, update.next, "utf-8");
    if (!yes) {
      // eslint-disable-next-line no-console -- completion install reports completion.
      console.log(
