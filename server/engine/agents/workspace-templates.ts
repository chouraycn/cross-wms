 ...
 *
 * 降级说明：
 *  - `import.meta.url` 改用 `__filename`（CommonJS 模块）。
 *  - openclaw `../utils.js` 的 `pathExists` 改从 `../infra/fs-safe.js` 导入。
 *  - `fileURLToPath(import.meta.url)` 替换为 `__filename`（已是文件路径）。
 */
...
export async function resolveWorkspaceTemplateDir(opts?: {
  ...
  resolvingTemplateDir = (async () => {
    const moduleUrl = opts?.moduleUrl ?? __filename;
