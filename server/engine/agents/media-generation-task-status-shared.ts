/**
 * 移植自 openclaw/src/agents/media-generation-task-status-shared.ts
 *
 * Shared media generation task status and duplicate-guard helpers.
 * Cross-wms simplified: inlined normalization, removed deep task-registry imports.
 */
...
function resolveNonNegativeIntegerOption(value: number, fallback: number): number {
...
export function findActiveMediaGenerationTaskForSession(params: {
  ...
  const activeEntries = entries.filter((entry) => {
    ...
    return (
      ...
      (!taskLabel || mediaGenerationTaskLabelMatches(task, taskLabel))
    );
  });
  // Prefer running over queued
  const running = activeEntries.find((e) => e.task.status === "running");
  if (running) {
    return { ...running.task };
  }
  return activeEntries.length > 0 ? { ...activeEntries[0].task } : undefined;
}
...
export function listActiveMediaGenerationTasksForSession(params: {
  ...
  const active = entries
    ...
    .map((e) => e.task);
  return [
    ...active.filter((t) => t.status === "running"),
    ...active.filter((t) => t.status !== "running"),
  ];
}
