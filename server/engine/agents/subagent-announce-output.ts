...
export function applySubagentWaitOutcome(params: {
  ...
  if (terminalOutcome?.status === "timeout") {
    ...
    outcome = { status: "error", error: (terminalOutcome as any).error ?? waitError };
...
export async function buildCompactAnnounceStatsLine(params: {
  ...
  const storePath = subagentAnnounceOutputDeps.resolveStorePath((cfg as any).session?.store, { agentId });
