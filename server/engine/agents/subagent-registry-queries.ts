 ...
 *
 * Cross-wms: uses local SubagentRunRecord type and simplified liveness checks.
 */

import type { SubagentRunRecord } from "./subagent-registry.store.js";
  ...
  const countActiveDescendantRuns = (rootSessionKey: string): number => {
    ...
    for (const requester of pending) {
      ...
      for (const [childSessionKey, pair] of latestByChild.entries()) {
        ...
        if (isLiveUnendedSubagentRun(pair.entry)) {
          count += 1;
        }
...
/** Resolves the requester for the latest child-session run. */
