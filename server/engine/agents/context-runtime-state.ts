 ...
 *
 * 移植自 openclaw/src/agents/context-runtime-state.ts。
 * 降级策略：
 *   - `createLazyImportLoader`、`LazyPromiseLoader` 来自 ../shared/lazy-promise.js（cross-wms 已有）。
 *   - `models-config.runtime.js` 在 cross-wms 未实现，本地定义最小模块类型
 *     `ModelsConfigRuntimeModule`，并降级 loader 的 import 目标为占位函数。
 *   - `OpenClawConfig` 用本地最小子集类型 `OpenClawConfigLike`。
 *   - 从本地 ./context-cache.js 导入缓存常量。
 */
...
/**
 * 本地降级类型：models-config.runtime 模块的最小导出形态。
...
/**
 * 本地降级类型：仅保留 context-runtime-state 关心的字段。
 * 对应 openclaw 的 OpenClawConfig 的最小子集。
 */
type OpenClawConfigLike = unknown;

type ContextWindowRuntimeState = {
  generation: number;
  loadPromise: Promise<void> | null;
  loadGeneration: number | null;
  configuredConfig: OpenClawConfigLike | undefined;
  ...
  modelsConfigRuntimeLoader: LazyPromiseLoader<ModelsConfigRuntimeModule>;
};
...
export const CONTEXT_WINDOW_RUNTIME_STATE = (() => {
  ...
  if (!state) {
    ...
    state.modelsConfigRuntimeLoader ??= createLazyImportLoader<ModelsConfigRuntimeModule>(
      async () => ({
        ensureOpenClawModelsJson: () => undefined,
      }),
    );
  }
