/**
 * apply_patch 解析器的 update-hunk 应用。
 *
 * 以容错匹配定位预期旧行，按顺序应用 chunk，
 * 并返回以单个换行结尾的规范化文件内容。
 */
...
/** 将解析出的 update chunk 应用到一个文件并返回新文件内容。 */
...
function computeReplacements(
  ...
  for (const chunk of chunks) {
    ...
    if (found === null && pattern[pattern.length - 1] === "") {
      // 解析出的 hunk 可能以空行作为 EOF 哨兵。去掉它重试，使等价文件内容仍能匹配。
...
function applyReplacements(
  ...
  const result = [...lines];
  // 从文件末尾向前应用，使较早的替换索引在较晚的替换改变数组时保持稳定。
...
function seekSequence(
  ...
  // 逐步回退到更容错的比较。这样在保留正常精确匹配的同时，
  // 也能接受生成的 patch 文本中常见的空白/标点差异。
  for (let i = searchStart; i <= maxStart; i += 1) {
