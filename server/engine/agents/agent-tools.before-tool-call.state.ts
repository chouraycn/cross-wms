 ...
 * 选定的规范化负载。
 */
export const adjustedParamsByToolCallId = new Map<string, any>();
...
export function consumeAdjustedParamsForToolCall(toolCallId: string, runId?: string): any {
...
export function peekAdjustedParamsForToolCall(toolCallId: string, runId?: string): any {
  const key = buildAdjustedParamsKey({ runId, toolCallId });
  const params = adjustedParamsByToolCallId.get(key);
