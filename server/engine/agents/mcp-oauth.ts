 ...
 *
 * 降级策略：
 *  - 文件存储路径基于 resolveStateDir()（~/.openclaw/mcp-oauth/）
 *  - OAuth flow 使用 @modelcontextprotocol/sdk/client/auth.js 的 auth() 函数
 *  - 当 SDK 不可用时，回调抛出明确错误
 */
...
/** Creates the MCP SDK OAuth provider backed by the private state directory. */
