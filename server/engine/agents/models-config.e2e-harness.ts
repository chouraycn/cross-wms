 ...
 * Ported from openclaw/src/agents/models-config.e2e-harness.ts
 *
 * The full implementation requires vitest, config cache, and plugin loader.
 * This adapted version provides the env helpers and test constants.
 */
...
export async function withModelsTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "cross-wms-models-"));
  try {
...
export function installModelsConfigTestHooks(_opts?: {
  restoreFetch?: boolean;
  resetPluginLoaderState?: boolean;
}): void {
  // No-op in cross-wms: vitest hooks and plugin loader state not available.
  // Callers should manage their own test isolation.
}
...
export function unsetEnv(vars: string[]): void {
  for (const envVar of vars) {
    delete process.env[envVar];
  }
}
