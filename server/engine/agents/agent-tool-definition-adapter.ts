        ...
        try {
          if (!beforeHookWrapped) {
            ...
            if (hookOutcome.blocked) {
              if ((hookOutcome as any).kind === "veto") {
                return buildBlockedToolResult({
                  reason: (hookOutcome as any).reason,
                  deniedReason: (hookOutcome as any).deniedReason,
                  toolCallId,
                  runId: hookContext?.runId,
                });
              }
              throw new Error((hookOutcome as any).reason);
            }
      ...
      execute: async (...args: ToolExecuteArgs): Promise<AgentToolResult<unknown>> => {
        ...
        try {
          ...
          if (outcome.blocked) {
            ...
            if ((outcome as any).kind === "veto") {
              return buildBlockedToolResult({
                reason: (outcome as any).reason,
                deniedReason: (outcome as any).deniedReason,
                toolCallId,
                runId: hookContext?.runId,
              });
            }
            throw new Error((outcome as any).reason);
          }
