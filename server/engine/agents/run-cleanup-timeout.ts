 ...
 *
 * 移植自 openclaw/src/agents/run-cleanup-timeout.ts
 * 降级策略：
 *  - formatErrorMessage 从 ../infra/errors.js 导入（cross-wms 已存在）
 *  - parseStrictPositiveInteger 从 ../infra/parse-finite-number.js 导入（cross-wms 已存在）
 */
...
function normalizeExplicitTimeoutMs(value: any): number | undefined {
...
export async function runAgentCleanupStep(params: {
  ...
  const observedCleanupPromise = cleanupPromise.catch((error: any) => {
    if (!timedOut) {
      params.log.warn(
  ...
  if (result === "timeout") {
    ...
    void cleanupPromise.catch((error: any) => {
      params.log.warn(
