 ...
 * request ids, and binary payload guardrails into stable error shapes.
 *
...
/** Redacts common secret patterns from text. Simplified: no-op in cross-wms. */
...
/** Reads a bounded response body as bytes. */
async function readResponseWithLimit(
  response: Response,
  maxBytes: number,
  opts?: { onOverflow?: (ctx: { maxBytes: number }) => Error },
): Promise<Uint8Array> {
  const reader = response.body?.getReader();
  if (!reader) {
  ...
  try {
    while (true) {
      ...
      if (total > maxBytes) {
        await reader.cancel().catch(() => {});
        if (opts?.onOverflow) {
          throw opts.onOverflow({ maxBytes });
        }
        break;
      }
      chunks.push(value);
