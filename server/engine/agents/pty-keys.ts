// 将终端按键、hex、字面量与粘贴输入编码为 PTY 字节序列
// 处理 xterm 修饰键与 DECCKM 应用光标模式
...
/** 当请求按键依赖 normal vs application 光标键模式时返回 true */
...
/** 将字面量、hex 与命名按键 token 编码为一个 PTY 输入字符串 */
...
/** 在启用 bracketed-paste 时将粘贴文本包裹在标记中 */
...
function encodeKeyToken(
  ...
  // 方向键根据光标键模式特殊处理
  // DECCKM 仅改变无修饰键；有修饰键使用 xterm 修饰键方案
  if (
