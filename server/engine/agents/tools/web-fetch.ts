 ...
 * - Replaced `../../logger.js` import of `logDebug` with a local no-op stub
 *   because cross-wms logger is a stub and does not export `logDebug`.
 * - Replaced `../../web-fetch/runtime.js` import of `resolveWebFetchDefinition`
 *   with a local stub because cross-wms does not expose that runtime module.
 * - Replaced `../../../packages/web-content-core/src/provider-runtime-shared.js`
 *   import of `resolveWebProviderConfig` with a local stub because that package
 *   is not part of cross-wms.
 * - Replaced `../schema/string-enum.js` with `../string-enum.js` because
 *   cross-wms keeps `stringEnum` at `agents/string-enum.ts`.
 * - Replaced `./web-guarded-fetch.js` with `../web-guarded-fetch.js` because
 *   cross-wms keeps web-guarded-fetch at the agents/ folder. The cross-wms stub
 *   returns `unknown`, so type assertions are added at call sites to preserve
 *   type safety.
 * - `readPositiveIntegerParam`, `readStringParam`, `scheduleToolProgress` from
 *   openclaw `./common.js` are not exported by cross-wms common.ts, so local
 *   helpers are added.
 */
import {
...
} from "../../plugins/_openclaw__normalization_core__string_coerce.js";
...
/** Local no-op stub for `logDebug`. cross-wms logger does not export this. */
...
/**
 * Local stub for openclaw `resolveWebFetchDefinition`. cross-wms does not yet
 * expose this runtime function; returns null so callers fall back to the
 * built-in fetch path.
 */
function resolveWebFetchDefinition(_params: {
  config?: OpenClawConfig;
  sandboxed?: boolean;
  runtimeWebFetch?: RuntimeWebFetchMetadata;
  preferRuntimeProviders?: boolean;
}): WebFetchProviderFallback {
  return null;
}

/**
 * Local stub for openclaw `resolveWebProviderConfig`. Returns undefined so the
 * tool falls back to default config values.
 */
...
const WebFetchSchema = Type.Object({
  ...
  extractMode: Type.Optional(
    stringEnum(EXTRACT_MODES, {
      ...
      default: "markdown",
    }) as unknown as ReturnType<typeof Type.Optional>,
  ),
...
type WebFetchConfig = NonNullable<OpenClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { fetch?: infer Fetch }
    ...
    : undefined
  : undefined;
type WebFetchRuntimeModule = {
  resolveWebFetchDefinition: typeof resolveWebFetchDefinition;
};
...
export function createWebFetchTool(options?: {
  ...
  const tool: AnyAgentTool = {
    ...
    parameters: WebFetchSchema as unknown as Record<
      string,
      {
        name: string;
    ...
    execute: async (_toolCallId: string, args: unknown, signal?: AbortSignal, onUpdate?: (result: AgentToolResult<unknown>) => void) => {
      ...
      const resolveProviderFallback = async () => {
        if (!providerFallbackResolved) {
          const { resolveWebFetchDefinition: resolver } = await loadWebFetchRuntime();
          providerFallbackCache = resolver({
            config,
      ...
      try {
        const result = await runWebFetch({
          ...
          maxChars: resolveMaxChars(
            maxChars ?? (executionFetch && "maxChars" in executionFetch ? executionFetch.maxChars : undefined),
            DEFAULT_FETCH_MAX_CHARS,
            maxCharsCap,
          ),
          maxResponseBytes,
          maxRedirects: resolveMaxRedirects(
            executionFetch && "maxRedirects" in executionFetch ? executionFetch.maxRedirects : undefined,
            DEFAULT_FETCH_MAX_REDIRECTS,
          ),
          timeoutSeconds: resolveTimeoutSeconds(
            executionFetch && "timeoutSeconds" in executionFetch ? executionFetch.timeoutSeconds : undefined,
            DEFAULT_TIMEOUT_SECONDS,
          ),
          cacheTtlMs: resolveCacheTtlMs(
            executionFetch && "cacheTtlMinutes" in executionFetch ? executionFetch.cacheTtlMinutes : undefined,
            DEFAULT_CACHE_TTL_MINUTES,
          ),
          userAgent,
          readabilityEnabled,
          ...
          ssrfPolicy:
            executionFetch && "ssrfPolicy" in executionFetch ? executionFetch.ssrfPolicy : undefined,
