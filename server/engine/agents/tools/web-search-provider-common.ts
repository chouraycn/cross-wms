 ...
 *   because cross-wms exposes the same paths under server/engine/.
 * - Replaced `./web-guarded-fetch.js` with `../web-guarded-fetch.js` because cross-wms
 *   keeps web-guarded-fetch at the agents/ folder, not agents/tools/. The cross-wms stub
 *   returns `unknown`, so type assertions are added at call sites to preserve type safety.
 */
import { normalizeLowercaseStringOrEmpty } from "../../plugins/_openclaw__normalization_core__string_coerce.js";
...
export async function withTrustedWebSearchEndpoint<T>(
  ...
  return (
    withTrustedWebToolsEndpoint(
      ...
      async ({ response }: { response: Response }) => run(response),
    ) as Promise<T>
  );
}
