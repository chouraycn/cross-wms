 ...
 *
 * Ported from openclaw/src/agents/tools/sessions-helpers.ts.
 *
 * cross-wms adjustments:
 * - Imports `resolveSandboxedSessionToolContext` from the ported `./sessions-access.js`.
 * - Imports session-reference helpers from the ported `./sessions-resolution.js`.
 */
export {
...
import { normalizeOptionalString, type FastMode } from "@cdf-know/normalization-core/string-coerce";
...
export function resolveSessionToolContext(opts?: {
  ...
  const cfg = opts?.config ?? (getRuntimeConfig as unknown as () => OpenClawConfig)();
  return {
    cfg,
    ...resolveSandboxedSessionToolContext({
      cfg,
