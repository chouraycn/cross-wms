 ...
 *
 * Ported from openclaw/src/agents/tools/sessions-resolution.ts.
 *
 * cross-wms adjustments:
 * - Replaced `../../../packages/gateway-protocol/src/client-info.js` with the
 *   `@cdf-know/gateway-protocol/client-info.js` path mapping.
 * - Imported `isAcpSessionKey` from `../sessions-resolution.js` (cross-wms
 *   already exposes it from the simplified agents/sessions-resolution.ts).
 * - Declared a local stub `GatewayClientRequestError` class because cross-wms
 *   does not yet export it from `../../gateway/client.js`. The stub preserves
 *   the `gatewayCode` and `message` shape used by the older-gateway fallback
...
} from "@cdf-know/gateway-protocol/client-info.js";
...
import { isAcpSessionKey } from "../sessions-resolution.js";
import { looksLikeSessionId } from "../../sessions/session-id.js";

/**
 * Stub error class standing in for openclaw's `GatewayClientRequestError`.
 *
 * Cross-wms gateway/client.ts does not yet export this error type. We keep a
 * local stub so that the older-gateway fallback branch type-checks; once the
 * real class is exported, callers should switch back to importing it.
 */
...
export function resolveMainSessionAlias(cfg: OpenClawConfig) {
  const mainKey = normalizeMainKey((cfg as { session?: { mainKey?: string } }).session?.mainKey);
  const scope = (cfg as { session?: { scope?: string } }).session?.scope ?? "per-sender";
