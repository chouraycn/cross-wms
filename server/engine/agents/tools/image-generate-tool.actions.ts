 ...
 *
 * Ported from openclaw/src/agents/tools/image-generate-tool.actions.ts.
 *
 * cross-wms adjustments:
 * - `listRuntimeImageGenerationProviders` in cross-wms does not accept a `config` field
...
export function createImageGenerateListActionResult(params: {
  ...
  // cross-wms `listRuntimeImageGenerationProviders` does not accept `config`; list all
  // configured providers regardless of cfg.
