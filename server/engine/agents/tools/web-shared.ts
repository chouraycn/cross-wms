...
function resolveExpiresAtMsFromDurationMs(durationMs: number, opts?: { nowMs?: number }): number | undefined {
...
function resolveTimerTimeoutMs(timeoutMs: number, minMs: number): number {
  const parsed = typeof timeoutMs === "number" && Number.isFinite(timeoutMs) ? timeoutMs : minMs;
  const maxMs = MAX_TIMER_TIMEOUT_SECONDS * 1000;
