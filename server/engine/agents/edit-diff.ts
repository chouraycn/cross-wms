 ...
 * Cross-wms simplified: inlined path resolution, uses 'diff' npm package.
 */
...
function getNotFoundError(path: string, editIndex: number, totalEdits: number): Error {
  if (totalEdits === 1) return new Error(`Could not find the exact text in ${path}. The old text must match exactly including all whitespace and newlines.`);
...
function getDuplicateError(path: string, editIndex: number, totalEdits: number, occurrences: number): Error {
  if (totalEdits === 1) return new Error(`Found ${occurrences} occurrences of the text in ${path}. The text must be unique. Please provide more context to make it unique.`);
...
function getNoChangeError(path: string, totalEdits: number): Error {
  if (totalEdits === 1) return new Error(`No changes made to ${path}. The replacement produced identical content. This might indicate an issue with special characters or the text not existing as expected.`);
...
export function applyEditsToNormalizedContent(
  ...
  for (let i = 1; i < matchedEdits.length; i++) {
    ...
    if (prev.matchIndex + prev.matchLength > curr.matchIndex) {
      throw new Error(`edits[${prev.editIndex}] and edits[${curr.editIndex}] overlap in ${path}. Merge them into one edit or target disjoint regions.`);
    }
  }
    ...
    if (part.added || part.removed) {
      ...
      if (hasLeadingChange && hasTrailingChange) {
        if (raw.length <= contextLines * 2) {
          ...
          oldLineNum += skippedLines; newLineNum += skippedLines;
...
export async function computeEditsDiff(
  ...
  try {
    ...
    const rawContent = typeof rawContentResult === "string" ? rawContentResult : rawContentResult.toString("utf-8");
