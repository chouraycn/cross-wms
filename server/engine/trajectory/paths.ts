...
// 移植自 openclaw/src/trajectory/paths.ts
// 适配：cross-wms 的 isPathInside 签名为 (targetPath, rootDir)，
// openclaw 为 (basePath, targetPath)，因此调用时参数顺序互换。
...
function resolveContainedPath(baseDir: string, fileName: string): string {
  ...
  if (resolvedFile === resolvedBase || !isPathInside(resolvedFile, resolvedBase)) {
