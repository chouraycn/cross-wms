...
// 移植自 openclaw/src/trajectory/command-export.ts
//
// 适配说明：
// - pathExists: 从 ../infra/fs-safe.js 导入（cross-wms 已有）
// - isPathInside: 从 ../infra/path-guards.js 导入，签名为 (targetPath, rootDir)
//   openclaw 为 (basePath, targetPath)，调用时参数顺序互换
// - exportTrajectoryBundle/resolveDefaultTrajectoryExportDir: 从 ./export.js 导入
...
async function validateExistingExportDirectory(params: {
  ...
  if (!isPathInside(realDir, params.realWorkspace)) {
...
export async function resolveTrajectoryCommandOutputDir(params: {
  ...
  // cross-wms isPathInside 签名为 (targetPath, rootDir)，参数顺序与 openclaw 相反
  if (!isPathInside(realExistingParent, realBase)) {
