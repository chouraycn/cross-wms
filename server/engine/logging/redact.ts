...
// Migrated from openclaw/src/logging/redact.ts — core redaction logic preserved verbatim;
// only import paths adapted to cross-wms and backward-compat shims (redactObject/addRedactPattern)
// appended for existing consumers.
import type { LoggerSettings } from "./types.js";
import { compileConfigRegex } from "./config-regex.js";
import { readLoggingConfig } from "./config.js";
import { replacePatternBounded } from "./redact-bounded.js";

export type RedactSensitiveMode = "off" | "tools";
export type RedactPattern = string | RegExp;
type LoggingConfig = LoggerSettings;
...
// Backward-compat: cross-wms consumers register custom patterns via addRedactPattern.
// These are merged with config patterns so they apply alongside the defaults.
const customRedactPatterns: RedactPattern[] = [];
...
function resolveConfigRedaction(): RedactOptions {
  ...
  const allPatterns = [...(userPatterns ?? []), ...customRedactPatterns];
  return {
...
function resolveToolPayloadRedaction(
  ...
  const allCustom = [...(userPatterns ?? []), ...customRedactPatterns];
  const patterns =
    allCustom.length > 0
      ? [...allCustom, ...DEFAULT_REDACT_PATTERNS]
      : undefined;
...
// ============================================================================
// Backward-compat shims for cross-wms consumers (logger.ts, diagnostic-support-redaction.ts).
// ============================================================================

/** Recursively redacts secrets from an object tree. Alias for redactSecrets. */
export function redactObject<T>(obj: T): T {
  return redactSecrets(obj);
}

/** Registers a custom redact pattern merged with the built-in defaults. */
export function addRedactPattern(pattern: RegExp, _replacement?: string): void {
  customRedactPatterns.push(pattern);
}
