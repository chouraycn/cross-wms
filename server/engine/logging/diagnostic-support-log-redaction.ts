...
// Simplified placeholder for unported SupportRedactionContext from diagnostic-support-redaction.
export type SupportRedactionContext = {
  mode?: "tools" | "full";
};

// Simplified placeholder for unported redactSupportString; truncates to maxLength.
function redactSupportString(
  value: string,
  _redaction: SupportRedactionContext,
  options: { maxLength?: number; truncationSuffix?: string } = {},
): string {
  const maxLength = options.maxLength ?? 240;
  if (value.length <= maxLength) {
    return value;
  }
  const suffix = options.truncationSuffix ?? "";
