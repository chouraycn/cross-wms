...
type DiagnosticStabilitySnapshot = ReturnType<typeof getDiagnosticStabilitySnapshot>;

export const DIAGNOSTIC_STABILITY_BUNDLE_VERSION = 1;
export const DEFAULT_DIAGNOSTIC_STABILITY_BUNDLE_LIMIT = 200;
...
function readErrorMessage(error: any): string | undefined {
  ...
  const sanitized = redactSensitiveText(message).replace(/\s+/gu, " ").trim();
  if (!sanitized) {
    return undefined;
  }
...
function readStabilityEventRecord(
  ...
  } as DiagnosticStabilitySnapshot["events"][number];
  ...
  if (record.webhooks !== undefined) {
    ...
    (sanitized as unknown as { webhooks: { received: number; processed: number; errors: number } }).webhooks = {
  ...
  if (record.memory !== undefined) {
    (sanitized as unknown as { memory: DiagnosticMemoryUsage }).memory = readMemoryUsage(record.memory, `${label}.memory`);
  }
  if (record.usage !== undefined) {
    const usage = readObject(record.usage, `${label}.usage`);
    (sanitized as unknown as { usage: Record<string, number> }).usage = {
  ...
  if (record.context !== undefined) {
    ...
    (sanitized as unknown as { context: { limit?: number; used?: number } }).context = {
...
function sanitizeSessionEvidencePath(relativePath: string): string {
  ...
  return redactSensitiveText(relativePath);
}
