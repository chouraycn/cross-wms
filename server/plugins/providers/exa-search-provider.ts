/**
 * Exa Search Provider — Exa 搜索 Provider 实现
 *
 * Neural + keyword search with date filters and content extraction.
 * https://exa.ai/
 */
...
function normalizeResults(data: Record<string, unknown>): WebSearchResult[] {
  ...
  const items = (data as unknown).results;
...
const plugin: WebSearchProviderPlugin = {
  ...
  createTool(ctx: WebSearchProviderContext): WebSearchProviderToolDefinition | null {
    ...
    return {
      description: "Search the web using Exa Search API, a neural + keyword search engine with content extraction.",
      parameters: {
        type: "object",
        properties: {
          query: {
