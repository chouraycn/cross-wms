/**
 * Moonshot Web Search Provider — 月之暗面 AI 搜索 Provider 实现
 *
 * 基于 Moonshot API 的网页搜索，需要 API Key。
 * 支持网页搜索，返回标题、链接和摘要。
 */
...
async function performSearch(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Moonshot 搜索请求失败: HTTP ${response.status} ${errorText}`);
    }
    ...
    if (e instanceof DOMException && e.name === "AbortError") {
      ...
      throw new Error("Moonshot 搜索超时（10秒）");
    }
    throw e;
  }
}
...
const plugin: WebSearchProviderPlugin = {
  ...
  autoDetectOrder: 40,
  credentialPath: "tools.web.search.providers.moonshot.apiKey",
  inactiveSecretPaths: [],
  ...
  createTool(ctx: WebSearchProviderContext): WebSearchProviderToolDefinition | null {
    ...
    return {
      description: "使用 Moonshot AI 搜索网页，由月之暗面提供支持。",
      parameters: {
        type: "object",
        properties: {
          query: {
...
registerWebSearchProvider("moonshot", plugin);
