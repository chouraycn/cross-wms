...
function normalizeResults(data: Record<string, any>): WebSearchResult[] {
  ...
  if (choices && choices.length > 0) {
    ...
    if (message) {
      ...
      const toolCalls = message.tool_calls as Array<Record<string, any>> | undefined;
      if (toolCalls) {
        for (const tc of toolCalls) {
          const fn = tc.function as Record<string, any> | undefined;
  ...
  // 3. 尝试从顶层字段提取（兼容旧格式）
  if (results.length === 0) {
    const items = data.results || data.data || data.items || (data as { search_results?: any[] }).search_results;
