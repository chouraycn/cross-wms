/**
 * Grok Web Search Provider — xAI Grok 实时搜索 Provider 实现
 *
 * 基于 xAI API 的 Grok 实时搜索，需要 API Key。
 * 支持网页搜索，返回标题、链接和摘要。
 */
...
async function performSearch(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Grok 搜索请求失败: HTTP ${response.status} ${errorText}`);
    }
...
function normalizeResults(data: Record<string, unknown>): WebSearchResult[] {
  ...
  const items = data.results || data.data || data.items || (data as { web_results?: unknown[] }).web_results;
...
const plugin: WebSearchProviderPlugin = {
  id: "grok",
  label: "Grok",
  hint: "xAI Grok with real-time search",
  ...
  autoDetectOrder: 50,
  credentialPath: "tools.web.search.providers.grok.apiKey",
  inactiveSecretPaths: [],
  ...
  createTool(ctx: WebSearchProviderContext): WebSearchProviderToolDefinition | null {
    ...
    return {
      description: "Search the web using xAI Grok with real-time search capabilities.",
      parameters: {
        type: "object",
        properties: {
          query: {
...
registerWebSearchProvider("grok", plugin);
