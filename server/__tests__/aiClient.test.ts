...
describe('validateToolMessages（通过消息传递和日志调用间接验证）', () => {
  ...
  it('缺失 tool 结果时应自动修复（移除无响应的 tool_calls）', async () => {
    const messages = [
    ...
    expect(mockLogError).toHaveBeenCalledWith(
      expect.stringContaining('移除'),
    );
  });

  it('所有 tool_calls 均缺失对应 tool 消息时应清空 tool_calls 字段', async () => {
    const messages = [
