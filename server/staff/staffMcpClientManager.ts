 ...
 *
 * 数字员工的 MCP server 配置存在 `sd_mcp_servers` 表，与主程序的 `mcp_servers` 表
 * （全局 McpClientManager 单例的来源）完全独立。为避免污染全局单例、保证租户/员工
 * 隔离，这里用 `McpClientManager.create()` 创建一个独立实例，仅在本次员工会话内
 * 连接并分发该员工配置的 MCP server。
 *
 * 执行时：
...
import { McpClientManager, type McpServerConfig } from '../engine/mcpClientManager.js';
import { type McpServerRow } from '../types/staff.js';
import * as mcpServerDao from '../dao/staff/staffMcpServerDao.js';
import { logger } from '../logger.js';

/** 将 sd_mcp_servers 行转换为 McpClientManager 所需的 McpServerConfig */
function toMcpServerConfig(row: McpServerRow): McpServerConfig {
  ...
  return {
    id: row.id,
    name: row.name,
    // sd_mcp_servers 用 `transport` 字段，McpServerConfig 用 `transportType`
