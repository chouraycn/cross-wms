...
// 数据库文件路径：使用 ~/.crosswms/chat.db
const crosswmsDir = path.join(os.homedir(), '.crosswms');
const dbPath = path.join(crosswmsDir, 'chat.db');

// 确保 ~/.crosswms 目录存在
if (!fs.existsSync(crosswmsDir)) {
  fs.mkdirSync(crosswmsDir, { recursive: true });
