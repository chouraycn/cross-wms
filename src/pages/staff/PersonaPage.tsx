...
import type { AgentProfileRead, PersonaRead, UIConfigRead } from '../../components/staff/types/index.js';
...
export default function PersonaPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<UIConfigRead>(`/ui-config?tenant_id=${TENANT_ID}`)
      .then((row) => {
        setUiForm({
  ...
  useEffect(() => {
    ...
    if (agent) {
      if (agent.is_overall) {
        api
          .get<PersonaRead>(`/persona?tenant_id=${TENANT_ID}`)
          .then((row) => {
            setForm({
  ...
  async function save() {
    ...
    try {
      if (selectedAgent) {
        const row = await api.put<AgentProfileRead>(`/agents/${selectedAgent.id}`, {
          tenant_id: TENANT_ID,
        ...
        setUpdatedAt(row.updated_at);
        if (row.is_overall) {
          await api.put<PersonaRead>('/persona', {
            tenant_id: TENANT_ID,
        ...
        const row = await api.put<PersonaRead>('/persona', {
          tenant_id: TENANT_ID,
          system_prompt: form.system_prompt,
        });
        setUpdatedAt(row.updated_at);
  ...
  async function saveUiConfig() {
    ...
    setUiLoading(true);
    try {
      const row = await api.put<UIConfigRead>('/ui-config', {
        tenant_id: TENANT_ID,
