                  ...
                  <li key={item.id}>
                    <a
                      ...
                      className={
                        ...
                        (activeId === item.id
                          ? 'font-medium text-[#3a4fbf]'
                          : 'text-[#464c5e] hover:text-[#18181a]')
                      }
                    >
                      {item.label}
                    </a>
      ...
      <article className="min-w-0 flex-1">
        ...
        <section id="runtime" className="mb-[40px] scroll-mt-[40px]">
          ...
          <p className="mt-[12px] text-[14px] leading-[22px] text-[#464c5e]">
            ...
            <code className="mx-[4px] rounded bg-[#f2f3f7] px-[4px] py-[2px] text-[12px]">staffChatExecutor</code>
            驱动，按当前数字员工的岗位人设、绑定的技能 SOP 与实时检索的知识上下文组装系统提示词，
            再交由 REACT 模式的执行器完成推理与动作。
          </p>
          <ul className="mt-[12px] list-disc space-y-[8px] pl-[20px] text-[14px] leading-[22px] text-[#464c5e]">
          ...
          <p className="mt-[12px] text-[13px] leading-[20px] text-[#858b9c]">
            未配置可用模型时，闭环以占位流式响应运行，用于验证链路连通性，不会返回真实推理结果。
          </p>
        </section>

        <section id="governance" className="mb-[40px] scroll-mt-[40px]">
          <h2 className="m-0 text-[24px] font-semibold text-[#18181a]">治理与复盘</h2>
          <p className="mt-[12px] text-[14px] leading-[22px] text-[#464c5e]">
        ...
        <section id="architecture" className="mb-[40px] scroll-mt-[40px]">
          <h2 className="m-0 text-[24px] font-semibold text-[#18181a]">架构概览</h2>
          <p className="mt-[12px] text-[14px] leading-[22px] text-[#464c5e]">
            数字员工模块沿用了 CrossWMS 的整体架构：
          </p>
          <ul className="mt-[12px] list-disc space-y-[8px] pl-[20px] text-[14px] leading-[22px] text-[#464c5e]">
            <li><strong>前端</strong>：React + Vite 单页应用，数字员工相关页面挂载在 <code className="rounded bg-[#f2f3f7] px-[4px] py-[2px] text-[12px]">/enterprise/*</code> 与 <code className="rounded bg-[#f2f3f7] px-[4px] py-[2px] text-[12px]">/workspace/*</code> 下。</li>
            <li><strong>后端</strong>：TypeScript/Express，所有接口统一前缀 <code className="rounded bg-[#f2f3f7] px-[4px] py-[2px] text-[12px]">/api/staffdeck</code>（含模型配置、技能、知识库、工具、对话流式、定时任务等）。</li>
            <li><strong>存储</strong>：SQLite，业务数据存储在 <code className="rounded bg-[#f2f3f7] px-[4px] py-[2px] text-[12px]">sd_*</code> 系列表中（员工、会话、消息、事件 Trace、知识分块等）。</li>
            <li><strong>运行载体</strong>：通过 PyWebView 打包为桌面应用，数字员工作为子模块随主程序启动。</li>
          </ul>
        </section>

        <section id="flow" className="mb-[40px] scroll-mt-[40px]">
          <h2 className="m-0 text-[24px] font-semibold text-[#18181a]">执行流程</h2>
          <ol className="mt-[12px] m-0 list-decimal space-y-[10px] pl-[20px] text-[14px] leading-[22px] text-[#464c5e]">
        ...
        <section id="reference" className="mb-[40px] scroll-mt-[40px]">
          <h2 className="m-0 text-[24px] font-semibold text-[#18181a]">配置参考</h2>
          <ul className="mt-[12px] list-disc space-y-[8px] pl-[20px] text-[14px] leading-[22px] text-[#464c5e]">
        ...
        <section id="development" className="mb-[40px] scroll-mt-[40px]">
          <h2 className="m-0 text-[24px] font-semibold text-[#18181a]">开发指南</h2>
          <p className="mt-[12px] text-[14px] leading-[22px] text-[#464c5e]">
            扩展数字员工能力主要有三类方式：
          </p>
          <ul className="mt-[12px] list-disc space-y-[8px] pl-[20px] text-[14px] leading-[22px] text-[#464c5e]">
        ...
        <section id="showcase" className="mb-[40px] scroll-mt-[40px]">
          <h2 className="m-0 text-[24px] font-semibold text-[#18181a]">案例展示</h2>
          <ul className="mt-[12px] list-disc space-y-[8px] pl-[20px] text-[14px] leading-[22px] text-[#464c5e]">
        ...
        <section id="faq" className="mb-[40px] scroll-mt-[40px]">
          ...
          <dl className="mt-[12px] space-y-[14px]">
            <div>
              <dt className="text-[14px] font-semibold text-[#18181a]">需要登录吗？</dt>
              <dd className="mt-[4px] text-[14px] leading-[22px] text-[#464c5e]">
                不需要。数字员工作为桌面应用子模块，进入即获得默认桌面身份，无 admin 登录门。
              </dd>
            </div>
            <div>
              <dt className="text-[14px] font-semibold text-[#18181a]">对话没有任何回复 / 一直是占位内容？</dt>
              <dd className="mt-[4px] text-[14px] leading-[22px] text-[#464c5e]">
                通常是尚未在「模型配置」中配置可用 LLM。配置并测试通过、设为默认后即可返回真实推理结果。
              </dd>
            </div>
            <div>
              <dt className="text-[14px] font-semibold text-[#18181a]">知识库内容没有被引用？</dt>
              <dd className="mt-[4px] text-[14px] leading-[22px] text-[#464c5e]">
                请确认文档已通过「新增文档」入库完成向量化，且已在对应员工下绑定该知识库。
              </dd>
            </div>
            <div>
              <dt className="text-[14px] font-semibold text-[#18181a]">数据存在哪里？</dt>
              <dd className="mt-[4px] text-[14px] leading-[22px] text-[#464c5e]">
                全部存储在本地 SQLite 的 <code className="rounded bg-[#f2f3f7] px-[4px] py-[2px] text-[12px]">sd_*</code> 表中，随桌面应用数据目录本地保存。
              </dd>
            </div>
          </dl>
        </section>
