    ...
    >
                  ...
                  <Box component="li" key={item.id}>
                    <Box
                      ...
                      sx={{
                        ...
                        ...(activeId === item.id
                          ? { fontWeight: 500, color: '#0f766e' }
                          : { color: '#343633', '&:hover': { color: '#20201d' } }),
                      }}
                    >
                      {item.label}
                    </Box>
      ...
      <Box component="article" sx={{ minWidth: 0, flex: 1 }}>
        <Box component="section" id="intro" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>项目简介</Box>
          <Box component="p" sx={{ mt: '12px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
          ...
          <Box
            ...
            alt="数字员工平台概览"
            sx={{ mt: '20px', width: '100%', maxHeight: '300px', objectFit: 'contain', borderRadius: '12px', bgcolor: '#fbfaf6', p: '8px' }}
          />
        </Box>

        <Box component="section" id="install" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>安装说明</Box>
          <Box component="p" sx={{ mt: '12px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
            前端基于 React + Vite，后端提供 <Box component="code" sx={{ borderRadius: '4px', bgcolor: '#fbfaf6', px: '4px', py: '2px', fontSize: '12px' }}>/api/staffdeck</Box> 接口，
        ...
        <Box component="section" id="quickstart" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>快速开始</Box>
          <Box component="ol" sx={{ mt: '12px', m: 0, listStyle: 'none', p: 0, '& > * + *': { mt: '16px' } }}>
            {QUICK_STEPS.map((step) => (
              <Box component="li" key={step.title} sx={{ borderRadius: '12px', border: '1px solid #ded7cc', bgcolor: 'background.paper', p: '16px' }}>
                <Box sx={{ fontSize: '15px', fontWeight: 600, color: '#20201d' }}>{step.title}</Box>
                <Box component="p" sx={{ mt: '6px', fontSize: '13px', lineHeight: '20px', color: '#343633' }}>{step.body}</Box>
        ...
        <Box component="section" id="core-features" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>能力总览</Box>
          <Box sx={{ mt: '16px', display: 'grid', gap: '16px', '@media (min-width: 640px)': { gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }, '@media (min-width: 1024px)': { gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' } }}>
            {FEATURES.map((feature) => (
              <Box key={feature.title} sx={{ borderRadius: '12px', border: '1px solid #ded7cc', bgcolor: 'background.paper', p: '16px' }}>
                <Box sx={{ fontSize: '15px', fontWeight: 600, color: '#20201d' }}>{feature.title}</Box>
                <Box sx={{ mt: '2px', fontSize: '12px', color: '#858b9c' }}>{feature.subtitle}</Box>
                <Box component="p" sx={{ mt: '8px', fontSize: '13px', lineHeight: '20px', color: '#343633' }}>{feature.body}</Box>
                <Box component="p" sx={{ mt: '8px', fontSize: '12px', color: '#0f766e' }}>{feature.proof}</Box>
              </Box>
            ))}
          </Box>
        </Box>
        ...
        <Box component="section" id="runtime" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>运行闭环</Box>
          <Box component="p" sx={{ mt: '12px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
            每一轮对话都遵循「路由决策 → 技能执行 → 工具调用 → 反思校验」的闭环。后端由
            <Box component="code" sx={{ mx: '4px', borderRadius: '4px', bgcolor: '#fbfaf6', px: '4px', py: '2px', fontSize: '12px' }}>staffChatExecutor</Box>
            驱动，按当前数字员工的岗位人设、绑定的技能 SOP 与实时检索的知识上下文组装系统提示词，
            再交由 REACT 模式的执行器完成推理与动作。
          </Box>
          <Box component="ul" sx={{ mt: '12px', listStyle: 'disc', pl: '20px', fontSize: '14px', lineHeight: '22px', color: '#343633', '& > * + *': { mt: '8px' } }}>
          ...
          <Box sx={{ mt: '16px', display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
            {COT_STEPS.map((step) => (
              <Box key={step.label} sx={{ display: 'flex', alignItems: 'center', gap: '8px', borderRadius: '10px', border: '1px solid #ded7cc', bgcolor: 'background.paper', px: '12px', py: '8px' }}>
                <Box component="img" src={step.icon} alt={step.label} sx={{ width: '22px', height: '22px', objectFit: 'contain' }} />
                <Box component="span" sx={{ fontSize: '13px', color: '#343633' }}>{step.label}</Box>
              </Box>
            ))}
          </Box>
        ...
        <Box component="section" id="governance" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>治理与复盘</Box>
          <Box component="p" sx={{ mt: '12px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
        ...
        <Box component="section" id="architecture" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>架构概览</Box>
          <Box component="p" sx={{ mt: '12px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
            数字员工模块沿用了 CDF Know Claw 的整体架构：
          </Box>
          <Box component="ul" sx={{ mt: '12px', listStyle: 'disc', pl: '20px', fontSize: '14px', lineHeight: '22px', color: '#343633', '& > * + *': { mt: '8px' } }}>
            <li><strong>前端</strong>：React + Vite 单页应用，数字员工相关页面挂载在 <Box component="code" sx={{ borderRadius: '4px', bgcolor: '#fbfaf6', px: '4px', py: '2px', fontSize: '12px' }}>/enterprise/*</Box> 与 <Box component="code" sx={{ borderRadius: '4px', bgcolor: '#fbfaf6', px: '4px', py: '2px', fontSize: '12px' }}>/workspace/*</Box> 下。</li>
            <li><strong>后端</strong>：TypeScript/Express，所有接口统一前缀 <Box component="code" sx={{ borderRadius: '4px', bgcolor: '#fbfaf6', px: '4px', py: '2px', fontSize: '12px' }}>/api/staffdeck</Box>（含模型配置、技能、知识库、工具、对话流式、定时任务等）。</li>
            <li><strong>存储</strong>：SQLite，业务数据存储在 <Box component="code" sx={{ borderRadius: '4px', bgcolor: '#fbfaf6', px: '4px', py: '2px', fontSize: '12px' }}>sd_*</Box> 系列表中（员工、会话、消息、事件 Trace、知识分块等）。</li>
        ...
        <Box component="section" id="flow" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>执行流程</Box>
          <Box component="ol" sx={{ mt: '12px', m: 0, listStyle: 'decimal', p: 0, pl: '20px', fontSize: '14px', lineHeight: '22px', color: '#343633', '& > * + *': { mt: '10px' } }}>
        ...
        <Box component="section" id="reference" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>配置参考</Box>
          <Box component="ul" sx={{ mt: '12px', listStyle: 'disc', pl: '20px', fontSize: '14px', lineHeight: '22px', color: '#343633', '& > * + *': { mt: '8px' } }}>
        ...
        <Box component="section" id="development" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>开发指南</Box>
          <Box component="p" sx={{ mt: '12px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
            扩展数字员工能力主要有三类方式：
          </Box>
          <Box component="ul" sx={{ mt: '12px', listStyle: 'disc', pl: '20px', fontSize: '14px', lineHeight: '22px', color: '#343633', '& > * + *': { mt: '8px' } }}>
        ...
        <Box component="section" id="showcase" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>案例展示</Box>
          <Box component="ul" sx={{ mt: '12px', listStyle: 'disc', pl: '20px', fontSize: '14px', lineHeight: '22px', color: '#343633', '& > * + *': { mt: '8px' } }}>
          ...
          <Box
            ...
            alt="案例展示"
            sx={{ mt: '20px', width: '100%', borderRadius: '12px', border: '1px solid #ded7cc', bgcolor: 'background.paper' }}
          />
        </Box>

        <Box component="section" id="faq" sx={{ mb: '40px', scrollMarginTop: '40px' }}>
          <Box component="h2" sx={{ m: 0, fontSize: '24px', fontWeight: 600, color: '#20201d' }}>常见问题</Box>
          <Box component="dl" sx={{ mt: '12px', '& > * + *': { mt: '14px' } }}>
            <div>
              <Box component="dt" sx={{ fontSize: '14px', fontWeight: 600, color: '#20201d' }}>需要登录吗？</Box>
              <Box component="dd" sx={{ mt: '4px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
                不需要。数字员工作为桌面应用子模块，进入即获得默认桌面身份，无 admin 登录门。
              </Box>
            </div>
            <div>
              <Box component="dt" sx={{ fontSize: '14px', fontWeight: 600, color: '#20201d' }}>对话没有任何回复 / 一直是占位内容？</Box>
              <Box component="dd" sx={{ mt: '4px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
                通常是尚未在「模型配置」中配置可用 LLM。配置并测试通过、设为默认后即可返回真实推理结果。
              </Box>
            </div>
            <div>
              <Box component="dt" sx={{ fontSize: '14px', fontWeight: 600, color: '#20201d' }}>知识库内容没有被引用？</Box>
              <Box component="dd" sx={{ mt: '4px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
                请确认文档已通过「新增文档」入库完成向量化，且已在对应员工下绑定该知识库。
              </Box>
            </div>
            <div>
              <Box component="dt" sx={{ fontSize: '14px', fontWeight: 600, color: '#20201d' }}>数据存在哪里？</Box>
              <Box component="dd" sx={{ mt: '4px', fontSize: '14px', lineHeight: '22px', color: '#343633' }}>
                全部存储在本地 SQLite 的 <Box component="code" sx={{ borderRadius: '4px', bgcolor: '#fbfaf6', px: '4px', py: '2px', fontSize: '12px' }}>sd_*</Box> 表中，随桌面应用数据目录本地保存。
              </Box>
            </div>
          </Box>
        </Box>
