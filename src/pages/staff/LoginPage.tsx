...
import { setEnterpriseAuthSession, type EnterpriseAuthSession } from '../../components/staff/auth.js';
import AppHeader from '../../components/staff/AppHeader.js';
import BrandLogo from '../../components/staff/BrandLogo.js';
...
/**
 * 登录页。点击「登录」展开账号密码表单，调用 /api/staffdeck/auth/login。
 */
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  async function login() {
    ...
    setLoading(true);
    try {
      const session = await api.post<EnterpriseAuthSession>('/auth/login', {
        tenant_id: TENANT_ID,
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-white">
      ...
      <main className="flex flex-1 flex-col items-center px-[32px]">
        ...
        {/* TODO: 接入产品预览图 */}
        <div className="mt-[32px] flex w-full max-w-[1200px] justify-center rounded-[16px] border border-dashed border-[#e3e7f1] bg-[#f6f6f6] py-[80px] text-center text-[14px] text-[#858b9c]">
          StaffDeck 产品预览
        </div>
      </main>
    </div>
