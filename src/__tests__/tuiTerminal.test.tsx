...
let mod: typeof import('../pages/TuiTerminalPage');
beforeAll(async () => {
  mod = await import('../pages/TuiTerminalPage');
}, 30000);
...
describe('Web TUI: ANSI 颜色解析', () => {
  it('应解析基本颜色代码', () => {
    const segments = mod.parseAnsi('\x1b[31m红色\x1b[0m');
  ...
  it('应解析多种颜色', () => {
    const segments = mod.parseAnsi('\x1b[32m绿色\x1b[0m \x1b[34m蓝色\x1b[0m');
  ...
  it('应解析粗体样式', () => {
    const segments = mod.parseAnsi('\x1b[1m粗体\x1b[0m');
    expect(segments[0].bold).toBe(true);
  });

  it('应解析斜体样式', () => {
    const segments = mod.parseAnsi('\x1b[3m斜体\x1b[0m');
    expect(segments[0].italic).toBe(true);
  });

  it('应解析下划线样式', () => {
    const segments = mod.parseAnsi('\x1b[4m下划线\x1b[0m');
    expect(segments[0].underline).toBe(true);
  });

  it('应解析背景色', () => {
    const segments = mod.parseAnsi('\x1b[41m红底\x1b[0m');
    expect(segments[0].bg).toBe('#ef4444');
  });

  it('应解析复合样式代码', () => {
    const segments = mod.parseAnsi('\x1b[1;31m粗体红色\x1b[0m');
    expect(segments[0].bold).toBe(true);
    expect(segments[0].fg).toBe('#ef4444');
  });

  it('无 ANSI 代码时应返回原文本', () => {
    const segments = mod.parseAnsi('普通文本');
    expect(segments).toHaveLength(1);
    expect(segments[0].text).toBe('普通文本');
  });

  it('应处理空字符串', () => {
    const segments = mod.parseAnsi('');
    expect(segments).toHaveLength(1);
    expect(segments[0].text).toBe('');
  });

  it('应处理混合文本和 ANSI 代码', () => {
    const segments = mod.parseAnsi('前\x1b[32m绿\x1b[0m后');
    expect(segments.length).toBeGreaterThanOrEqual(2);
