...
/** Code block with lightweight syntax highlighting and an optional copy button. */
export default function CodeBlock({ code, language, className, showCopy = true }: CodeBlockProps) {
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      <pre
        className="code-block-vscode overflow-x-auto rounded-[10px] bg-[#1e1e1e] p-[12px] text-[12px] leading-[1.6] text-[#d4d4d4]"
        data-language={language || undefined}
      >
        <code>{renderCodeTokens(code, language)}</code>
      </pre>
    </div>
  );
}
