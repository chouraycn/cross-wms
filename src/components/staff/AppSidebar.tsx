...
import type { AgentProfileRead, ChatSession } from './types/index.js'
...
function AgentSwitcher({
  ...
  'sidebarAgent' | 'scopeAgents' | 'selectedAgentId' | 'onSelectAgent'
>) {
  ...
  const nameLabel = currentAgent ? employeeDisplayNameWithCreator(currentAgent) : '-'
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          {currentAgent ? (
            ...
            <div className="relative h-[30px] w-[60px]">
              <div className="absolute inset-0 flex items-end justify-center">
                <span className="flex h-[71px] w-[60px] items-center justify-center rounded-[10px] border-[0.5px] border-[#e3e7f1] bg-white text-sidebar-foreground">
          ...
          <ChevronDown className="size-[14px] shrink-0 text-sidebar-foreground group-data-[collapsible=icon]:hidden" />
        </div>
      </DropdownMenuTrigger>
      <DropdownMenuContent
        align="start"
      ...
      >
        {employeeAgents.map((agent) => (
          <DropdownMenuItem
            ...
            className="shrink-0 cursor-pointer gap-2 rounded-[14px] focus:bg-[#F6F6F6] focus:[&_strong]:text-foreground! focus:[&_small]:text-muted-foreground! data-[active=true]:bg-[#F6F6F6] data-[active=true]:[&_strong]:text-foreground!"
          >
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title="切换到对话端"
        aria-label="切换到对话端"
        className="flex size-[32px] shrink-0 rotate-90 items-center justify-center rounded-[8px] text-sidebar-foreground transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
      >
        <PanelLeftClose className="size-[16px]!" />
      </button>
    </div>
  )
}
...
function CollapsedGroupLabel({ children }: { children: string }) {
  return <span className="text-[10px] leading-none text-[#464c5e]">{children}</span>
}
...
> & { nameLabel: string }) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="start"
        className="flex max-h-[320px] min-w-[180px] flex-col gap-[4px] overflow-y-auto"
      >
        {employeeAgents.map((agent) => (
          <DropdownMenuItem
            key={agent.id}
      ...
      <div className="flex w-full flex-col items-center gap-[10px]">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            ...
            >
              <PanelLeftClose className="size-[16px]! -rotate-90" />
            </button>
          </TooltipTrigger>
          <TooltipContent side="right" align="center">
            展开边栏
        ...
        <SidebarHeader className="gap-[24px] px-[20px] pt-[10px] group-data-[collapsible=icon]:px-[20px]">
          <div className="flex items-center justify-between">
            ...
            {!brandCollapsed && (
              ...
              >
                <PanelLeftClose className="size-[14px]! -rotate-90" />
              </button>
            )}
          </div>
...
function sessionAgentFor(
  session: ChatSession,
  agents: AgentProfileRead[],
): AgentProfileRead | null {
...
function sessionSubtitleFor(session: ChatSession, _agent: AgentProfileRead | null): string {
  const recent = (session.last_agent_question || session.summary || '').replace(
    /^最近回复[:：]\s*/,
    '',
  )
...
function ChatSessionFilter({
  ...
  AppSidebarChatProps,
  'sessionFilter' | 'sessionFilterOptions' | 'onSessionFilterChange'
> & { collapsed?: boolean }) {
  const current =
    sessionFilterOptions.find((option) => option.value === sessionFilter)
    || sessionFilterOptions[0]
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        {collapsed ? (
          ...
          >
            <StaffdeckIcon name="filter" size={14} className="shrink-0 text-[#858b9c]" />
          </button>
        ) : (
          <button
            type="button"
      ...
      >
        {sessionFilterOptions.map((option) => {
          const [optionName, optionCount] = option.label
            .split('·')
            .map((part) => part.trim())
            ...
            >
              <span
                className={cn(
                  ...
                  active ? 'text-[#18181a]!' : 'text-[#858b9c]!',
                )}
              >
                {optionName}
              </span>
              {optionCount && (
                <span
                  className={cn(
                    ...
                    active ? 'bg-white' : 'bg-[#f6f6f6] group-focus/filter:bg-white',
                  )}
                >
                  {optionCount}
                </span>
  ...
  return (
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col justify-between self-stretch py-[3px]">
        <span
          className="truncate text-[14px] leading-none text-[#464c5e] capitalize"
          title={title}
        >
          {title}
        </span>
        <span
          className="truncate text-[12px] leading-none text-[#757f9c]"
          title={subtitle}
        >
          {subtitle}
        </span>
      ...
      {unread && (
        <span
          className="ml-[2px] size-[7px] shrink-0 rounded-full bg-[#f5483b]"
          aria-label="未读回复"
        />
      )}
        ...
        <div className="no-scrollbar mx-[-8px] flex min-h-0 w-[calc(100%+16px)] flex-1 flex-col items-center gap-[10px] overflow-y-auto py-[2px]">
          {sessionsLoading
            ? Array.from({ length: 5 }).map((_, index) => (
                <span
                  key={index}
                  className="size-[36px] shrink-0 animate-pulse rounded-[10px] bg-[#eef0f4]"
                />
              ))
                  ...
                  <Tooltip key={session.id}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        {unread && (
                          <span
                            className="absolute right-[2px] top-[2px] size-[7px] rounded-full bg-[#f5483b] ring-[1.5px] ring-white"
                            aria-label="未读回复"
                          />
                        )}
                      </button>
      ...
      <div className="flex h-full w-(--sidebar-width) shrink-0 flex-col">
        <SidebarHeader className="gap-[24px] px-[20px] pt-[10px]">
          ...
          <div className="flex flex-col gap-[16px]">
            ...
            >
              <Globe className="size-[16px]! shrink-0" />
