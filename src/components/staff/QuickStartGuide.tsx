...
const STEPS: QuickStartStep[] = [
  {
    title: '配置模型 API Key',
    description:
      '模型是数字员工的大脑。点击「新建模型」，填写 Base URL、Model 和 API Key 即可接入。',
  ...
  },
  {
    title: 'SOP · 流程型技能',
    description:
      'SOP 是流程型技能：数字员工按步骤执行业务流程，可打断、可恢复，比固定的 workflow 更灵活。一句话描述需求，即可快速生成 SOP。',
...
function findStepTarget(step: QuickStartStep) {
  return (
    ...
    || (step.fallbackTarget ? findVisibleTarget(step.fallbackTarget) : undefined)
  )
}
...
export default function QuickStartGuide({ isAdmin }: QuickStartGuideProps) {
  ...
  function runAction() {
    const stepCurrent = steps[step]
    navigate(stepCurrent.route)
    const eventName = stepCurrent.eventName
    if (eventName) {
      window.setTimeout(() => window.dispatchEvent(new Event(eventName)), 0)
    } else {
      window.setTimeout(() => findStepTarget(stepCurrent)?.click(), 50)
    }
  }
