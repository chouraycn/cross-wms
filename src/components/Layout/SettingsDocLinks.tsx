...
import {
  Box, Typography, Card, CardContent, Chip, TextField, Button, IconButton, Tooltip, Divider, InputAdornment, FormControl, InputLabel, Select, MenuItem, Alert,
} from '@mui/material';
 ...
 * 文档链接管理子组件
 *
 * 负责：腾讯文档 API 授权状态显示、授权操作、文档链接 CRUD、在线数据输入、容积率文档管理。
 */
interface SettingsDocLinksProps {
  draft: AppSettings;
...
const SettingsDocLinks: React.FC<SettingsDocLinksProps> = ({ draft, setDraft, errors, setErrors, openInBrowser, onNavigateToVolumeDocs }) => {
  // === 认证状态 ===
  ...
  const handleAddData = useCallback(() => {
    ...
    const newEntry: OnlineDataEntry = {
      id: `data-${Date.now()}`, name: newDataName.trim(), dataType: newDataDataType,
      data: newDataContent.trim(), updatedAt: new Date().toISOString(),
    };
...
export default SettingsDocLinks;
