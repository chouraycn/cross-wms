...
const TencentDocsTab: React.FC<TencentDocsTabProps> = ({
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, maxWidth: 560 }}>
      <Typography sx={{ fontSize: '0.95rem', fontWeight: 600, color: gs.textPrimary, mb: 1 }}>腾讯文档集成</Typography>
      <Typography sx={{ fontSize: '0.8rem', color: gs.textDisabled, mb: 1 }}>配置腾讯文档 API 后，可直接在应用内读取文档内容（不嵌入网页）。所有数据本地渲染。</Typography>

      {/* OAuth */}
      <Card elevation={0} sx={{ border: `1px solid ${tdocAuth?.authenticated ? '#27A17C' : gs.border}`, borderRadius: 2, p: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
          {tdocAuth?.authenticated ? <CloudDoneIcon sx={{ color: '#27A17C', fontSize: 20 }} /> : <CloudOffIcon sx={{ color: gs.textDisabled, fontSize: 20 }} />}
          <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary }}>API 授权状态</Typography>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          <TextField label="Client ID" size="small" fullWidth placeholder="在 docs.qq.com/open/developers 注册应用获取" value={tdocClientId} onChange={(e) => setTdocClientId(e.target.value)} sx={textFieldSx} InputProps={{ startAdornment: <InputAdornment position="start"><VpnKeyIcon sx={{ fontSize: 16, color: gs.textDisabled }} /></InputAdornment> }} />
          ...
          {tdocAuth?.authenticated ? (
            ...
            <>
              ...
              <Button variant="outlined" size="small" onClick={handleTdocExchange} disabled={tdocAuthLoading || !tdocAuthCode} sx={{ borderColor: gs.textPrimary, color: gs.textPrimary, fontSize: '0.8rem', alignSelf: 'flex-start' }}>用授权码换取 Token</Button>
            </>
          )}
      ...
      {/* 文档链接管理 */}
      <Typography sx={{ fontSize: '0.9rem', fontWeight: 600, color: gs.textPrimary }}>文档链接管理</Typography>
      <Card elevation={0} sx={{ border: `1px solid ${gs.border}`, borderRadius: 2, p: 2 }}>
        <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary, mb: 1.5 }}>添加文档链接</Typography>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          <TextField label="文档链接" size="small" fullWidth placeholder="https://docs.qq.com/sheet/DZVJnQmJ4a3F2bWN5" value={newLinkUrl} onChange={(e) => { setNewLinkUrl(e.target.value); setErrors((prev) => { const n = { ...prev }; delete n['docLink.url']; return n; }); }} onPaste={handleTencentLinkPaste} InputProps={{ startAdornment: <InputAdornment position="start"><LinkIcon sx={{ fontSize: 18, color: gs.textDisabled }} /></InputAdornment> }} sx={textFieldSx} error={Boolean(errors['docLink.url'])} helperText={errors['docLink.url'] || '粘贴链接后自动识别并填充表单'} FormHelperTextProps={{ sx: { fontSize: '0.75rem', color: gs.textDisabled } }} />
          <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
            <TextField label="文档名称（选填）" size="small" sx={{ flex: 1, ...textFieldSx }} value={newLinkTitle} onChange={(e) => setNewLinkTitle(e.target.value)} placeholder="自动从链接解析" InputProps={{ endAdornment: fetchingTdocTitle ? <InputAdornment position="end"><Box component="span" sx={{ fontSize: '0.7rem', color: gs.textDisabled }}>获取中...</Box></InputAdornment> : undefined }} />
            ...
            <Button variant="contained" startIcon={<AddIcon />} onClick={handleAddLink} sx={{ backgroundColor: gs.textPrimary, '&:hover': { backgroundColor: gs.textSecondary }, height: 40, whiteSpace: 'nowrap' }}>添加</Button>
          </Box>
        </Box>
      </Card>
      ...
      {draft.tencentDocs.docLinks.length === 0 ? (
        <Box sx={{ textAlign: 'center', py: 4, border: `1px dashed ${gs.border}`, borderRadius: 2 }}>
          <DescriptionIcon sx={{ fontSize: 36, color: gs.borderDarker, mb: 1 }} />
          <Typography sx={{ color: gs.textDisabled, fontSize: '0.85rem' }}>暂无文档链接，请在上方添加</Typography>
        </Box>
      ) : (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          {draft.tencentDocs.docLinks.map((doc) => (
            <Card key={doc.id} elevation={0} sx={{ border: `1px solid ${gs.border}`, borderRadius: 2, overflow: 'visible' }}>
              <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  <Box sx={{ width: 32, height: 32, borderRadius: 1, backgroundColor: '#27A17C', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><DescriptionIcon sx={{ color: gs.bgPanel, fontSize: 18 }} /></Box>
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.title}</Typography>
                      <Chip label={doc.dataType === 'inventory' ? '库存' : doc.dataType === 'transit' ? '在途' : doc.dataType === 'warehouses' ? '仓库' : '其他'} size="small" sx={{ height: 20, fontSize: '0.7rem', backgroundColor: gs.bgHover, color: gs.textMuted }} />
                    ...
                    <Typography sx={{ fontSize: '0.75rem', color: gs.textDisabled, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.url}</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
                    <Tooltip title="在浏览器中打开"><IconButton size="small" onClick={() => openInBrowser(doc.url)} sx={{ color: gs.textMuted }}><OpenInNewIcon sx={{ fontSize: 16 }} /></IconButton></Tooltip>
                    <Tooltip title="删除链接"><IconButton size="small" onClick={() => handleRemoveLink(doc.id)} sx={{ color: gs.textDisabled, '&:hover': { color: '#EF4444' } }}><DeleteOutlineIcon sx={{ fontSize: 18 }} /></IconButton></Tooltip>
                  </Box>
                </Box>
      ...
      {/* 在线数据输入 */}
      <Typography sx={{ fontSize: '0.95rem', fontWeight: 600, color: gs.textPrimary, mb: 0.5 }}>在线数据输入</Typography>
      <Typography sx={{ fontSize: '0.8rem', color: gs.textDisabled, mb: 2 }}>支持直接输入 JSON 格式数据，用于仪表盘实时展示。无需腾讯文档 API 授权即可使用。</Typography>
      <Card elevation={0} sx={{ border: `1px solid ${gs.border}`, borderRadius: 2, p: 2 }}>
        <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary, mb: 1.5 }}>{editingDataId ? '编辑数据' : '添加数据'}</Typography>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
            ...
            {editingDataId ? (
              <Box sx={{ display: 'flex', gap: 1 }}>
                <Button variant="contained" startIcon={<SaveIcon />} onClick={handleSaveEdit} sx={{ backgroundColor: gs.textPrimary, '&:hover': { backgroundColor: gs.textSecondary }, height: 40, whiteSpace: 'nowrap' }}>保存</Button>
                <Button variant="outlined" onClick={handleCancelEdit} sx={{ height: 40, whiteSpace: 'nowrap' }}>取消</Button>
              </Box>
            ) : (
              <Button variant="contained" startIcon={<AddIcon />} onClick={handleAddData} sx={{ backgroundColor: gs.textPrimary, '&:hover': { backgroundColor: gs.textSecondary }, height: 40, whiteSpace: 'nowrap' }}>添加</Button>
            )}
          </Box>
      ...
      {draft.tencentDocs.onlineData.length === 0 ? (
        <Box sx={{ textAlign: 'center', py: 4, border: `1px dashed ${gs.border}`, borderRadius: 2 }}>
          <CodeIcon sx={{ fontSize: 36, color: gs.borderDarker, mb: 1 }} />
          <Typography sx={{ color: gs.textDisabled, fontSize: '0.85rem' }}>暂无在线数据，请在上方添加</Typography>
        </Box>
      ) : (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          {draft.tencentDocs.onlineData.map((entry) => (
            <Card key={entry.id} elevation={0} sx={{ border: `1px solid ${gs.border}`, borderRadius: 2 }}>
              <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                  <Box sx={{ width: 32, height: 32, borderRadius: 1, backgroundColor: '#3B82F6', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><CodeIcon sx={{ color: gs.bgPanel, fontSize: 18 }} /></Box>
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{entry.name}</Typography>
                      <Chip label={entry.dataType === 'inventory' ? '库存' : entry.dataType === 'transit' ? '在途' : entry.dataType === 'warehouses' ? '仓库' : '其他'} size="small" sx={{ height: 20, fontSize: '0.7rem', backgroundColor: gs.bgHover, color: gs.textMuted }} />
                    </Box>
                    <Typography sx={{ fontSize: '0.75rem', color: gs.textDisabled }}>更新：{new Date(entry.updatedAt).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}{' · '}{(() => { try { const p = JSON.parse(entry.data); return Array.isArray(p) ? p.length : 1; } catch { return 0; } })()} 条记录</Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
                    <Tooltip title="编辑"><IconButton size="small" onClick={() => handleEditData(entry)} sx={{ color: gs.textMuted }}><EditIcon sx={{ fontSize: 18 }} /></IconButton></Tooltip>
                    <Tooltip title="删除"><IconButton size="small" onClick={() => handleRemoveData(entry.id)} sx={{ color: gs.textDisabled, '&:hover': { color: '#EF4444' } }}><DeleteOutlineIcon sx={{ fontSize: 18 }} /></IconButton></Tooltip>
                  </Box>
                </Box>
              </CardContent>
            </Card>
      ...
      {/* 企业微信文档 */}
      <Typography sx={{ fontSize: '0.95rem', fontWeight: 600, color: gs.textPrimary, mb: 0.5 }}>企业微信文档</Typography>
      <Typography sx={{ fontSize: '0.8rem', color: gs.textDisabled, mb: 2 }}>通过 wecom-cli 读取企业微信文档（doc.weixin.qq.com），支持文档、智能表格、智能文档</Typography>

      <Card elevation={0} sx={{ border: `1px solid ${wecomAuth?.authorized ? '#07C160' : gs.border}`, borderRadius: 2, p: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5 }}>
          {wecomAuth?.authorized ? <CloudDoneIcon sx={{ color: '#07C160', fontSize: 20 }} /> : <CloudOffIcon sx={{ color: gs.textDisabled, fontSize: 20 }} />}
          <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary }}>企业微信授权状态</Typography>
      ...
      <Divider sx={{ my: 2 }} />
      <Typography sx={{ fontSize: '0.9rem', fontWeight: 600, color: gs.textPrimary }}>企业文档链接</Typography>

      <Card elevation={0} sx={{ border: `1px solid ${gs.border}`, borderRadius: 2, p: 2, mt: 1 }}>
        <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary, mb: 1.5 }}>添加企业文档链接</Typography>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          <TextField label="文档链接" size="small" fullWidth placeholder="https://doc.weixin.qq.com/doc/e3_xxxxxxxx" value={newWecomLinkUrl} onChange={(e) => { setNewWecomLinkUrl(e.target.value); setWecomLinkErrors((prev) => { const n = { ...prev }; delete n['wecomLink.url']; return n; }); }} onPaste={handleWecomLinkPaste} InputProps={{ startAdornment: <InputAdornment position="start"><LinkIcon sx={{ fontSize: 18, color: gs.textDisabled }} /></InputAdornment> }} sx={textFieldSx} error={Boolean(wecomLinkErrors['wecomLink.url'])} helperText={wecomLinkErrors['wecomLink.url'] || '粘贴链接后自动识别并填充表单'} FormHelperTextProps={{ sx: { fontSize: '0.75rem', color: gs.textDisabled } }} />
          <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
            <TextField label="文档名称（选填）" size="small" sx={{ flex: 1, ...textFieldSx }} value={newWecomLinkTitle} onChange={(e) => setNewWecomLinkTitle(e.target.value)} placeholder="自动从链接解析" InputProps={{ endAdornment: fetchingWecomTitle ? <InputAdornment position="end"><Box component="span" sx={{ fontSize: '0.7rem', color: gs.textDisabled }}>获取中...</Box></InputAdornment> : undefined }} />
            <FormControl size="small" sx={{ minWidth: 140 }}>
              <InputLabel sx={{ fontSize: '0.875rem' }}>数据类型</InputLabel>
      ...
      {(draft.wecomDocs?.docLinks ?? []).length === 0 ? (
        <Box sx={{ textAlign: 'center', py: 3, border: `1px dashed ${gs.border}`, borderRadius: 2, mt: 1.5 }}>
          <DescriptionIcon sx={{ fontSize: 32, color: gs.borderDarker, mb: 1 }} />
          <Typography sx={{ color: gs.textDisabled, fontSize: '0.85rem' }}>暂无企业文档链接，请在上方添加</Typography>
        </Box>
      ) : (
          ...
          {(draft.wecomDocs?.docLinks ?? []).map((doc) => {
            ...
            return (
              <Card key={doc.id} elevation={0} sx={{ border: `1px solid ${gs.border}`, borderRadius: 2, overflow: 'visible' }}>
                <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Box sx={{ width: 32, height: 32, borderRadius: 1, backgroundColor: '#07C160', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><DescriptionIcon sx={{ color: gs.bgPanel, fontSize: 18 }} /></Box>
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Typography sx={{ fontSize: '0.85rem', fontWeight: 600, color: gs.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.title}</Typography>
                        <Chip label={doc.dataType === 'inventory' ? '库存' : doc.dataType === 'transit' ? '在途' : doc.dataType === 'warehouses' ? '仓库' : '其他'} size="small" sx={{ height: 20, fontSize: '0.7rem', backgroundColor: gs.bgHover, color: gs.textMuted }} />
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexShrink: 0 }}>
                      <Tooltip title="在浏览器中打开"><IconButton size="small" onClick={() => openInBrowser(doc.url)} sx={{ color: gs.textMuted }}><OpenInNewIcon sx={{ fontSize: 16 }} /></IconButton></Tooltip>
                      <Tooltip title="删除链接"><IconButton size="small" onClick={() => handleRemoveWecomLink(doc.id)} sx={{ color: gs.textDisabled, '&:hover': { color: '#EF4444' } }}><DeleteOutlineIcon sx={{ fontSize: 18 }} /></IconButton></Tooltip>
                    </Box>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mt: 1, ml: 5 }}>
                    <FormControl size="small" sx={{ minWidth: 130 }}>
