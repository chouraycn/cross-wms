...
export const warehouseHandlers = [
  ...
  http.delete('/api/warehouses/:id', async ({ params }) => {
    await delay(200);
    const { id } = params;
    const idx = warehouses.findIndex(w => w.id === id);
    if (idx === -1) {
...
export const transitHandlers = [
  // GET /api/transit - 获取运单列表
  http.get('/api/transit', async ({ request }) => {
    await delay(200);
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get('page') || '1');
    const pageSize = parseInt(url.searchParams.get('pageSize') || '10');
  ...
  // GET /api/transit/:id - 获取运单详情
  http.get('/api/transit/:id', async ({ params }) => {
    await delay(150);
    const { id } = params;
  ...
  // POST /api/transit - 新建运单
  http.post('/api/transit', async ({ request }) => {
    await delay(300);
  ...
  // PUT /api/transit/:id/status - 更新运单状态
  http.put('/api/transit/:id/status', async ({ params, request }) => {
    await delay(200);
    const { id } = params;
...
export const inventoryHandlers = [
  ...
  // GET /api/inventory/warehouse/:warehouseId - 获取指定仓库库存
  http.get('/api/inventory/warehouse/:warehouseId', async ({ params }) => {
    await delay(150);
...
export const recordHandlers = [
  ...
  http.get('/api/inbound', async ({ request }) => {
    await delay(200);
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get('page') || '1');
    const pageSize = parseInt(url.searchParams.get('pageSize') || '10');
  ...
  http.get('/api/outbound', async ({ request }) => {
    await delay(200);
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get('page') || '1');
    const pageSize = parseInt(url.searchParams.get('pageSize') || '10');
...
export const dashboardHandlers = [
  ...
  // GET /api/dashboard/volume-history - 容积率趋势
  http.get('/api/dashboard/volume-history', async ({ request }) => {
    await delay(200);
    const url = new URL(request.url);
  ...
  // GET /api/dashboard/warehouse-volume - 仓库容积分布
  http.get('/api/dashboard/warehouse-volume', async () => {
    await delay(200);
    return HttpResponse.json(success(createMockWarehouseVolumeData(warehouses)));
  }),

  // GET /api/dashboard/category-volume - 品类分布
  http.get('/api/dashboard/category-volume', async () => {
    await delay(200);
    return HttpResponse.json(success(createMockCategoryVolumeData()));
  }),

  // GET /api/dashboard/transit-efficiency - 在途时效
  http.get('/api/dashboard/transit-efficiency', async () => {
    await delay(200);
    return HttpResponse.json(success(createMockTransitEfficiencyData()));
  }),

  // GET /api/dashboard/heatmap - 热力图数据
  http.get('/api/dashboard/heatmap', async ({ request }) => {
    await delay(300);
    const url = new URL(request.url);
  ...
  // GET /api/dashboard/inventory-alerts - 库存预警
  http.get('/api/dashboard/inventory-alerts', async () => {
    await delay(200);
