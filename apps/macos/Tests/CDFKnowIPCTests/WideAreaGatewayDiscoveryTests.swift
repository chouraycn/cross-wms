...
import Foundation
import Testing
@testable import CDFKnowDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("CDFKNOW_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("CDFKNOW_WIDE_AREA_DOMAIN", "cdfknow.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("CDFKNOW_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("CDFKNOW_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._cdfknow-gw._tcp.cdfknow.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.cdfknow.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (CDFKnow)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/cdfknow/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (CDFKnow)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/cdfknow/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._cdfknow-gw._tcp.cdfknow.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
