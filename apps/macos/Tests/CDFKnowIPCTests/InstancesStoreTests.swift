...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: CDFKnowProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CDFKnowProtocol.AnyCodable] = [
            "presence": .init([CDFKnowProtocol.AnyCodable(entry)]),
        ]
        let payload = CDFKnowProtocol.AnyCodable(payloadMap)
