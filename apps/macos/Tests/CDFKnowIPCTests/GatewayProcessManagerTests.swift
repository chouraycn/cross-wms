...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["CDFKNOW_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "cdfknow-gateway",
                executablePath: "/tmp/cdfknow-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 cdfknow-gateway @ /tmp/cdfknow-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
