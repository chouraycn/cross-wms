...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(CDFKnowCapability.browser.rawValue))
        #expect(!commands.contains(CDFKnowBrowserCommand.proxy.rawValue))
        #expect(commands.contains(CDFKnowCanvasCommand.present.rawValue))
        #expect(commands.contains(CDFKnowSystemCommand.notify.rawValue))
    }
