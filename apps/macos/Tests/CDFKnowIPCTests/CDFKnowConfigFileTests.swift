...
@Suite(.serialized)
struct CDFKnowConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("cdfknow-config-\(UUID().uuidString)")
            .appendingPathComponent("cdfknow.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["CDFKNOW_CONFIG_PATH": override]) {
            #expect(CDFKnowConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["CDFKNOW_CONFIG_PATH": override]) {
            ...
            #expect(CDFKnowConfigFile.remoteGatewayPort() == 19999)
            #expect(CDFKnowConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(CDFKnowConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(CDFKnowConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(CDFKnowConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(CDFKnowConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["CDFKNOW_CONFIG_PATH": override]) {
            ...
            CDFKnowConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = CDFKnowConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["CDFKNOW_CONFIG_PATH": override]) {
            ...
            CDFKnowConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = CDFKnowConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["CDFKNOW_CONFIG_PATH": override]) {
            ...
            CDFKnowConfigFile.clearRemoteGatewayUrl()
            let root = CDFKnowConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            "CDFKNOW_CONFIG_PATH": nil,
            "CDFKNOW_STATE_DIR": dir,
        ]) {
            #expect(CDFKnowConfigFile.stateDirURL().path == dir)
            #expect(CDFKnowConfigFile.url().path == "\(dir)/cdfknow.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("cdfknow-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("cdfknow.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "CDFKNOW_STATE_DIR": stateDir.path,
            "CDFKNOW_CONFIG_PATH": configPath.path,
        ]) {
            CDFKnowConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-cdfknow-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = CDFKnowConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try CDFKnowConfigFile.withTestingFileLock {
                CDFKnowConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                let loaded = CDFKnowConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = CDFKnowConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
