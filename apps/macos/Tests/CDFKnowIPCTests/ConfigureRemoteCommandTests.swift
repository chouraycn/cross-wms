...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("cdfknow-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.cdfknow.mac", "ai.cdfknow.mac.debug"]
        let keys = [
            "cdfknow.connectionMode",
            "cdfknow.remoteTarget",
            "cdfknow.onboardingSeen",
            "cdfknow.onboardingVersion",
            "cdfknow.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["CDFKNOW_CONFIG_PATH": configURL.path]) {
            let output = try configureRemote(.init(
                ...
                identity: nil,
                projectRoot: nil,
                cliPath: "/opt/homebrew/bin/cdfknow"))
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "cdfknow.connectionMode") == "remote")
                #expect(defaults.string(forKey: "cdfknow.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "cdfknow.onboardingSeen") == true)
                #expect(defaults.string(forKey: "cdfknow.remoteCliPath") == "/opt/homebrew/bin/cdfknow")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("cdfknow-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("cdfknow-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("cdfknow-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["CDFKNOW_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
