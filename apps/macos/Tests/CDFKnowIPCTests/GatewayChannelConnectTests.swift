...
struct GatewayChannelConnectTests {
    ...
    private func withTemporaryStateDir<T>(_ operation: () async throws -> T) async throws -> T {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["CDFKNOW_STATE_DIR"]
        setenv("CDFKNOW_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("CDFKNOW_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("CDFKNOW_STATE_DIR")
            }
        ...
        try await self.withTemporaryStateDir {
            ...
            let channel = try GatewayChannelActor(
                ...
                connectOptions: GatewayConnectOptions(
                    ...
                    commands: [],
                    permissions: [:],
                    clientId: "cdfknow-macos",
                    clientMode: "ui",
                    clientDisplayName: "CDFKnow macOS Debug CLI"))
