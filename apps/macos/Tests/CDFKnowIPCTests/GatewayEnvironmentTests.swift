...
struct GatewayEnvironmentTests {
    @Test func `semver parses common forms`() {
        ...
        // Product-prefixed output from `cdfknow --version` should NOT parse as semver
        // (the prefix must be stripped by the caller, not the parser).
        #expect(Semver.parse("CDFKnow 2026.3.23-1") == nil)
    }

    @Test func `gateway version output strips product prefix before parsing`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("  CDFKnow 2026.3.23-1 \n")
    ...
    @Test func `gateway version output strips trailing commit hash`() {
        let normalized = GatewayEnvironment.normalizeGatewayVersionOutput("CDFKnow 2026.4.2 (d74a122)")
        ...
        let normalized2 = GatewayEnvironment.normalizeGatewayVersionOutput("CDFKnow 2026.4.2-1 (d74a122)")
    ...
    @Test func `gateway port defaults and respects override`() async {
        ...
        await TestIsolation.withIsolatedState(
            env: ["CDFKNOW_CONFIG_PATH": configPath],
