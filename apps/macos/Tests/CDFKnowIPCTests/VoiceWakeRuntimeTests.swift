...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["cdfknow"]
        let text = "hey cdfknow"
    ...
    @Test func `trigger only rejects trailing wake word mentions in ordinary speech`() {
        let triggers = ["cdfknow"]
        let text = "tell me about cdfknow"
        #expect(!VoiceWakeRuntime._testIsTriggerOnly(text, triggers: triggers))
    }
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["cdfknow"]
        let text = "uh cdfknow"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "cdfknow")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["cdfknow", "hey cdfknow"]
        let text = "hey cdfknow"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey cdfknow")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey cdfknow do thing"
        let config = WakeWordGateConfig(triggers: ["cdfknow"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["cdfknow"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "cdfknow")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "cdfknow tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["cdfknow", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["cdfknow", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "cdfknow")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["cdfknow"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "cdfknow", start: 0.2, duration: 0.1, range: transcript.range(of: "cdfknow")),
