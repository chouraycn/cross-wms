...
enum PermissionManager {
    ...
    private static func ensureCapability(_ cap: Capability, interactive: Bool) async -> Bool {
        ...
        case .sound, .keychain, .fileWatcher, .embedding, .database:
            // Feature capabilities don't need system permission
            return true
        }
    }
    ...
    static func status(_ caps: [Capability] = Capability.allCases) async -> [Capability: Bool] {
        ...
        for cap in caps {
            ...
            case .sound, .keychain, .fileWatcher, .embedding, .database:
                // Feature capabilities - always considered granted
                results[cap] = true
            }
        }
        return results
...
enum AppleScriptPermission {
    private static let logger = Logger(label: "com.crosswms.appleScriptPermission")
