...
const FEISHU_USER_AGENT = `cdf-know-feishu/${pluginVersion}/${process.platform}`;
...
export const FEISHU_HTTP_TIMEOUT_ENV_VAR = "FEISHU_HTTP_TIMEOUT_MS";
export const FEISHU_HTTP_TIMEOUT_MS = 30_000;
export const FEISHU_HTTP_TIMEOUT_MAX_MS = 120_000;

export function resolveConfiguredHttpTimeoutMs(creds: {
  httpTimeoutMs?: number;
  config?: Pick<FeishuConfig, "httpTimeoutMs">;
}): number {
  const fromConfig = creds.config?.httpTimeoutMs ?? creds.httpTimeoutMs;
  if (typeof fromConfig === "number" && fromConfig > 0) {
    return Math.min(fromConfig, FEISHU_HTTP_TIMEOUT_MAX_MS);
  }
  const fromEnv = typeof process !== "undefined" ? process.env[FEISHU_HTTP_TIMEOUT_ENV_VAR] : undefined;
  if (fromEnv) {
    const parsed = Number.parseInt(fromEnv, 10);
    if (Number.isFinite(parsed) && parsed > 0) {
      return Math.min(parsed, FEISHU_HTTP_TIMEOUT_MAX_MS);
    }
  }
