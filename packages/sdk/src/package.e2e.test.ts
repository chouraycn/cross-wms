...
const WORKSPACE_PACKAGE_NAMES = [
  "@cdf-know/gateway-protocol",
  "@cdf-know/gateway-client",
  "@cdf-know/sdk",
] as const;
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@cdf-know/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
describe("OpenClaw SDK package e2e", () => {
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@cdf-know/sdk")?.tarball ?? "";
    ...
    const importScript = `
      import { GatewayClientTransport, OpenClaw, normalizeGatewayEvent } from "@cdf-know/sdk";
