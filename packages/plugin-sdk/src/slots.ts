import type { PluginCapabilityKind, PluginSlotKey } from './types';
...
export class Slots {
  private slots: Record<PluginSlotKey, string> = {
    memory: 'memory-core',
    contextEngine: 'legacy',
  };

  get(key: PluginSlotKey): string | undefined {
    return this.slots[key];
  }

  set(key: PluginSlotKey, id: string): void {
    this.slots[key] = id;
  }

  getAll(): Record<PluginSlotKey, string> {
    return { ...this.slots };
  }

  applySelection(pluginId: string, kind?: PluginCapabilityKind | PluginCapabilityKind[]): SlotSelectionResult {
