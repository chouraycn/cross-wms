...
import { getStructuredFrontmatterValue, parseFrontmatterBlock } from "./frontmatter.js";
...
describe("getStructuredFrontmatterValue", () => {
  it("returns the nested metadata object as a real object", () => {
    const content = `---
name: nested-yaml
metadata:
  openclaw:
...
---
`;
    const value = getStructuredFrontmatterValue(content, "metadata") as Record<string, unknown>;
    expect(value).toBeDefined();
    const openclaw = value.openclaw as Record<string, unknown>;
...
---
`;
    expect(getStructuredFrontmatterValue(content, "name")).toBeUndefined();
    expect(getStructuredFrontmatterValue(content, "missing")).toBeUndefined();
  });

  it("returns undefined when no frontmatter is present", () => {
    expect(getStructuredFrontmatterValue("# Plain markdown", "metadata")).toBeUndefined();
  });
});
