...
/**
 * Returns the structured value of a frontmatter key, preserving nested objects/arrays
 * (e.g. `metadata: { openclaw: { events: [...] } }` instead of the default JSON-encoded string form).
 * Returns `undefined` when the key is missing, scalar, or the YAML library produced no
 * structured representation.
 */
export function getStructuredFrontmatterValue(
  content: string,
  key: string,
): unknown {
  ...
  try {
    ...
    // Only return structured values (objects/arrays); scalars are still available via parseFrontmatterBlock.
