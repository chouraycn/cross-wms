...
hdiutil create -volname "CrossWMS" \
  -srcfolder "$APP_PATH" \
  -ov -format UDZO \
...
NOTES_CONTENT="CrossWMS v${VERSION} 发布"
...
if gh release view "v${VERSION}" &>/dev/null 2>&1; then
  ...
  gh release upload "v${VERSION}" \
    "$SCRIPT_DIR/release/$DMG_NAME#CrossWMS DMG" \
