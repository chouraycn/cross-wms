...
// ========== 嵌入模式（CrossWMS 桌面端 iframe 加载）登录态处理 ==========
// 桌面端没有独立登录体系：后端在「无 token」时回退到 default-user(admin)。
// 因此嵌入模式下，若本地无会话，直接以 default-user 身份进入，跳过登录页。
// 同时支持通过 postMessage 接收父窗口下发的真实会话（真正的主程序登录态透传）。
const STAFFDECK_MSG_REQUEST_AUTH = 'STAFFDECK_REQUEST_AUTH';
const STAFFDECK_MSG_AUTH = 'STAFFDECK_AUTH';

const DEFAULT_DESKTOP_SESSION: EnterpriseAuthSession = {
  token: '',
  user: {
    id: 'default-user',
    tenant_id: 'default',
...
function detectEmbedded(): boolean {
  // 在 iframe 内，或非顶层窗口；或显式带 ?embedded=1（便于调试/独立打开）
  if (typeof window === 'undefined') return false;
  const u = new URLSearchParams(window.location.search);
  if (u.get('embedded') === '1') return true;
  try {
...
export default function App() {
  ...
  const [auth, setAuth] = useState<EnterpriseAuthSession | null>(() => {
    ...
    // 嵌入模式下无本地会话 → 直接以 default-user 进入，避免强制登录
    if (detectEmbedded()) return DEFAULT_DESKTOP_SESSION;
    return null;
  });
  ...
  // ========== 跨 iframe 登录态透传桥 ==========
  // 嵌入模式下，优先复用父窗口（CrossWMS 主程序）已登录的真实会话：
  // 1) 挂载时若无本地会话，向父窗口请求会话；
  // 2) 收到父窗口下发的 STAFFDECK_AUTH 后写入 localStorage 并刷新以应用真实身份。
  useEffect(() => {
    if (!detectEmbedded()) return undefined;
    if (window.opener === null && window.parent === window.self) return undefined;

    const onMessage = (event: MessageEvent) => {
      if (event.data?.type !== STAFFDECK_MSG_AUTH) return;
      const incoming = event.data?.session as EnterpriseAuthSession | undefined;
      // 允许 token='' 的 desktop 默认会话（后端兜底 default-user）。
      // 原先 !incoming?.token 把空 token 过滤掉，导致 desktop session 被忽略 → 闪动循环。
      if (!incoming?.user?.id) return;
      setEnterpriseAuthSession(incoming);
      // embedded 模式下身份由 React state 驱动，直接 setAuth 应用真实身份，
      // 不再整页 reload —— 否则每次进入数字员工都会出现二次白屏。
      setAuth(incoming);
      setAuthChecked(true);
    };
    window.addEventListener('message', onMessage);

    // 本地无会话时向父窗口请求（父窗口会回复 STAFFDECK_AUTH）
    if (!getEnterpriseAuthSession()) {
      try {
        window.parent.postMessage({ type: STAFFDECK_MSG_REQUEST_AUTH }, '*');
      } catch {
        /* ignore */
      }
    }
  ...
  return (
    <TooltipProvider>
      {/* 子路径嵌入：iframe 经 /staffdeck-app/ 加载，BrowserRouter 必须带 basename，
          否则 navigate('/workspace/chat/:id') 会写成绝对路径 /workspace/chat/:id，
          被主程序 SPA fallback 接管 → iframe 内白屏。带 basename 后变为
          /staffdeck-app/workspace/chat/:id，命中数字员工专属 fallback。 */}
      <BrowserRouter basename="/staffdeck-app">
        <Routes>
          <Route
            path="/*"
            element={
